	...
	public class CursorEditor : UITypeEditor
	{
		...
		private class CursorUI : ListBox
		{
			...
			public CursorUI(UITypeEditor editor)
			{
				...
				{
					foreach (object item in this.cursorConverter.GetStandardValues())
					{
						base.Items.Add(item);
					}
				}
			...
			protected override void OnDrawItem(DrawItemEventArgs die)
			{
				...
				if (die.Index != -1)
				{
					...
					die.Graphics.DrawString(s, font, brush, (float)(die.Bounds.X + 36), (float)(die.Bounds.Y + (die.Bounds.Height - font.Height) / 2));
					brush.Dispose();
				}
			}
