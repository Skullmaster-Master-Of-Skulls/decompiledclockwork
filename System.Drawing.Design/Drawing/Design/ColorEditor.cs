...
namespace System.Drawing.Design
{
	...
	public class ColorEditor : UITypeEditor
	{
		...
		private class ColorPalette : Control
		{
				...
				for (int i = 0; i < 8; i++)
				{
					for (int j = 0; j < 8; j++)
					{
						if (this.SelectedColor.Equals(this.GetColorFromCell(j, i)))
						{
							...
							ColorEditor.ColorPalette.FillRectWithCellBounds(j, i, ref rect);
			...
			{
				Rectangle rect = default(Rectangle);
				ColorEditor.ColorPalette.FillRectWithCellBounds(this.focus.X, this.focus.Y, ref rect);
				base.Invalidate(Rectangle.Inflate(rect, 5, 5));
			...
			protected override void OnPaint(PaintEventArgs pe)
			{
				...
				for (int i = 0; i < 8; i++)
				{
					for (int j = 0; j < 8; j++)
					{
						...
						ColorEditor.ColorPalette.PaintValue(colorFromCell, graphics, rect);
					}
				}
			}
			...
			public class ColorPaletteAccessibleObject : Control.ControlAccessibleObject
			{
				...
				public ColorPaletteAccessibleObject(ColorEditor.ColorPalette owner) : base(owner)
				{
		...
		private class ColorUI : Control
		{
			...
			private void InitializeComponent()
			{
				...
				foreach (object item in this.ColorValues)
				{
					this.lbCommon.Items.Add(item);
				}
				this.lbSystem.Items.Clear();
				foreach (object item2 in this.SystemColorValues)
				{
		...
		private class CustomColorDialog : ColorDialog
		{
			...
			protected override IntPtr HookProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam)
			{
				...
				case 272:
				{
					...
					NativeMethods.EnableWindow(dlgItem, false);
				...
				case 273:
				{
					...
					{
						bool[] err = new bool[1];
						byte red = (byte)NativeMethods.GetDlgItemInt(hwnd, 706, err, false);
						byte green = (byte)NativeMethods.GetDlgItemInt(hwnd, 707, err, false);
						byte blue = (byte)NativeMethods.GetDlgItemInt(hwnd, 708, err, false);
						base.Color = Color.FromArgb((int)red, (int)green, (int)blue);
