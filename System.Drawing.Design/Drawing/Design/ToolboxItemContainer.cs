...
namespace System.Drawing.Design
{
	...
	public class ToolboxItemContainer : ISerializable
	{
		...
		{
			ICollection result = this._filter;
					...
					if (array != null)
					{
						...
						else
						{
							...
							for (short num3 = 0; num3 < num2; num3 += 1)
							{
								...
								array2[(int)num3] = new ToolboxItemFilterAttribute(filterString, (ToolboxItemFilterType)filterType);
							}
							this._filter = array2;
						}
					}
				...
				else if (creators != null)
				{
					foreach (object obj in creators)
					{
						...
						if (this._dataObject.GetDataPresent(toolboxItemCreator.Format))
						{
							...
							if (toolboxItem != null)
							{
								result = ToolboxItemContainer.MergeFilter(toolboxItem);
								break;
							}
						}
					}
		...
		public virtual ToolboxItem GetToolboxItem(ICollection creators)
		{
			...
			if (this._toolboxItem == null)
			{
				if (this._dataObject.GetDataPresent("CF_TOOLBOXITEMCONTAINER_CONTENTS"))
				{
					...
					if (this._toolboxItem == null)
					{
						this._toolboxItem = new ToolboxItemContainer.BrokenToolboxItem(exceptionString);
					}
		...
		private class BrokenToolboxItem : ToolboxItem
		{
			...
			protected override IComponent[] CreateComponentsCore(IDesignerHost host)
			{
				...
				{
					throw new InvalidOperationException(SR.GetString("ToolboxServiceBadToolboxItemWithException", new object[]
					{
						this._exceptionString
					}));
				}
		...
		private sealed class ToolboxItemSerializer : ISerializable
		{
			...
			private ToolboxItemSerializer(SerializationInfo info, StreamingContext context)
			{
				...
				ToolboxItemContainer.ToolboxItemSerializer._formatter.Binder = new ToolboxItemContainer.ToolboxSerializationBinder(name);
				try
				{
					this._toolboxItem = (ToolboxItem)ToolboxItemContainer.ToolboxItemSerializer._formatter.Deserialize(new MemoryStream(buffer));
				}
				finally
				{
