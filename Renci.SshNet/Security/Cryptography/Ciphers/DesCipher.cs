...
namespace Renci.SshNet.Security.Cryptography.Ciphers
{
	...
	{
		// Token: 0x06000728 RID: 1832 RVA: 0x0001865A File Offset: 0x0001685A
		public DesCipher(byte[] key, CipherMode mode, CipherPadding padding) : base(key, 8, mode, padding)
		{
		}

		// Token: 0x06000729 RID: 1833 RVA: 0x00018668 File Offset: 0x00016868
		public override int EncryptBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
		{
			...
			{
				this._encryptionKey = this.GenerateWorkingKey(true, base.Key);
			}
			DesCipher.DesFunc(this._encryptionKey, inputBuffer, inputOffset, outputBuffer, outputOffset);
			return (int)base.BlockSize;
		}

		// Token: 0x0600072A RID: 1834 RVA: 0x000186DC File Offset: 0x000168DC
		public override int DecryptBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
		{
			...
			{
				this._decryptionKey = this.GenerateWorkingKey(false, base.Key);
			}
			DesCipher.DesFunc(this._decryptionKey, inputBuffer, inputOffset, outputBuffer, outputOffset);
			return (int)base.BlockSize;
		}

		// Token: 0x0600072B RID: 1835 RVA: 0x00018750 File Offset: 0x00016950
		protected int[] GenerateWorkingKey(bool encrypting, byte[] key)
		{
			...
			for (int i = 0; i < 56; i++)
			{
				int num = (int)DesCipher.pc1[i];
				array2[i] = (((short)key[(int)((uint)num >> 3)] & DesCipher.bytebit[num & 7]) != 0);
			}
			for (int j = 0; j < 16; j++)
			{
				int num2;
				...
				for (int k = 0; k < 28; k++)
				{
					int num4 = k + (int)DesCipher.totrot[j];
					if (num4 < 28)
					{
				...
				for (int l = 28; l < 56; l++)
				{
					int num4 = l + (int)DesCipher.totrot[j];
					if (num4 < 56)
					{
				...
				for (int m = 0; m < 24; m++)
				{
					if (array3[(int)DesCipher.pc2[m]])
					{
						array[num2] |= DesCipher.bigbyte[m];
					}
					if (array3[(int)DesCipher.pc2[m + 24]])
					{
						array[num3] |= DesCipher.bigbyte[m];
					}
				}
			}
		...
		// Token: 0x0600072C RID: 1836 RVA: 0x0001892C File Offset: 0x00016B2C
		...
		// Token: 0x0600072D RID: 1837 RVA: 0x00018960 File Offset: 0x00016B60
		protected static void DesFunc(int[] wKey, byte[] input, int inOff, byte[] outBytes, int outOff)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				num3 = (num2 << 28 | num2 >> 4);
				num3 ^= (uint)wKey[i * 4];
				uint num4 = DesCipher.SP7[(int)(num3 & 63U)];
				num4 |= DesCipher.SP5[(int)(num3 >> 8 & 63U)];
				num4 |= DesCipher.SP3[(int)(num3 >> 16 & 63U)];
				num4 |= DesCipher.SP1[(int)(num3 >> 24 & 63U)];
				num3 = (num2 ^ (uint)wKey[i * 4 + 1]);
				num4 |= DesCipher.SP8[(int)(num3 & 63U)];
				num4 |= DesCipher.SP6[(int)(num3 >> 8 & 63U)];
				num4 |= DesCipher.SP4[(int)(num3 >> 16 & 63U)];
				num4 |= DesCipher.SP2[(int)(num3 >> 24 & 63U)];
				num ^= num4;
				num3 = (num << 28 | num >> 4);
				num3 ^= (uint)wKey[i * 4 + 2];
				num4 = DesCipher.SP7[(int)(num3 & 63U)];
				num4 |= DesCipher.SP5[(int)(num3 >> 8 & 63U)];
				num4 |= DesCipher.SP3[(int)(num3 >> 16 & 63U)];
				num4 |= DesCipher.SP1[(int)(num3 >> 24 & 63U)];
