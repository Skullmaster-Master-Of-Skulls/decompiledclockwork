	...
	internal class TcpServerTransportSink : IServerChannelSink, IChannelSinkBase
	{
		...
		internal void ServiceRequest(object state)
		{
			...
			try
			{
				...
				{
					IMessage message;
					serverProcessing = this._nextSink.ProcessMessage(serverChannelSinkStack, null, transportHeaders, requestStream, out message, out headers, out contentStream);
				}
				finally
				{
					if (flag)
			...
			case ServerProcessing.Complete:
				...
				tcpServerSocketHandler.SendResponse(headers, contentStream);
				break;
