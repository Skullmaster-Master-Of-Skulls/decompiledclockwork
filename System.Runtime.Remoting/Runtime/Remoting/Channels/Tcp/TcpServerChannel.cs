	...
	public class TcpServerChannel : IChannelReceiver, IChannel, ISecurableChannel
	{
		...
		public TcpServerChannel(IDictionary properties, IServerChannelSinkProvider sinkProvider, IAuthorizeRemotingConnection authorizeCallback)
		{
			...
			if (properties != null)
			{
				foreach (object obj in properties)
				{
					...
					string key;
					switch (key = (string)dictionaryEntry.Key)
					{
					case "name":
		...
		private void AcceptSocketCallback(IAsyncResult ar)
		{
			...
			try
			{
				...
				if (socket == null)
				{
					throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Socket_Accept"), new object[]
					{
				...
				tcpServerSocketHandler = new TcpServerSocketHandler(socket, CoreChannel.RequestQueue, stream);
				WindowsIdentity windowsIdentity = null;
				flag = false;
				if (this._secure)
				{
					windowsIdentity = this.Authenticate(ref stream, tcpServerSocketHandler);
					tcpServerSocketHandler = new TcpServerSocketHandler(socket, CoreChannel.RequestQueue, stream);
					if (this._authorizeRemotingConnection != null && !this._authorizeRemotingConnection.IsConnectingIdentityAuthorized(windowsIdentity))
					{
				...
				tcpServerSocketHandler.DataArrivedCallback = new WaitCallback(this._transportSink.ServiceRequest);
			...
			catch (Exception ex)
			{
				try
				{
					if (tcpServerSocketHandler != null)
					{
						tcpServerSocketHandler.SendErrorResponse(ex, false);
					}
					if (socket != null)
					{
						if (flag)
		...
		private WindowsIdentity Authenticate(ref Stream netStream, TcpServerSocketHandler streamManager)
		{
			...
			try
			{
				...
				negotiateStream.AuthenticateAsServer((NetworkCredential)CredentialCache.DefaultCredentials, this._protectionLevel, requiredImpersonationLevel);
				netStream = negotiateStream;
				result = (WindowsIdentity)negotiateStream.RemoteIdentity;
			}
			catch
			{
