...
namespace System.Runtime.Remoting.Channels.Tcp
{
	...
	internal abstract class TcpSocketHandler : SocketHandler
	{
		...
		protected void ReadVersionAndOperation(out ushort operation)
		{
			...
			if (b != 1 || b2 != 0)
			{
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Tcp_UnknownProtocolVersion"), new object[]
				{
					b.ToString(CultureInfo.CurrentCulture) + "." + b2.ToString(CultureInfo.CurrentCulture)
				}));
			}
		...
		protected void ReadContentLength(out bool chunked, out int contentLength)
		{
			...
			throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Tcp_ExpectingContentLengthHeader"), new object[]
			{
		...
		protected void ReadToEndOfHeaders(BaseTransportHeaders headers)
		{
			...
			for (ushort num = base.ReadUInt16(); num != 0; num = base.ReadUInt16())
			{
				if (num == 1)
				{
					string key = this.ReadCountedString();
					string value = this.ReadCountedString();
					headers[key] = value;
				}
				else if (num == 4)
				{
					...
					string requestUri;
					if (TcpChannelHelper.ParseURL(text2, out requestUri) == null)
					{
						requestUri = text2;
					}
				...
				else if (num == 6)
				{
					...
					string contentType = this.ReadCountedString();
				...
				else
				{
					...
					case 4:
						...
						break;
					default:
						throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Tcp_UnknownHeaderType"), new object[]
						{
							num,
							b
						}));
			...
			if (flag)
			{
				...
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Tcp_GenericServerError"), new object[]
				{
					text
				}));
			}
			...
			if (enumerator != null)
			{
				while (enumerator.MoveNext())
				{
					...
					if (!StringHelper.StartsWithDoubleUnderscore(text))
					{
						...
						{
							this.WriteContentTypeHeader(value, outputStream);
						}
						else
						{
							this.WriteCustomHeader(text, value, outputStream);
						}
					}
				}
			}
		...
		protected string ReadCountedString()
		{
			...
			default:
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Tcp_UnrecognizedStringFormat"), new object[]
				{
		...
		private void ReadAndVerifyHeaderFormat(string headerName, byte expectedFormat)
		{
			...
			if (b != expectedFormat)
			{
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Tcp_IncorrectHeaderFormat"), new object[]
				{
					expectedFormat,
					headerName
				}));
