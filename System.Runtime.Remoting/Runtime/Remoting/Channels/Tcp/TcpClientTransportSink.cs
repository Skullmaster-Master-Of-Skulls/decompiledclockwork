...
namespace System.Runtime.Remoting.Channels.Tcp
{
	...
	internal class TcpClientTransportSink : BaseChannelSinkWithProperties, IClientChannelSink, IChannelSinkBase
	{
		...
		private Stream CreateAuthenticatedStream(Stream netStream, string machinePortAndSid)
		{
			...
			{
				credential = new NetworkCredential(this._securityUserName, this._securityPassword, this._securityDomain);
			}
			else
			{
			...
			try
			{
				...
				negotiateStream.AuthenticateAsClient(credential, this._spn, this._protectionLevel, this._tokenImpersonationLevel);
			}
			catch (IOException innerException)
			{
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Tcp_AuthenticationFailed"), new object[0]), innerException);
			}
		...
		internal TcpClientTransportSink(string channelURI, TcpClientChannel channel)
		{
			...
			string text;
			string uriString = TcpChannelHelper.ParseURL(channelURI, out text);
			...
			{
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Tcp_UrlMustHavePort"), new object[]
				{
					channelURI
				}));
			}
			...
			IPAddress.TryParse(this.m_machineName, out ipaddress);
			if (ipaddress != null && (ipaddress.IsIPv6LinkLocal || ipaddress.IsIPv6SiteLocal))
			{
				this.m_machineName = "[" + uri.DnsSafeHost + "]";
			}
		...
		private TcpClientSocketHandler SendRequestWithRetry(IMessage msg, ITransportHeaders requestHeaders, Stream requestStream)
		{
			...
			try
			{
				tcpClientSocketHandler = (TcpClientSocketHandler)this.ClientSocketCache.GetSocket(machinePortAndSid, openNew);
				tcpClientSocketHandler.SendRequest(msg, requestHeaders, requestStream);
			}
			catch (SocketException)
			{
				int num = 0;
				while (num < this._retryCount && canSeek && flag)
				{
					try
					{
		...
		public override ICollection Keys
		{
			get
			{
				...
				{
					TcpClientTransportSink.s_keySet = new ArrayList(6)
					{
						"username",
						"password",
						"domain",
