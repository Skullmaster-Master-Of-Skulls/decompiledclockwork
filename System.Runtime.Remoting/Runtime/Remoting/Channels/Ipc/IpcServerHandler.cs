	...
	internal class IpcServerHandler : TcpSocketHandler
	{
		...
		protected new void ReadToEndOfHeaders(BaseTransportHeaders headers)
		{
			...
			for (ushort num = base.ReadUInt16(); num != 0; num = base.ReadUInt16())
			{
				if (num == 1)
				{
					string key = base.ReadCountedString();
					string value = base.ReadCountedString();
					headers[key] = value;
				}
				else if (num == 4)
				{
					...
					string requestUri;
					if (IpcChannelHelper.ParseURL(text2, out requestUri) == null)
					{
						requestUri = text2;
					}
				...
				else if (num == 6)
				{
					...
					string contentType = base.ReadCountedString();
