...
namespace System.Runtime.Remoting.Channels.Ipc
{
	...
	internal class IpcPort : IDisposable
	{
		...
		internal static IpcPort Create(string portName, CommonSecurityDescriptor securityDescriptor, bool exclusive)
		{
			...
			PipeHandle pipeHandle = NativePipe.CreateNamedPipe(lpName, 1073741827U | (exclusive ? 524288U : 0U), 0U, 255U, 8192U, 8192U, uint.MaxValue, security_ATTRIBUTES);
			gchandle.Free();
			if (pipeHandle.Handle.ToInt32() == -1)
			{
				int lastWin32Error = Marshal.GetLastWin32Error();
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Ipc_CreateIpcFailed"), new object[]
				{
					IpcPort.GetMessage(lastWin32Error)
				}));
			}
		...
		internal static IpcPort Connect(string portName, bool secure, TokenImpersonationLevel impersonationLevel, int timeout)
		{
			...
			return new IpcPort(portName, pipeHandle);
			Block_4:
			throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Ipc_ConnectIpcFailed"), new object[]
			{
				IpcPort.GetMessage(lastWin32Error)
			}));
			Block_5:
			throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Ipc_Busy"), new object[]
			{
				IpcPort.GetMessage(lastWin32Error)
			}));
		}
		...
		internal static string GetMessage(int errorCode)
		{
			...
			return string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_UnknownError_Num"), new object[]
			{
		...
		internal void ImpersonateClient()
		{
			...
			{
				int lastWin32Error = Marshal.GetLastWin32Error();
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Ipc_ImpersonationFailed"), new object[]
				{
					IpcPort.GetMessage(lastWin32Error)
				}));
			}
		...
		internal unsafe int Read(byte[] data, int offset, int length)
		{
			...
			fixed (byte* ptr = data)
			{
				flag = NativePipe.ReadFile(this._handle, ptr + offset, length, ref result, IntPtr.Zero);
			}
			if (!flag)
			{
				int lastWin32Error = Marshal.GetLastWin32Error();
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Ipc_ReadFailure"), new object[]
				{
					IpcPort.GetMessage(lastWin32Error)
				}));
			}
		...
		internal unsafe int EndRead(IAsyncResult iar)
		{
			...
			if (pipeAsyncResult._errorCode != 0)
			{
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Ipc_ReadFailure"), new object[]
				{
					IpcPort.GetMessage(pipeAsyncResult._errorCode)
				}));
			}
		...
		internal unsafe void Write(byte[] data, int offset, int size)
		{
			...
			{
				int lastWin32Error = Marshal.GetLastWin32Error();
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Ipc_WriteFailure"), new object[]
				{
					IpcPort.GetMessage(lastWin32Error)
				}));
			}
