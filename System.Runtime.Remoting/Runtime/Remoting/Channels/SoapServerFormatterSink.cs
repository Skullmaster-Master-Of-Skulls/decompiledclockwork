	...
	public class SoapServerFormatterSink : IServerChannelSink, IChannelSinkBase
	{
		...
		public ServerProcessing ProcessMessage(IServerChannelSinkStack sinkStack, IMessage requestMsg, ITransportHeaders requestHeaders, Stream requestStream, out IMessage responseMsg, out ITransportHeaders responseHeaders, out Stream responseStream)
		{
			...
			if (flag)
			{
				...
				try
				{
					...
					if (baseTransportHeaders != null)
					{
						uri = baseTransportHeaders.RequestUri;
					}
					else
					{
					...
					if (this._protocol == SoapServerFormatterSink.Protocol.Http)
					{
						string text5 = (string)requestHeaders["User-Agent"];
						if (text5 != null)
						{
							if (text5.IndexOf("MS .NET Remoting") == -1)
							{
								flag2 = false;
							}
						}
					...
					string text6;
					Header[] channelHeaders = this.GetChannelHeaders(requestHeaders, out text6);
					...
					if (text6 != null && !SoapServices.IsSoapActionValidForMethodBase(text6, ((IMethodMessage)requestMsg).MethodBase))
					{
						throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Soap_InvalidSoapAction"), new object[]
						{
							text6
						}));
					}
					sinkStack.Push(this, null);
					result = this._nextSink.ProcessMessage(sinkStack, requestMsg, requestHeaders, null, out responseMsg, out responseHeaders, out responseStream);
		...
		private Header[] GetChannelHeaders(ITransportHeaders requestHeaders, out string soapActionToVerify)
		{
			...
			if (!SoapServices.GetTypeAndMethodNameFromSoapAction(text2, out text3, out text4))
			{
				...
				{
					throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_TypeNotFoundFromUri"), new object[]
					{
						text
					}));
				}
