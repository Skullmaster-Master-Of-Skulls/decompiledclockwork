	...
	internal static class CoreChannel
	{
		...
		internal static IMessage DeserializeMessage(string mimeType, Stream xstm, bool methodRequest, IMessage msg, Header[] h)
		{
			...
			else
			{
				...
				serializationStream = memoryStream2;
			}
			IRemotingFormatter remotingFormatter = CoreChannel.MimeTypeToFormatter(mimeType, false);
			object obj;
			if (flag2)
			{
				obj = ((BinaryFormatter)remotingFormatter).UnsafeDeserializeMethodResponse(serializationStream, null, (IMethodCallMessage)msg);
			}
			else if (methodRequest)
			{
				MethodCall methodCall = new MethodCall(h);
				remotingFormatter.Deserialize(serializationStream, new HeaderHandler(methodCall.HeaderHandler));
				obj = methodCall;
			}
			else
			{
				...
				MethodResponse methodResponse = new MethodResponse(h, mcm);
				remotingFormatter.Deserialize(serializationStream, new HeaderHandler(methodResponse.HeaderHandler));
				obj = methodResponse;
			}
			return (IMessage)obj;
		}
		...
		internal static void VerifyNoProviderData(string providerTypeName, ICollection providerData)
		{
			...
			{
				throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Providers_Config_NotExpectingProviderData"), new object[]
				{
					providerTypeName
				}));
			}
		...
		{
			throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Providers_Config_UnknownProperty"), new object[]
			{
				providerTypeName,
				propertyName
			}));
		...
		internal static string SidToString(IntPtr sidPointer)
		{
			...
			uint num = (uint)Marshal.ReadByte(sidSubAuthorityCount);
			...
			{
				uint num2 = (uint)((int)array[5] + ((int)array[4] << 8) + ((int)array[3] << 16) + ((int)array[2] << 24));
				stringBuilder.Append(string.Format(CultureInfo.CurrentCulture, "{0:x12}", new object[]
				{
					num2
				}));
			}
			...
			{
				IntPtr sidSubAuthority = NativeMethods.GetSidSubAuthority(sidPointer, num3);
				lastWin32Error = Marshal.GetLastWin32Error();
				if (lastWin32Error != 0)
				{
					throw new Win32Exception(lastWin32Error);
				}
				uint num4 = (uint)Marshal.ReadInt32(sidSubAuthority);
				stringBuilder.Append(string.Format(CultureInfo.CurrentCulture, "-{0:x12}", new object[]
				{
					num4
				}));
