...
namespace System.Runtime.Remoting.Channels
{
	...
	internal class ChunkedMemoryStream : Stream
	{
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			byte[] buffer2 = this._readChunk.Buffer;
			...
			while (count > 0)
			{
				if (this._readOffset == num)
				{
					...
					this._readOffset = 0;
					buffer2 = this._readChunk.Buffer;
					num = buffer2.Length;
					if (this._readChunk.Next == null)
					{
						num = this._writeOffset;
				...
				Buffer.BlockCopy(buffer2, this._readOffset, buffer, offset, num3);
				offset += num3;
				count -= num3;
		...
		public override int ReadByte()
		{
			...
			return (int)buffer[this._readOffset++];
		}
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			byte[] buffer2 = this._writeChunk.Buffer;
			int num = buffer2.Length;
			while (count > 0)
			{
				if (this._writeOffset == num)
				{
					...
					this._writeOffset = 0;
					buffer2 = this._writeChunk.Buffer;
					num = buffer2.Length;
				}
				int num2 = Math.Min(count, num - this._writeOffset);
				Buffer.BlockCopy(buffer, offset, buffer2, this._writeOffset, num2);
				offset += num2;
				count -= num2;
		...
		public virtual void WriteTo(Stream stream)
		{
			...
			for (;;)
			{
				...
				int count = num - this._readOffset;
