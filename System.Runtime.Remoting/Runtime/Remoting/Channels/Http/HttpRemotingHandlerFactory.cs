	...
	public class HttpRemotingHandlerFactory : IHttpHandlerFactory
	{
		...
		public IHttpHandler GetHandler(HttpContext context, string verb, string url, string filePath)
		{
			...
			if (flag2)
			{
				...
				Type type = (Type)HttpRemotingHandlerFactory.s_registeredDynamicTypeTable[text2];
				if (type != compiledType)
				{
					RegistrationHelper.RegisterType(machineAndAppName, compiledType, text2);
					HttpRemotingHandlerFactory.s_registeredDynamicTypeTable[text2] = compiledType;
				}
		...
		private static Type WebServicesFactoryType
		{
			get
			{
				if (HttpRemotingHandlerFactory.s_webServicesFactoryType == null)
				{
					...
					if (assembly == null)
					{
						throw new RemotingException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_AssemblyLoadFailed"), new object[]
						{
							"System.Web.Services"
						}));
					}
