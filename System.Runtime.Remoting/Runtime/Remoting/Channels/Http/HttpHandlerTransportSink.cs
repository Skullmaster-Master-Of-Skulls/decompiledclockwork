...
namespace System.Runtime.Remoting.Channels.Http
{
	...
	internal class HttpHandlerTransportSink : IServerChannelSink, IChannelSinkBase
	{
		...
		public void HandleRequest(HttpContext context)
		{
			...
			switch (this._nextSink.ProcessMessage(serverChannelSinkStack, null, baseTransportHeaders, inputStream, out message, out responseHeaders, out httpContentStream))
			{
			case ServerProcessing.Complete:
				this.SendResponse(response, 200, responseHeaders, httpContentStream);
				return;
			case ServerProcessing.OneWay:
				this.SendResponse(response, 202, responseHeaders, httpContentStream);
				break;
