...
namespace System.Runtime.Remoting.Channels.Http
{
	...
	internal sealed class HttpServerSocketHandler : HttpSocketHandler
	{
		...
		public BaseTransportHeaders ReadHeaders()
		{
			...
			string requestUri;
			if (HttpChannelHelper.ParseURL(text2, out requestUri) == null)
			{
				requestUri = text2;
			}
			baseTransportHeaders["__RequestVerb"] = text;
			baseTransportHeaders.RequestUri = requestUri;
		...
		public Stream GetResponseStream(string statusCode, string reasonPhrase, ITransportHeaders headers)
		{
			...
			if (obj2 != null)
			{
				...
				{
					length = Convert.ToInt32(obj2, CultureInfo.InvariantCulture);
				}
			}
			...
			{
				this._responseStream = new HttpFixedLengthResponseStream(this.NetStream, length);
			}
		...
		private bool ReadFirstLine(out string verb, out string requestURI, out string version)
		{
			...
			int count;
			HttpChannelHelper.DecodeUriInPlace(array, out count);
			requestURI = Encoding.UTF8.GetString(array, 0, count);
