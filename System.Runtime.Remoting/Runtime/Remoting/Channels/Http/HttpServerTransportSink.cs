...
namespace System.Runtime.Remoting.Channels.Http
{
	...
	internal class HttpServerTransportSink : IServerChannelSink, IChannelSinkBase
	{
		...
		internal void ServiceRequest(object state)
		{
			...
			Stream httpContentStream;
			ServerProcessing serverProcessing = this._nextSink.ProcessMessage(serverChannelSinkStack, null, transportHeaders, requestStream, out message, out headers, out httpContentStream);
			switch (serverProcessing)
			{
			case ServerProcessing.Complete:
				serverChannelSinkStack.Pop(this);
				httpServerSocketHandler.SendResponse(httpContentStream, "200", "OK", headers);
				break;
			case ServerProcessing.OneWay:
				httpServerSocketHandler.SendResponse(null, "202", "Accepted", headers);
				break;
