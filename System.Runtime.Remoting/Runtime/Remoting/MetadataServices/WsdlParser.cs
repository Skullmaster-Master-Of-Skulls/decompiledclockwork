...
namespace System.Runtime.Remoting.MetadataServices
{
	...
	internal class WsdlParser
	{
		...
		internal string SchemaNamespaceString
		{
			get
			{
				...
				case XsdVersion.V1999:
					result = WsdlParser.s_schemaNamespaceString1999;
					break;
				case XsdVersion.V2000:
					result = WsdlParser.s_schemaNamespaceString2000;
					break;
				case XsdVersion.V2001:
					result = WsdlParser.s_schemaNamespaceString;
					break;
				}
				return result;
			}
		...
		private WsdlParser.URTComplexType ParseComplexType(WsdlParser.URTNamespace parsingNamespace, string typeName)
		{
			...
			{
				string baseTypeNS = this.ParseQName(ref text, parsingNamespace);
				urtcomplexType.Extends(text, baseTypeNS);
			}
		...
		private WsdlParser.URTSimpleType ParseSimpleType(WsdlParser.URTNamespace parsingNamespace, string typeName)
		{
			...
			if (urtsimpleType == null)
			{
				...
				{
					string baseTypeNS = this.ParseQName(ref text2, parsingNamespace);
					urtsimpleType.Extends(text2, baseTypeNS);
				}
		...
		private void ParseEnumeration(WsdlParser.URTSimpleType parsingSimpleType, int enumFacetNum)
		{
			...
			{
				string valueString = this.LookupAttribute(WsdlParser.s_valueString, null, true);
				parsingSimpleType.IsEnum = true;
				parsingSimpleType.AddFacet(new WsdlParser.EnumFacet(valueString, enumFacetNum));
				return;
			}
		...
		{
			string name = this.LookupAttribute(WsdlParser.s_nameString, null, true);
			string left = this.LookupAttribute(WsdlParser.s_minOccursString, null, false);
			string text = this.LookupAttribute(WsdlParser.s_maxOccursString, null, false);
			bool bOptional = false;
			if (WsdlParser.MatchingStrings(left, WsdlParser.s_zeroString))
			{
				bOptional = true;
			}
			bool bArray = false;
			string arraySize = null;
			if (!WsdlParser.MatchingStrings(text, WsdlParser.s_emptyString) && !WsdlParser.MatchingStrings(text, WsdlParser.s_oneString))
			{
				if (WsdlParser.MatchingStrings(text, WsdlParser.s_unboundedString))
				{
			...
			if (this._XMLReader.IsEmptyElement)
			{
				typeName = this.LookupAttribute(WsdlParser.s_typeString, null, false);
				this.ResolveTypeAttribute(ref typeName, out xmlNS, out bEmbedded, out bPrimitive);
				this.ReadNextXmlElement();
			}
			else
			{
				xmlNS = parsingNamespace.Namespace;
				...
				while (this._XMLReader.Depth > depth)
				{
					...
					{
						WsdlParser.URTComplexType urtcomplexType = this.ParseComplexType(parsingNamespace, typeName);
						if (urtcomplexType.IsEmittableFieldType)
						{
							xmlNS = urtcomplexType.FieldNamespace;
					...
					{
						WsdlParser.URTSimpleType urtsimpleType = this.ParseSimpleType(parsingNamespace, typeName);
						if (urtsimpleType.IsEmittableFieldType)
						{
							xmlNS = urtsimpleType.FieldNamespace;
			...
			parsingComplexType.AddField(new WsdlParser.URTField(name, typeName, xmlNS, this, bPrimitive, bEmbedded, false, bOptional, bArray, arraySize, parsingNamespace));
		}
		...
		private void ParseAttributeField(WsdlParser.URTNamespace parsingNamespace, WsdlParser.URTComplexType parsingComplexType)
		{
			...
			if (this._XMLReader.IsEmptyElement)
			{
				text = this.LookupAttribute(WsdlParser.s_typeString, null, true);
				this.ResolveTypeAttribute(ref text, out text2, out bEmbedded, out bPrimitive);
				this.ReadNextXmlElement();
				if (WsdlParser.MatchingStrings(text, WsdlParser.s_idString) && this.MatchingSchemaStrings(text2))
				{
			...
			{
				text2 = parsingNamespace.Namespace;
				text = parsingNamespace.GetNextAnonymousName();
				bPrimitive = false;
				bEmbedded = true;
				int depth = this._XMLReader.Depth;
				this.ReadNextXmlElement();
				while (this._XMLReader.Depth > depth)
				{
					...
					{
						WsdlParser.URTSimpleType urtsimpleType = this.ParseSimpleType(parsingNamespace, text);
						if (urtsimpleType.IsEmittableFieldType)
						{
							text2 = urtsimpleType.FieldNamespace;
							text = urtsimpleType.FieldName;
							bPrimitive = urtsimpleType.PrimitiveField;
							parsingNamespace.RemoveSimpleType(urtsimpleType);
						}
					}
			...
			parsingComplexType.AddField(new WsdlParser.URTField(name, text, text2, this, bPrimitive, bEmbedded, true, bOptional, false, null, parsingNamespace));
		}
		...
		private void ParseRestrictionField(WsdlParser.URTNamespace parsingNamespace, WsdlParser.BaseType parsingType)
		{
			...
			while (this._XMLReader.Depth > depth)
			{
				...
				{
					string left = this.LookupAttribute(WsdlParser.s_refString, null, true);
					string left2 = this.ParseQName(ref left, parsingNamespace);
					if (WsdlParser.MatchingStrings(left2, WsdlParser.s_soapEncodingString) && WsdlParser.MatchingStrings(left, WsdlParser.s_arrayTypeString))
					{
						WsdlParser.URTComplexType urtcomplexType = (WsdlParser.URTComplexType)parsingType;
						string arrayType = this.LookupAttribute(WsdlParser.s_arrayTypeString, WsdlParser.s_wsdlNamespaceString, true);
						WsdlParser.URTNamespace urtnamespace = null;
						this.ParseQName(ref arrayType, null, out urtnamespace);
						urtcomplexType.AddArray(arrayType, urtnamespace);
						urtnamespace.AddComplexType(urtcomplexType);
				...
				{
					WsdlParser.URTSimpleType parsingSimpleType = (WsdlParser.URTSimpleType)parsingType;
					this.ParseEnumeration(parsingSimpleType, num);
					num++;
				}
				else
				{
		...
		{
			string elmName = this.LookupAttribute(WsdlParser.s_nameString, null, true);
			string name = parsingNamespace.Name;
			string typeName = this.LookupAttribute(WsdlParser.s_typeString, null, false);
			string name2;
			bool bPrimitive;
			if (this._XMLReader.IsEmptyElement)
			{
				bool flag;
				this.ResolveTypeAttribute(ref typeName, out name2, out flag, out bPrimitive);
				this.ReadNextXmlElement();
			}
			else
			{
			...
			parsingNamespace.AddElementDecl(new WsdlParser.ElementDecl(elmName, name, typeName, name2, bPrimitive));
		}
		...
		private void ParseReaderStreamLocation(WsdlParser.ReaderStream reader, WsdlParser.ReaderStream currentReaderStream)
		{
			...
			if (num == -1)
			{
				...
				{
					throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_Import"), new object[]
					{
						reader.Location
					}));
				}
		...
		private void ParseInput(WsdlParser.ReaderStream input)
		{
			...
			try
			{
				...
				else
				{
					...
					{
						throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_UnknownElementAtRootLevel"), new object[]
						{
							localName
						}));
					}
		...
		private void ParseWsdl()
		{
			...
			WsdlParser.URTNamespace inparsingNamespace = this.ParseNamespace();
			while (this._XMLReader.Depth > depth)
			{
				string localName = this._XMLReader.LocalName;
			...
			while (this._XMLReader.Depth > depth)
			{
				...
				if (WsdlParser.MatchingStrings(localName, WsdlParser.s_inputString))
				{
					...
					if (WsdlParser.MatchingStrings(wsdlPortTypeOperationContent.name, WsdlParser.s_emptyString))
					{
						...
						{
							throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_DuplicatePortTypesOperationName"), new object[]
							{
					...
					else
					{
						if (portType.sections.ContainsKey(wsdlPortTypeOperationContent.name))
						{
							throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_DuplicatePortSectionName"), new object[]
							{
								wsdlPortTypeOperationContent.name
							}));
						}
						portType.sections[wsdlPortTypeOperationContent.name] = portTypeOperation;
					}
				...
				else
				{
					if (this.MatchingNamespace(WsdlParser.s_wsdlSudsNamespaceString))
					{
						...
						if (WsdlParser.MatchingStrings(localName, WsdlParser.s_classString) || WsdlParser.MatchingStrings(localName, WsdlParser.s_structString))
						{
							...
							string use = this.LookupAttribute(WsdlParser.s_rootTypeString, null, false);
							wsdlBindingSuds.sudsUse = this.ProcessSudsUse(use, localName);
						...
						if (WsdlParser.MatchingStrings(localName, WsdlParser.s_interfaceString))
						{
							...
							string use2 = this.LookupAttribute(WsdlParser.s_rootTypeString, null, false);
							wsdlBindingSuds2.sudsUse = this.ProcessSudsUse(use2, localName);
		...
		{
			WsdlParser.SudsUse result = WsdlParser.SudsUse.Class;
					...
					if (wsdlBinding.suds != null && wsdlBinding.suds.Count > 0)
					{
						...
						using (IEnumerator enumerator2 = wsdlBinding.suds.GetEnumerator())
						{
							while (enumerator2.MoveNext())
							{
								...
								{
									this.ResolveWsdlClass(wsdlBinding, wsdlBindingSuds, bFirstSuds);
									bFirstSuds = false;
								}
								else
								{
			...
			if (suds != null)
			{
				...
				using (IEnumerator enumerator = suds.nestedTypes.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						...
						WsdlParser.WsdlBindingSudsNestedType nested = (WsdlParser.WsdlBindingSudsNestedType)obj;
						this.ResolveWsdlNestedType(binding, suds, nested);
					}
					goto IL_222;
				}
			}
			...
			if (bFirstSuds && (urtcomplexType.SudsUse == WsdlParser.SudsUse.MarshalByRef || urtcomplexType.SudsUse == WsdlParser.SudsUse.ServicedComponent || urtcomplexType.SudsUse == WsdlParser.SudsUse.Delegate || urtcomplexType.SudsUse == WsdlParser.SudsUse.Interface))
			{
				...
				foreach (object obj3 in arrayList)
				{
					...
					else
					{
						...
						{
							throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_WsdlInvalidMessage"), new object[]
							{
								wsdlMethodInfo.methodName
							}));
						}
		...
		private void ResolveWsdlNestedType(WsdlParser.WsdlBinding binding, WsdlParser.WsdlBindingSuds suds, WsdlParser.WsdlBindingSudsNestedType nested)
		{
			...
			if (suds.ns != ns)
			{
				throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_CantResolveNestedTypeNS"), new object[]
				{
					suds.typeName,
					suds.ns
				}));
			...
			if (urtcomplexType == null)
			{
				throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_CantResolveNestedType"), new object[]
				{
					suds.typeName,
					suds.ns
				}));
		...
		private ArrayList ResolveWsdlMethodInfo(WsdlParser.WsdlBinding binding)
		{
			...
			for (int i = 0; i < binding.operations.Count; i++)
			{
				...
				if (wsdlBindingOperation.soapOperation != null)
				{
					...
					{
						throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_WsdlInvalidPortType"), new object[]
						{
							binding.type
						}));
					}
					...
					foreach (object obj in wsdlBindingOperation.sections)
					{
						...
						if (WsdlParser.MatchingStrings(wsdlBindingOperationSection.elementName, WsdlParser.s_inputString))
						{
							...
							if (wsdlPortTypeOperation == null)
							{
								...
								if (num > 0)
								{
									string key = wsdlBindingOperationSection.name.Substring(0, num);
									wsdlPortTypeOperation = (WsdlParser.WsdlPortTypeOperation)wsdlPortType.sections[key];
								}
							}
					...
					if (wsdlPortTypeOperation != null)
					{
						foreach (object obj4 in wsdlPortTypeOperation.contents)
						{
							...
							if (WsdlParser.MatchingStrings(wsdlPortTypeOperationContent.element, WsdlParser.s_inputString))
							{
								...
								if (wsdlMessage == null)
								{
									throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_WsdlMissingMessage"), new object[]
									{
										wsdlPortTypeOperationContent.message
									}));
								}
							...
							else
							{
								if (!WsdlParser.MatchingStrings(wsdlPortTypeOperationContent.element, WsdlParser.s_outputString))
								{
									throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_WsdlInvalidPortType"), new object[]
									{
										wsdlPortTypeOperationContent.element
									}));
								}
		...
		{
			int depth = this._XMLReader.Depth;
			WsdlParser.URTNamespace parsingNamespace = this.ParseNamespace();
			while (this._XMLReader.Depth > depth)
			{
				string localName = this._XMLReader.LocalName;
				if (WsdlParser.MatchingStrings(localName, WsdlParser.s_complexTypeString))
		...
		{
			string result = WsdlParser.s_emptyString;
			bool flag;
			if (attrNS != null)
			{
			...
			if (flag)
			{
				result = this.Atomize(this._XMLReader.Value.Trim());
			}
		...
		private string ParseQName(ref string qname, WsdlParser.URTNamespace defaultNS, out WsdlParser.URTNamespace returnNS)
		{
			...
			else
			{
				...
				text = this._XMLReader.LookupNamespace(prefix);
			}
		...
		private static void InitKeywords()
		{
			...
			hashtable["abstract"] = value;
			hashtable["base"] = value;
			hashtable["bool"] = value;
			hashtable["break"] = value;
			hashtable["byte"] = value;
			hashtable["case"] = value;
			hashtable["catch"] = value;
			hashtable["char"] = value;
			hashtable["checked"] = value;
			hashtable["class"] = value;
			hashtable["const"] = value;
			hashtable["continue"] = value;
			hashtable["decimal"] = value;
			hashtable["default"] = value;
			hashtable["delegate"] = value;
			hashtable["do"] = value;
			hashtable["double"] = value;
			hashtable["else"] = value;
			hashtable["enum"] = value;
			hashtable["event"] = value;
			hashtable["exdouble"] = value;
			hashtable["exfloat"] = value;
			hashtable["explicit"] = value;
			hashtable["extern"] = value;
			hashtable["false"] = value;
			hashtable["finally"] = value;
			hashtable["fixed"] = value;
			hashtable["float"] = value;
			hashtable["for"] = value;
			hashtable["foreach"] = value;
			hashtable["goto"] = value;
			hashtable["if"] = value;
			hashtable["implicit"] = value;
			hashtable["in"] = value;
			hashtable["int"] = value;
			hashtable["interface"] = value;
			hashtable["internal"] = value;
			hashtable["is"] = value;
			hashtable["lock"] = value;
			hashtable["long"] = value;
			hashtable["namespace"] = value;
			hashtable["new"] = value;
			hashtable["null"] = value;
			hashtable["object"] = value;
			hashtable["operator"] = value;
			hashtable["out"] = value;
			hashtable["override"] = value;
			hashtable["private"] = value;
			hashtable["protected"] = value;
			hashtable["public"] = value;
			hashtable["readonly"] = value;
			hashtable["ref"] = value;
			hashtable["return"] = value;
			hashtable["sbyte"] = value;
			hashtable["sealed"] = value;
			hashtable["short"] = value;
			hashtable["sizeof"] = value;
			hashtable["static"] = value;
			hashtable["string"] = value;
			hashtable["struct"] = value;
			hashtable["switch"] = value;
			hashtable["this"] = value;
			hashtable["throw"] = value;
			hashtable["true"] = value;
			hashtable["try"] = value;
			hashtable["typeof"] = value;
			hashtable["uint"] = value;
			hashtable["ulong"] = value;
			hashtable["unchecked"] = value;
			hashtable["unsafe"] = value;
			hashtable["ushort"] = value;
			hashtable["using"] = value;
			hashtable["virtual"] = value;
			hashtable["void"] = value;
			hashtable["while"] = value;
		...
		internal static void CheckValidIdentifier(string ident)
		{
			...
			{
				throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_WsdlInvalidStringSyntax"), new object[]
				{
					ident
				}));
			}
		...
		internal class WriterStream
		{
			...
			internal static WsdlParser.WriterStream GetWriterStream(ref WsdlParser.WriterStream outputStreams, string outputDir, string fileName, ref string completeFileName)
			{
				...
				TextWriter writer = new StreamWriter(text2, false, new UTF8Encoding(false));
				writerStream = new WsdlParser.WriterStream(fileName, writer);
		...
		internal class URTParam
		{
			...
			private static string[] PTypeString = new string[]
			{
				"",
				"out ",
				"ref "
		...
		internal abstract class URTMethod
		{
			...
			private void FindMethodAttributes()
			{
				...
				for (int i = 0; i < arrayList.Count; i++)
				{
					...
					try
					{
						MethodInfo method = type.GetMethod(this.Name, bindingAttr);
						if (method != null)
						{
							array = new MethodInfo[]
							{
								method
							};
						}
			...
			internal virtual void PrintCSC(TextWriter textWriter, string indentation, string namePrefix, string curNS, WsdlParser.MethodPrintEnum methodPrintEnum, bool bURTType, string bodyPrefix, StringBuilder sb)
			{
				...
				if (WsdlParser.URTMethod.FlagTest(methodPrintEnum, WsdlParser.MethodPrintEnum.PrintBody))
				{
					...
					if (bodyPrefix == null)
					{
						...
						sb.Append(value);
						sb.Append("return");
						string text = WsdlParser.URTMethod.ValueString(this.GetTypeString(curNS, true));
						if (text != null)
						{
							sb.Append('(');
							sb.Append(text);
			...
			private void PrintMethodName(StringBuilder sb, string bodyPrefix, string name)
			{
				...
				if (bodyPrefix == null)
				{
					...
					sb.Append(")");
					sb.Append(WsdlParser.IsValidCS(identifier2));
					return;
				}
				sb.Append("((");
				sb.Append(WsdlParser.IsValidCS(identifier));
			...
			protected void ResolveWsdlParams(WsdlParser parser, string targetNS, string targetName, bool bRequest, WsdlParser.WsdlMethodInfo wsdlMethodInfo)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					else if (bRequest && !this._wsdlMethodInfo.bProperty)
					{
						...
						string text5 = wsdlMethodInfo.inputNamesNs[i];
						text3 = wsdlMethodInfo.inputTypes[i];
						text4 = wsdlMethodInfo.inputTypesNs[i];
						pType = WsdlParser.URTParamType.IN;
					}
					else
					{
						...
						string text6 = wsdlMethodInfo.outputNamesNs[i];
					...
					WsdlParser.URTNamespace urtnamespace = parser.LookupNamespace(text8);
					if (urtnamespace == null)
					{
						throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_CantResolveSchemaNS"), new object[]
						{
							text8,
							text7
						}));
					}
					WsdlParser.URTComplexType urtcomplexType = urtnamespace.LookupComplexType(text7);
					if (urtcomplexType != null && urtcomplexType.IsArray())
					{
						if (urtcomplexType.GetArray() == null)
						{
						...
						string array = urtcomplexType.GetArray();
						WsdlParser.URTNamespace arrayNS = urtcomplexType.GetArrayNS();
						this.AddParam(new WsdlParser.URTParam(name, array, arrayNS.Name, arrayNS.EncodedNS, pType, true, parser, arrayNS));
					}
					else if (urtnamespace.UrtType == UrtType.Xsd)
					{
						string typeName = parser.MapSchemaTypesToCSharpTypes(text7);
						this.AddParam(new WsdlParser.URTParam(name, typeName, urtnamespace.Namespace, urtnamespace.EncodedNS, pType, true, parser, urtnamespace));
					}
					else
					{
						string typeName2;
						if (urtcomplexType != null)
						{
							typeName2 = urtcomplexType.Name;
						}
						else
						{
							WsdlParser.URTSimpleType urtsimpleType = urtnamespace.LookupSimpleType(text7);
							if (urtsimpleType != null)
							{
								typeName2 = urtsimpleType.Name;
							}
							else
							{
								typeName2 = text7;
						...
						this.AddParam(new WsdlParser.URTParam(name, typeName2, urtnamespace.Namespace, urtnamespace.EncodedNS, pType, true, parser, urtnamespace));
					}
				}
			}
		...
		internal class SystemInterface : WsdlParser.BaseInterface
		{
			...
			internal SystemInterface(string name, string urlNS, string ns, WsdlParser parser, string assemName) : base(name, urlNS, ns, ns, parser)
			{
				...
				if (assembly == null)
				{
					throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_AssemblyNotFound"), new object[]
					{
						assemName
					}));
				}
				this._type = assembly.GetType(name2, true);
			}
				...
				while (i < num)
				{
					...
					foreach (MethodInfo methodInfo in methods)
					{
						...
						if (!bProxy)
						{
							...
							sb.Append(value);
							sb.Append("return");
							string text = WsdlParser.URTMethod.ValueString(WsdlParser.SystemInterface.CSharpTypeString(methodInfo.ReturnType.FullName));
							if (text != null)
							{
								sb.Append('(');
								sb.Append(text);
		...
		internal class URTInterface : WsdlParser.BaseInterface
		{
			...
			internal void ResolveTypes(WsdlParser parser)
			{
				for (int i = 0; i < this._baseIFaceNames.Count; i += 2)
				{
					...
					string text3;
					string assemName;
					UrtType urtType = parser.IsURTExportedType(text2, out text3, out assemName);
					WsdlParser.BaseInterface baseInterface;
					if (urtType != UrtType.Interop && text3.StartsWith("System", StringComparison.Ordinal))
					{
						baseInterface = new WsdlParser.SystemInterface(text, text2, text3, this._parser, assemName);
					}
					else
					{
			...
			internal void PrintCSC(TextWriter textWriter, string indentation, string curNS, StringBuilder sb)
			{
				...
				for (int j = 0; j < this._methods.Count; j++)
				{
					...
					((WsdlParser.URTMethod)this._methods[j]).PrintCSC(textWriter, indentation2, namePrefix, curNS, WsdlParser.MethodPrintEnum.InterfaceMethods, isURTInterface, null, sb);
				}
				textWriter.Write(indentation);
				textWriter.WriteLine('}');
			}
			...
			internal override void PrintClassMethods(TextWriter textWriter, string indentation, string curNS, ArrayList printedIFaces, bool bProxy, StringBuilder sb)
			{
				...
				if (this._methods.Count > 0)
				{
					...
					for (int j = 0; j < this._methods.Count; j++)
					{
						((WsdlParser.URTMethod)this._methods[j]).PrintCSC(textWriter, indentation, namePrefix, curNS, methodPrintEnum, true, bodyPrefix, sb);
					}
				}
		...
		internal class URTSimpleType : WsdlParser.BaseType
		{
			...
			private string MapToEnumType(string type)
			{
				...
				else
				{
					if (!(type == "UInt64"))
					{
						throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_InvalidEnumType"), new object[]
						{
							type
						}));
					}
		...
		internal class URTComplexType : WsdlParser.BaseType
		{
			...
			internal URTComplexType(string name, string urlNS, string ns, string encodedNS, SchemaBlockType blockDefault, bool bSUDSType, bool bAnonymous, WsdlParser parser, WsdlParser.URTNamespace xns) : base(name, urlNS, ns, encodedNS)
			{
				...
				if (num > 0)
				{
					...
					{
						WsdlParser.URTComplexType type = new WsdlParser.URTComplexType(text, urlNS, ns, encodedNS, blockDefault, bSUDSType, bAnonymous, parser, xns);
			...
			internal void Implements(string iFaceName, string iFaceNS, WsdlParser parser)
			{
				...
				{
					WsdlParser.URTInterface iface = new WsdlParser.URTInterface(iFaceName, urtnamespace.Name, urtnamespace.Namespace, urtnamespace.EncodedNS, this._parser);
					urtnamespace.AddInterface(iface);
				}
			}
			...
			internal void ResolveTypes(WsdlParser parser)
			{
				...
				{
					UrtType urtType = parser.IsURTExportedType(this._baseTypeXmlNS, out text, out assemName);
					if (urtType == UrtType.UrtSystem || text.StartsWith("System", StringComparison.Ordinal))
					{
						this._baseType = new WsdlParser.SystemType(this._baseTypeName, this._baseTypeXmlNS, text, assemName);
					}
					else
					{
				...
				{
					string text2 = (string)this._implIFaceNames[i];
					string text3 = (string)this._implIFaceNames[i + 1];
					string ns;
					string assemName2;
					UrtType urtType2 = parser.IsURTExportedType(text3, out ns, out assemName2);
					WsdlParser.BaseInterface baseInterface;
					if (urtType2 == UrtType.UrtSystem)
					{
						baseInterface = new WsdlParser.SystemInterface(text2, text3, ns, parser, assemName2);
					}
					else
					{
						WsdlParser.URTNamespace urtnamespace2 = parser.LookupNamespace(text3);
						...
						baseInterface = urtnamespace2.LookupInterface(text2);
						if (baseInterface == null)
						{
							throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_CantResolveTypeInNS"), new object[]
							{
			...
			internal void PrintCSC(TextWriter textWriter, string indentation, string curNS, StringBuilder sb)
			{
				...
				if (this._methods.Count > 0)
				{
					...
					for (int j = 0; j < this._methods.Count; j++)
					{
						((WsdlParser.URTMethod)this._methods[j]).PrintCSC(textWriter, text3, " ", curNS, WsdlParser.MethodPrintEnum.PrintBody, isURTType, bodyPrefix, sb);
					}
					textWriter.WriteLine();
				}
			...
			internal void ResolveArray()
			{
				...
				string str = null;
				string text = this._arrayType;
				int num = this._arrayType.IndexOf("[");
				if (num < 0)
				{
					throw new SUDSParserException(string.Format(CultureInfo.CurrentCulture, CoreChannel.GetResourceString("Remoting_Suds_WsdlInvalidArraySyntax"), new object[]
					{
						this._arrayType
					}));
				}
				text = this._arrayType.Substring(0, num);
				...
				case UrtType.UrtUser:
					...
					break;
				case UrtType.Xsd:
					str = this._parser.MapSchemaTypesToCSharpTypes(text);
					break;
				}
				this._clrarray = str + this.FilterDimensions(this._arrayType.Substring(num));
			}
		...
		internal class URTNamespace
		{
			...
			internal static string EncodeInterop(string name, WsdlParser parser)
			{
				...
				if (parser.ProxyNamespace != null && parser.ProxyNamespace.Length > 0)
				{
					...
					{
						str = parser.ProxyNamespaceCount.ToString(CultureInfo.InvariantCulture);
					}
					parser.ProxyNamespaceCount++;
					text = parser.ProxyNamespace + str;
				}
				else
				{
					int num = name.IndexOf(":");
			...
			{
				WsdlParser.URTComplexType result = null;
				for (int i = 0; i < this._URTComplexTypes.Count; i++)
				{
					WsdlParser.URTComplexType urtcomplexType = (WsdlParser.URTComplexType)this._URTComplexTypes[i];
					if (urtcomplexType != null && WsdlParser.MatchingStrings(urtcomplexType.SearchName, typeName))
					{
			...
			internal void PrintCSC(WsdlParser.WriterStream writerStream)
			{
				...
				if (this._numURTComplexTypes > 0)
				{
					for (int k = 0; k < this._URTComplexTypes.Count; k++)
					{
						...
						{
							urtcomplexType2.PrintCSC(outputStream, indentation, this._encodedNS, sb);
						}
					}
				}
				if (this._numURTSimpleTypes > 0)
				{
					for (int l = 0; l < this._URTSimpleTypes.Count; l++)
					{
						...
						{
							urtsimpleType2.PrintCSC(outputStream, indentation, this._encodedNS, sb);
						}
					}
				}
				for (int m = 0; m < this._URTInterfaces.Count; m++)
				{
					((WsdlParser.URTInterface)this._URTInterfaces[m]).PrintCSC(outputStream, indentation, this._encodedNS, sb);
				}
				if (this.Namespace != null && this.Namespace.Length != 0)
				{
					outputStream.WriteLine('}');
