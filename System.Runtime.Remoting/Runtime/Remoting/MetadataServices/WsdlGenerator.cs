...
namespace System.Runtime.Remoting.MetadataServices
{
	...
	internal class WsdlGenerator
	{
		...
		internal void ProcessType(Type type)
		{
			string name;
			Assembly assem;
			bool nsandAssembly = WsdlGenerator.GetNSAndAssembly(type, out name, out assem);
			WsdlGenerator.XMLNamespace xmlnamespace = this.LookupNamespace(name, assem);
			if (xmlnamespace != null)
			{
				string name2 = WsdlGenerator.RefName(type);
				if (xmlnamespace.LookupSchemaType(name2) != null)
				{
					return;
				}
			}
			...
			{
				xmlnamespace = this.AddNamespace(name, assem, nsandAssembly);
			}
			...
			if (!type.IsArray)
			{
				...
				WsdlGenerator.RealSchemaType rsType = new WsdlGenerator.RealSchemaType(type, xmlnamespace, serviceEndpoint, typeToServiceEndpoint, bUnique, this);
		...
		private void ProcessMethods(WsdlGenerator.RealSchemaType rsType)
		{
			...
			{
				WsdlGenerator.XMLNamespace xns2;
				if (xns.IsInteropType)
				{
					string name = xns.Name;
					xns2 = xns;
				}
				else
				{
					...
					xns2 = this.AddNamespace(name, xns.Assem);
					xns.DependsOnSchemaNS(xns2, false);
				}
				foreach (MethodInfo methodInfo in introducedMethods)
				{
					this.AddType(methodInfo.ReturnType, xns2);
					ParameterInfo[] parameters = methodInfo.GetParameters();
					for (int j = 0; j < parameters.Length; j++)
					{
						this.AddType(parameters[j].ParameterType, xns2);
					}
				}
			}
		}
		...
		private void AddType(Type type, WsdlGenerator.XMLNamespace xns)
		{
			...
			if (!type.IsPublic && !type.IsNotPublic)
			{
				...
				if (num > 0)
				{
					...
					Type type4 = assembly.GetType(name, true);
		...
		private void EnqueueType(Type type, WsdlGenerator.XMLNamespace xns)
		{
			if (!type.IsPrimitive || type == WsdlGenerator.s_charType)
			{
				...
				if (type.IsInterface || text != null || type == WsdlGenerator.s_voidType)
				{
		...
		private void PrintWsdl()
		{
			...
			string indentStr = "";
			string text = WsdlGenerator.IndentP(indentStr);
			...
			this.PrintWsdlNamespaces(this._textWriter, stringBuilder, text3);
			bool flag = false;
			for (int i = 0; i < this._namespaces.Count; i++)
			{
			...
			if (flag)
			{
				this.PrintTypesBeginWsdl(this._textWriter, stringBuilder, text);
				for (int j = 0; j < this._namespaces.Count; j++)
				{
					if (((WsdlGenerator.XMLNamespace)this._namespaces[j]).CheckForSchemaContent())
					{
						((WsdlGenerator.XMLNamespace)this._namespaces[j]).PrintSchemaWsdl(this._textWriter, stringBuilder, text2);
					}
				}
				this.PrintTypesEndWsdl(this._textWriter, stringBuilder, text);
			}
			ArrayList refNames = new ArrayList(25);
			for (int k = 0; k < this._namespaces.Count; k++)
			{
				((WsdlGenerator.XMLNamespace)this._namespaces[k]).PrintMessageWsdl(this._textWriter, stringBuilder, text, refNames);
			}
			this.PrintServiceWsdl(this._textWriter, stringBuilder, text, refNames);
		...
		private void PrintWsdlNamespaces(TextWriter textWriter, StringBuilder sb, string indent)
		{
			...
			for (int i = 0; i < this._namespaces.Count; i++)
			{
				((WsdlGenerator.XMLNamespace)this._namespaces[i]).PrintDependsOnWsdl(this._textWriter, sb, indent, usedNames);
			}
			sb.Length = 0;
			sb.Append(indent);
		...
		private class Restriction : WsdlGenerator.Particle
		{
			...
			public override void Print(TextWriter textWriter, StringBuilder sb, string indent)
			{
				...
				foreach (object obj in this._abstractElms)
				{
					...
					abstractElement.Print(textWriter, sb, WsdlGenerator.IndentP(indentStr));
				}
		...
		private class SimpleSchemaType : WsdlGenerator.SchemaType
		{
			...
			internal static WsdlGenerator.SimpleSchemaType GetSimpleSchemaType(Type type, WsdlGenerator.XMLNamespace xns, bool fInline)
			{
				...
				if (type.IsEnum)
				{
					...
					simpleSchemaType._restriction = new WsdlGenerator.Restriction(baseName, xns);
		...
		private abstract class ComplexSchemaType : WsdlGenerator.SchemaType
		{
			...
			protected void PrintBody(TextWriter textWriter, StringBuilder sb, string indent)
			{
				...
				string text = WsdlGenerator.IndentP(indent);
				string indentStr = WsdlGenerator.IndentP(text);
				if (count > 0)
				{
					...
					{
						((WsdlGenerator.Particle)this._particles[i]).Print(textWriter, sb, WsdlGenerator.IndentP(indentStr));
					}
					if (flag)
					{
						sb.Length = 0;
			...
			private static string[] schemaBlockBegin = new string[]
			{
				"<all>",
				"<sequence>",
				"<choice>",
			...
			private static string[] schemaBlockEnd = new string[]
			{
				"</all>",
				"</sequence>",
				"</choice>",
		...
		private class ArraySchemaType : WsdlGenerator.ComplexSchemaType
		{
			...
			internal override void PrintSchemaType(TextWriter textWriter, StringBuilder sb, string indent, bool bAnonymous)
			{
				string indent2 = WsdlGenerator.IndentP(indent);
				sb.Length = 0;
				sb.Append(indent);
		...
		private class RealSchemaType : WsdlGenerator.ComplexSchemaType
		{
			...
			internal RealSchemaType(Type type, WsdlGenerator.XMLNamespace xns, string serviceEndpoint, Hashtable typeToServiceEndpoint, bool bUnique, WsdlGenerator WsdlGenerator) : base(type)
			{
				this._type = type;
			...
			internal void Resolve(StringBuilder sb)
			{
				...
				if (this.IsNotSystemDefinedRoot(this._type, baseType))
				{
					...
					while (baseType2 != null && this.IsNotSystemDefinedRoot(type, baseType2))
					{
						if (this._typeToServiceEndpoint != null && !this._typeToServiceEndpoint.ContainsKey(baseType2.Name) && this._typeToServiceEndpoint.ContainsKey(type.Name))
						{
							this._typeToServiceEndpoint[baseType2.Name] = this._typeToServiceEndpoint[type.Name];
						}
						type = baseType2;
				...
				if (isSUDSType)
				{
					...
					if (this._iFaces.Length > 0)
					{
						...
						for (int i = 0; i < this._iFaces.Length; i++)
						{
							string ns;
							Assembly assem;
							WsdlGenerator.GetNSAndAssembly(this._iFaces[i], out ns, out assem);
							WsdlGenerator.XMLNamespace xmlnamespace = this._xns.LookupSchemaNamespace(ns, assem);
							sb.Length = 0;
					...
					if (this._methods.Length > 0)
					{
						...
						WsdlGenerator.XMLNamespace xmlnamespace2 = this._xns.LookupSchemaNamespace(ns2, this._xns.Assem);
						...
						for (int j = 0; j < this._methods.Length; j++)
						{
							...
							foreach (ParameterInfo parameterInfo in parameters)
							{
								...
								string value = WsdlGenerator.RealSchemaType.TypeName(parameterType, true, xmlnamespace2);
								if (flag)
								{
									phonySchemaType._inParamNames.Add(text);
									phonySchemaType._inParamTypes.Add(value);
								}
								if (flag2)
								{
									phonySchemaType._outParamNames.Add(text);
							...
							if (!RemotingServices.IsOneWay(methodInfo))
							{
								...
								{
									returnName = soapMethodAttribute.ReturnXmlElementName;
								}
								else
								{
									returnName = "return";
								...
								{
									name2 = soapMethodAttribute.ResponseXmlElementName;
								}
								else
								{
									name2 = name + "Response";
								}
								WsdlGenerator.PhonySchemaType phonySchemaType2 = new WsdlGenerator.PhonySchemaType(name2);
								phonySchemaType._returnName = returnName;
				...
				if (this._fields != null)
				{
					for (int l = 0; l < this._fields.Length; l++)
					{
						...
						base.AddParticle(new WsdlGenerator.SchemaElement(fieldInfo.Name, type2, false, this._xns));
					}
				}
			}
			...
			internal void PrintMessageWsdl(TextWriter textWriter, StringBuilder sb, string indent, ArrayList refNames)
			{
				string text = WsdlGenerator.IndentP(indent);
				string text2 = WsdlGenerator.IndentP(text);
				string value = WsdlGenerator.IndentP(text2);
				string value2 = null;
				string text3 = null;
				bool flag = false;
				string ns;
				...
				WsdlGenerator.XMLNamespace xmlnamespace = this._xns.LookupSchemaNamespace(ns, this._xns.Assem);
				int num = 0;
				...
				for (int i = 0; i < num; i++)
				{
					...
					string value3 = WsdlGenerator.RealSchemaType.PrintMethodName(methodInfo);
					sb.Length = 0;
						...
						if (!realSchemaType.Type.IsInterface && !realSchemaType.IsSUDSType)
						{
							...
							if (introducedInterfaces.Length > 0)
							{
								...
								while (m < introducedInterfaces.Length)
								{
									string ns2;
									Assembly assem;
									WsdlGenerator.GetNSAndAssembly(introducedInterfaces[num2], out ns2, out assem);
									WsdlGenerator.XMLNamespace xmlnamespace2 = this._xns.LookupSchemaNamespace(ns2, assem);
									sb.Length = 0;
				...
				for (int n = 0; n < num; n++)
				{
					...
					string text4 = SoapServices.GetXmlNamespaceForMethodCall(methodInfo);
					if (text4 == null)
					{
						sb.Append(value2);
					}
					...
					if (!flag)
					{
						...
						sb.Append(value);
						sb.Append("<soap:body use='encoded' encodingStyle='http://schemas.xmlsoap.org/soap/encoding/' namespace='");
						text4 = SoapServices.GetXmlNamespaceForMethodResponse(methodInfo);
						if (text4 == null)
						{
							sb.Append(value2);
						}
			...
			private void PrintSuds(Type type, string[] implIFaces, Type[] nestedTypes, TextWriter textWriter, StringBuilder sb, string indent)
			{
				...
				if (type.IsInterface)
				{
					value2 = "<suds:extends type='";
				}
				else
				{
					value2 = "<suds:implements type='";
				}
				if (implIFaces != null)
				{
			...
			private static string ProcessArray(Type type, WsdlGenerator.XMLNamespace xns)
			{
				...
				string text = WsdlGenerator.RealSchemaType.TypeName(elementType, true, xns);
				...
				if (arrayRank > 1)
				{
					str2 = arrayRank.ToString(CultureInfo.InvariantCulture);
				}
				string text3 = str + text2.Substring(0, 1).ToUpper(CultureInfo.InvariantCulture) + text2.Substring(1) + str2;
				text3 = text3.Replace('+', 'N');
				if (xns.LookupArraySchemaType(text3) == null)
				{
					WsdlGenerator.ArraySchemaType arraySchemaType = new WsdlGenerator.ArraySchemaType(type, text3, SchemaBlockType.ComplexContent, false);
			...
			internal static string TypeName(Type type, bool bEmbedded, WsdlGenerator.XMLNamespace thisxns)
			{
				...
				string value = WsdlGenerator.RefName(type);
				Type type2 = type;
				if (type.IsByRef)
				{
					type2 = type.GetElementType();
					value = WsdlGenerator.RefName(type2);
					if (type2.IsArray)
					{
						return WsdlGenerator.RealSchemaType.ProcessArray(type2, thisxns);
					}
				}
				string text = SudsConverter.MapClrTypeToXsdType(type2);
				if (text == null)
				{
			...
			private static MethodInfo[] GetIntroducedMethods(Type type, ref string[] methodAttributes)
			{
				...
				{
					foreach (MethodInfo methodInfo in type.GetInterfaceMap(interfaceType).TargetMethods)
					{
		...
		private class XMLNamespace
		{
			...
			internal void PrintSchemaWsdl(TextWriter textWriter, StringBuilder sb, string indent)
			{
				...
				if (flag)
				{
					string text = WsdlGenerator.IndentP(indent);
					string text2 = WsdlGenerator.IndentP(text);
					string indentStr = WsdlGenerator.IndentP(text2);
					WsdlGenerator.IndentP(indentStr);
					sb.Length = 0;
					sb.Append(indent);
					sb.Append("<schema ");
