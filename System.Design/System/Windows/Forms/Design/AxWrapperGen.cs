...
namespace System.Windows.Forms.Design
{
	...
	public class AxWrapperGen
	{
		...
		public AxWrapperGen(Type axType)
		{
			...
			object[] customAttributes = axType.GetCustomAttributes(typeof(ComSourceInterfacesAttribute), false);
			if (customAttributes.Length == 0 && axType.BaseType.GUID.Equals(axType.GUID))
			{
				customAttributes = axType.BaseType.GetCustomAttributes(typeof(ComSourceInterfacesAttribute), false);
			}
			if (customAttributes.Length > 0)
			{
				...
				int length = value.IndexOfAny(anyOf);
				string text = comSourceInterfacesAttribute.Value.Substring(0, length);
			...
			{
				customAttributes = type.GetCustomAttributes(typeof(CoClassAttribute), false);
				if (customAttributes.Length > 0)
				{
			...
			try
			{
				customAttributes = this.axctlType.GetCustomAttributes(typeof(InterfaceTypeAttribute), false);
				if (customAttributes.Length > 0)
				{
					InterfaceTypeAttribute interfaceTypeAttribute = (InterfaceTypeAttribute)customAttributes[0];
		...
		internal static string GenerateWrappers(AxImporter importer, Guid axClsid, Assembly rcwAssem, string[] refAssemblies, DateTime tlbTimeStamp, out string assem)
		{
			...
			if (flag)
			{
				...
				if (assem != null)
				{
					if (text == null)
					{
						throw new Exception(SR.GetString("AXNotValidControl", new object[]
						{
							"{" + axClsid + "}"
						}));
					}
