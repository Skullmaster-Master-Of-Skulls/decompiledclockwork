...
},clearHighlightedElement:function(g,f,d){var b=true;
if(g!=this._selectedEditorElement){b=false;
}var i=f?f:g.innerHTML.replace(/<\/?[^>]*>/ig,"");
var a=f?true:false;
this._editor.selectElement(g,false);
var c=new Telerik.Web.UI.Editor.GenericCommand(this._localizedName,this._editor.get_contentWindow(),this._editor);
var j=this._editor.get_document().createTextNode(i);
g.parentNode.replaceChild(j,g);
var h=this._consolidateText(j);
var e=this._editor.isIE;
if(e&&!i&&!f&&!d){return;
}h.collapse();
h.select();
if(a){this._editor.executeCommand(c);
}if(b){this._selectedEditorElement=null;
}},_consolidateText:function(g){var h=Telerik.Web.UI.Editor.Utils;
var e=this._editor.getDomRange();
var f=g.length;
var c=g.previousSibling;
var b=g.nextSibling;
var a=function(i,j){i.nodeValue+=j.nodeValue;
j.parentNode.removeChild(j);
};
e.selectNodeContents(g);
if(b&&h.isTextNode(b)){a(g,b);
}if(c&&h.isTextNode(c)){var d=c.length;
a(c,g);
e.setStart(c,d);
e.setEnd(c,d+f);
}return e;
...
if(d){if(d.scrollIntoViewIfNeeded){d.scrollIntoViewIfNeeded();
...
}else{if(Telerik.Web.Browser.edge){b.contentDocument.body.scrollTop+=d.getClientRects()[0].top+d.offsetHeight;
e=d.getClientRects()[0].top+d.offsetHeight;
}else{if($telerik.isSafari){e=d.getBoundingClientRect().y+a.height;
