...
},showHeaderMenu:function(){},resizeToFit:function(h,f){if(!this._canResizeToFit()){return;
}if(typeof f!="undefined"){this.get_realOwner()._includeAllHeaderRows=f;
}var i=this._getItemContainerTableView();
var d=Array.indexOf(i.get_columns(),this);
var e=this._createFitTestContainer(i.get_element(),h);
var g=this._calculateBestFitSizes(e);
this._removeFitTestContainer(i.get_element());
this.get_owner().resizeColumn(d,g||this.get_element().offsetWidth);
...
},_calculateBestFitSizes:function(f,e){var g=f.tBodies[0].getElementsByTagName("tr")[0];
if(g){var d=g.cells[e];
if(d){return d.offsetWidth;
}}return null;
...
j<k;
j++){var m=f.rows[j];
if(!((m.id&&m.id.indexOf(q.id)>-1)||(m.className&&m.className.indexOf("rgFooter")>-1))){n.push(m);
}}var o=n.pop();
...
},_insertHeaderCells:function(e,g){var l=e.createTHead(),f,j,i,d,h;
if(this._owner._hasMultiHeaders){this.get_realOwner()._includeAllHeaderRows=true;
}if(this.get_realOwner()._includeAllHeaderRows){j=$telerik.$(">tr:not(.rgCommandRow)",this.get_element().parentNode.parentNode);
for(h=0;
h<j.length;
h++){i=j[h];
f=l.insertRow(-1);
...
}}else{f=l.insertRow(-1);
i=this.get_element().parentNode;
d=i.cells;
this._insertCellsInRows(d,f);
}if(this.get_realOwner().get_element().tFoot){j=this.get_realOwner().get_element().tFoot.rows;
for(h=0;
h<j.length;
h++){i=j[h];
if(i.className.indexOf("rgFooter")>-1){f=l.insertRow(-1);
f.className=i.className;
this._insertCellsInRows(i.cells,f);
...
}}d.offEvent(b.EventNamesMap.up).onEvent(b.EventNamesMap.up,a.proxy(this._commandItemClick,this));
...
}},_getViewByType:function(g){var f=this.get_id().replace(/_Header|_Footer$/,"");
var e;
var h;
var d;
...
var d={};
this._owner.raise_rowCreating(new Telerik.Web.UI.GridDataItemCancelEventArgs(q,null));
var h=true;
...
if(!m.id&&m.parentNode.tagName.toLowerCase()=="tbody"&&m.className.indexOf("rgFooter")===-1){continue;
...
},resizeColumn:function(k,o){var i=this._hasMultiHeaders;
var g;
var j=[];
var f;
if(!this._validateResizeColumnParams(k,o)){return;
}if(typeof(k)=="string"){k=parseInt(k,10);
}var d=new Telerik.Web.UI.GridColumnCancelEventArgs(this.get_columns()[k],null);
...
if(this==this._owner.get_masterTableView()&&this._owner.get_masterTableViewHeader()){this._owner.get_masterTableViewHeader().resizeColumn(k,o);
}if(this.HeaderRow&&this.HeaderRow.style.display==="none"){var e=this._owner.get_masterTableView().get_element().tBodies;
if(e&&e.length>0){var n=e[0].rows;
if(n&&n.length>0){this.HeaderRow=n[0];
}}}if(this.HeaderRow||this.MultiHeaderCells){f=k;
j=i?this.MultiHeaderCells:this.HeaderRow.cells;
}if(this.ColGroup){if(this.ColGroup.Cols[k]){if(o>0){this.ColGroup.Cols[k].style.width=o+"px";
}}}if(this._owner.get_masterTableViewHeader()&&(this.get_id()==this._owner.get_masterTableViewHeader().get_id())){if(this._owner.get_masterTableView().ColGroup){if(this._owner.get_masterTableView().ColGroup.Cols[k]){if(o>0){this._owner.get_masterTableView().ColGroup.Cols[k].style.width=o+"px";
}}}if(this._owner.get_masterTableViewFooter()&&this._owner.get_masterTableViewFooter().ColGroup){if(this._owner.get_masterTableViewFooter().ColGroup.Cols[k]){if(o>0){this._owner.get_masterTableViewFooter().ColGroup.Cols[k].style.width=o+"px";
}}}}if(o.toString().indexOf("px")!=-1){o=o.replace("px","");
}if(o.toString().indexOf("%")==-1){o=o+"px";
}this._resizedColumnsData.add(String.format("{0},{1}",this._data.UniqueID,this.get_columns()[k].get_uniqueName()),o);
if(this._owner.get_masterTableViewHeader()){this._owner.ClientSettings.Resizing.ResizeGridOnColumnResize=true;
}g=j[f].offsetWidth-o;
if(this._owner.ClientSettings.Resizing.ResizeGridOnColumnResize){if(g!=null){this._resizeGridOnColumnResize(k,g,m);
}}else{this._noResizeGridOnColumnResize(m,k,l);
}this._owner._resizedColumns=this._resizedColumnsData.toString();
this._owner.updateClientState();
d=new Telerik.Web.UI.GridColumnEventArgs(this.get_columns()[k],null);
...
}if($get(this._owner.ClientID+"_Frozen")&&g!=null){var h=$get(this._owner.ClientID+"_FrozenScroll");
h.style.width=h.offsetWidth-g+"px";
}},_resizeGridOnColumnResize:function(j,f,k){var d;
if(this._owner.get_masterTableViewHeader()&&(this.get_id()==this._owner.get_masterTableViewHeader().get_id())){this.get_element().style.width=(k-f)+"px";
this._owner.get_masterTableView().get_element().style.width=this.get_element().style.width;
for(var h=0;
h<this.ColGroup.Cols.length;
h++){if(h!=j&&this.ColGroup.Cols[h].style.width==""){var e=h;
var g=this._hasMultiHeaders?this.MultiHeaderCells:this.HeaderRow.cells;
if(g[e].style.display!="none"){var m=g[e].offsetWidth;
this.ColGroup.Cols[h].style.width=m+"px";
...
}d=this.get_element().offsetWidth+"px";
}else{var l=k||this.get_element().offsetWidth;
if(l>f){this.get_element().style.width=d=(l-f)+"px";
}else{this.get_element().style.width=d=l+"px";
}this._owner.get_element().style.width=d;
}this._resizedControlData.add(this._data.UniqueID,d);
...
}if(h._scrolling&&h._scrolling.UseStaticHeaders){h.repaint();
...
}else{p.parentElement.className="rgActionButton rgExpand";
p.className="rgIcon rgExpandIcon";
}this._owner.get_hierarchyColsExpandedState()[this._data.UniqueID]=false;
}else{if(p.src){p.src=f._data.CollapseImageUrl;
}else{p.parentElement.className="rgActionButton rgCollapse";
p.className="rgIcon rgCollapseIcon";
...
d[h].children[0].className="rgIcon rgCollapseIcon";
...
d[h].children[0].className="rgIcon rgExpandIcon";
...
}}}}}if(n.className.indexOf("rgCollapse")>-1){n.className=n.className.replace(/rgCollapse/g,"rgExpand");
}else{n.className=n.className.replace(/rgExpand/g,"rgCollapse");
}this._owner.updateClientState();
...
},_toggleGroupsExpandAll:function(H,r,y,D){var J=false;
if(!D){J=true;
}var s=H;
var z=typeof(D)==="undefined"?false:true;
if(typeof(D)==="undefined"){D=s;
}if(!this._owner.ClientSettings.AllowGroupExpandCollapse){return;
}var N=s.id.split("__")[0];
var M=$find(N);
var I=s.id.split("__")[1];
var v=s.id.split("__")[2];
var w=false;
if(!this._originalGroupLevel){this._originalGroupLevel=v;
}if(parseInt(v,10)<parseInt(this._originalGroupLevel,10)||J){this._originalGroupLevel=v;
}var l=s.parentNode.cellIndex;
var G=s.parentNode.parentNode.sectionRowIndex;
var L=M.get_element().tBodies[0];
var o=this.get_columns()[l];
var d=new Sys.CancelEventArgs();
if(!z){if(y){this._owner.raise_groupCollapsing(d);
}else{this._owner.raise_groupExpanding(d);
}}if(d.get_cancel()){return false;
}if(s===D){if(o){if(s.src){if(!y&&s.src!==o._data.CollapseImageUrl){w=true;
s.src=o._data.CollapseImageUrl;
s.title=M._owner._groupingSettings.CollapseTooltip;
}else{if(y&&s.src!==o._data.ExpandImageUrl){w=true;
s.src=o._data.ExpandImageUrl;
s.title=M._owner._groupingSettings.ExpandTooltip;
}}}else{var m=null;
if(!y&&s.className.indexOf("rgCollapse")===-1){w=true;
...
var K=parseInt(this._groupLevelsCount,10)-parseInt(v,10);
var x;
var F;
var A;
var f;
var O=this.get_virtualItemCount();
for(x=G+1;
x<L.rows.length;
x++){F=L.rows[x];
if(O&&!y&&this._shouldSkipOtherRows(F,O)){continue;
}var E=this._previousRow(L.rows[x]);
if(F.className.indexOf("rgRow")<0&&F.className.indexOf("rgAltRow")<0&&E&&E.id){var u=this._getExpandButton(E);
if(u&&!this._isGroupButtonCollapse(u)&&F.className.indexOf("rgFooter")==-1){continue;
}}var k;
if(F.id||F.className.indexOf("rgGroupHeader")!==-1){k=this._getGroupExpandButton(F);
}if(!k){if(C==v){if(p>0){F.style.display=q;
}if(F.className.indexOf("rgFooter")!=-1){p--;
}else{F.style.display=q;
}}if(q==""||q=="table-row"){if(F.className.indexOf("rgFooter")!=-1){var h=1;
if(this._owner._groupingSettings.RetainGroupFootersVisibility){h=2;
}if(K<=h){F.style.display=q;
}K--;
}}}else{C=k.id.split("__")[2];
if(C==v||(parseInt(C,10)<parseInt(v,10))){if(r.groupLevel&&v==r.groupLevel&&q=="none"){f=v-C+1;
for(A=0;
A<f;
A++){F=L.rows[x-A-1];
if(!F.id){F.style.display="";
}}}break;
}else{if(parseInt(C,10)-parseInt(v,10)==1){if(k.src==s.src||(s.className==k.className)){if(k===D){if(q=="none"){if(s.src){k.src=o._data.CollapseImageUrl;
...
K=parseInt(this._groupLevelsCount,10)-parseInt(v,10);
}F.style.display=q;
}}}if(x==L.rows.length-1){g=true;
}}if(r.groupLevel&&g&&r.groupLevel==v&&q=="none"){f=v*1+1;
for(A=0;
A<f;
A++){F=L.rows[L.rows.length-A-1];
if(!F.id){F.style.display="";
}}}if(s===D&&w){var t=this._owner._expandedGroupItems;
var B=M._data.UniqueID+"!"+I;
p=0;
for(x=0;
x<t.length;
x++){if(t[x]==B){p++;
}}if(p===2){Array.remove(t,B);
}else{Array.add(t,B);
}}this._owner.updateClientState();
d=new Sys.EventArgs();
if(!z){if(y){this._owner.raise_groupCollapsed(d);
...
this._styleSortedColumnElements(this._sortExpressions._array[j].get_fieldName(),Telerik.Web.UI.GridSortOrder.None);
}this._sortExpressions.clear();
}if(m!=e){this._showSortIconForField(g,m);
this._styleSortedColumnElements(g,m);
}if(m!=Telerik.Web.UI.GridSortOrder.None){l.set_fieldName(g);
l.set_sortOrder(m);
this._sortExpressions.add(l);
}return l;
},_styleSortedColumnElements:function(g,l){var f=this.get_columns();
var h=this.get_dataItems();
var m=(l!=Telerik.Web.UI.GridSortOrder.None)?Sys.UI.DomElement.addCssClass:Sys.UI.DomElement.removeCssClass;
var k=this.get_owner().SortingSettings.EnableSkinSortStyles;
var d;
for(var e=0;
e<f.length;
e++){if(f[e]._data.SortExpression==g){if(k){m(f[e].get_element(),"rgSorted");
}for(var j=0;
j<h.length;
j++){d=h[j].get_cell(f[e].get_uniqueName());
if(k){m(d,"rgSorted");
}if(this.get_owner().SortingSettings.SortedBackColor){if(l!=Telerik.Web.UI.GridSortOrder.None){d.style.backgroundColor=this.get_owner().SortingSettings.SortedBackColor.Name;
...
}}if(((typeof(i)=="number"&&Telerik.Web.UI.GridFilterFunction.NoFilter==i)||(typeof(i)=="string"&&Telerik.Web.UI.GridFilterFunction.tryParse(i)==Telerik.Web.UI.GridFilterFunction.NoFilter))&&(d.get_filterDelay()!=null||(n!=null&&n!==""&&k))){if(d._data.Acff&&d._data.Acff!="NoFilter"){i=d._data.Acff;
...
this._owner.updateClientState();
},fireCommand:function(e,d){if(this._raiseCommandEvent(e,d)){if(e&&["ExportToExcel","ExportToWord","ExportToCsv","ExportToPdf"].indexOf(e)>-1){this._owner.__isExporting=true;
}this._executePostBackEvent("FireCommand:"+this._data.UniqueID+";"+e+";"+d);
...
Telerik.Web.UI.GridFilterFunction.tryParse=function(f,g,e){if(typeof g=="number"){return g;
}try{return f(g,!!e);
}catch(d){return null;
}};
...
this._groupPanel.style.touchAction="none";
...
if(b){this.raise_rowClick(new Telerik.Web.UI.GridDataItemEventArgs(b,a));
...
Telerik.Web.UI.GridMobileColumnsView=function(d){b.GridMobileColumnsView.initializeBase(this,[d]);
this._indexes=[];
this._uniqueNames=[];
this._changes=null;
...
b.GridMobileColumnsView.prototype={initialize:function(){b.GridMobileColumnsView.callBaseMethod(this,"initialize");
},dispose:function(){b.GridMobileColumnsView.callBaseMethod(this,"dispose");
},onInit:function(){this._initDragDrop();
},onShown:function(){this._changes=[];
if($telerik.isMobileIE10){this.get_$element().css("touch-action","none");
}this._$clone=this.get_$element().children().clone();
this._$columns=this.get_$element().find(".rgColumnItem");
this._initPositions();
},onApplyChanges:function(){var d=this.get_owner();
this._changes.forEach(function(e){d._reorderColumns(this._uniqueNames[e[0]],this._uniqueNames[e[1]]);
this._updateMetadata();
},this);
if(d._owner.ClientSettings.AllowColumnHide){this._$columns.each(function(){var e=a(this);
...
if(!this._draggable._isDragging||!i||!f){return;
...
this._uniqueNames=[];
this._indexes=[];
d.forEach(function(f,g){if(!e._isHelperColumnType(f._data.ColumnType)){this._indexes.push(g);
...
b.GridMobileColumnsView.registerClass("Telerik.Web.UI.GridMobileColumnsView",Telerik.Web.UI.GridMobileView);
})();
...
}else{if(c.hasClass("rgPrint")){this.close();
f.get_owner().print();
}}}}}}};
...
},_initializeEvents:function(){if($telerik.isMobileIE10){this._$element.css("touch-action","none");
}this._$element.find(".rgUngroup").offEvent(b.EventNamesMap.up).onEvent(b.EventNamesMap.up,a.proxy(this._handleCloseUp,this)).on("click",$telerik.preventDefault);
