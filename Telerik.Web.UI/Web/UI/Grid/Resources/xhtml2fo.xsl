...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="process-common-attributes">
    ...
    <xsl:choose>
	  ...
	  <!-- DESHEV: removed setting ID attributes to work around an APOC bug that 
	  added elements twice and crashed with a duplicate id exception -->
  ...
  <xsl:template name="process-table-column">
	...
	<xsl:template match="html:tr[@style and contains(translate(normalize-space(@style),' ',''), 'display:none')]">
	</xsl:template>
  ...
  <xsl:template match="processing-instruction('page-number')">
    <fo:page-number />
  </xsl:template>
