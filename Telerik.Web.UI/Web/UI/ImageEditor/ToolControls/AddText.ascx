...
<table id="Table" class="rieDialogsTable" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<td>
			<asp:TextBox ID="size" runat="server" ToolTip="Font Size ToolTip" CssClass="rieInputDigits" autocomplete="off" />
			<span class="riePxLabel"><asp:Literal ID="Literal1" Text=" px" runat="server" /></span>
		</td>
	</tr>
	<tr>
		<td class="rieRightAligned">
		...
		<td class="rieColorValue">
			<asp:TextBox ID="color" runat="server" ToolTip="Font Size ToolTip" CssClass="rieInputHex" autocomplete="off" />
	...
	<tr>
		<td class="rieRightAligned">
			<span class="rfdAspLabel"><asp:Literal ID="lPosition" Text="Position:" runat="server" /></span>
		</td>
		<td>
			<asp:Label ID="lblX" Text="X " runat="server" AssociatedControlID="txtX" CssClass="riePositionLabel" />
			<asp:TextBox ID="txtX" runat="server" ToolTip="Left" CssClass="rieInputDigits" />
			<span class="riePxLabel"><asp:Literal ID="Literal2" Text=" px" runat="server" /></span>
			&nbsp;&nbsp;&nbsp;
			<asp:Label ID="lblY" Text="Y " runat="server" AssociatedControlID="txtY" CssClass="riePositionLabel" />
			<asp:TextBox ID="txtY" runat="server" ToolTip="Left" CssClass="rieInputDigits" />
			<span class="riePxLabel"><asp:Literal ID="Literal3" Text=" px" runat="server" /></span>
		</td>
	</tr>
	<tr>
		<td class="rieRightAligned rieClearRightSpace" colspan="2">
			<telerik:RadButton ID="btnApply" runat="server" Text="Insert" ToolTip="Insert" AutoPostBack="false"
				CausesValidation="false" Primary="true" EnableViewState="false" CssClass="rbFlatButton">
			</telerik:RadButton>
			<telerik:RadButton ID="btnCancel" runat="server" Text="Cancel" ToolTip="Cancel" AutoPostBack="false"
				CausesValidation="false" EnableViewState="false" CssClass="rbFlatButton">
			</telerik:RadButton>
		</td>
	</tr>
</table>
		...
		(function ($, $T, $IE)
		{
			...
			$IE.AddText.prototype =
			{
				...
				dispose: function ()
				{
					...
					this._fontsDD.dispose();
					this._colorPicker.dispose();
					this._btnApply.dispose();
				...
				_attachEventHandlers: function (toAttach)
				{
					if (toAttach)
					{
						...
						this._changeColorTxtDelegate = Function.createDelegate(this, this._changeColorTxtHandler);
						$telerik.addHandler(this._colorTxt, "change", this._changeColorTxtDelegate);
					...
					else
					{
						...
						$telerik.removeHandler(this._colorTxt, "change", this._changeColorTxtDelegate);
						...
						this._changeYDelegate = this._colorChangeDelegate = this._textBoundsChangedDelegate =
						this._changeColorTxtDelegate = null;
					}
				},
				...
				_textBoundsChangedHandler: function (sender, args)
				{
					var dragPosition = args.get_position();
				...
				{
					//escape any HTML from the input to HTML entities to prevent DOM-based XSS
					this._draggableText.set_text($telerik.htmlEncode(this._textArea.value));
				},
				...
				_changeColorTxtHandler: function (event) {
					var input = event.target;
					if (this._isValidHex(input)) {
						this._colorPicker.set_selectedColor(input.value);
					}
				},
				_isValidHex: function (input) {
