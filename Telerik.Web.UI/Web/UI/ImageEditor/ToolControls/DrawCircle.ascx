...
<table id="Table" class="rieDialogsTable" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		<td class="rieRightAligned">
			<label id="lblFillColor" runat="server">
				Fill Color:</label>
		</td>
		<td>
	...
	<tr>
		<td class="rieRightAligned rieBottomSpace">
			<label id="lblLineSize" runat="server">
				Line size:</label>
		</td>
		<td class="rieBottomSpace">
	...
	<tr>
		<td class="rieRightAligned rieBottomCell">
			<label id="lblTransform" runat="server">
				Transform:</label>
		</td>
		<td class="rieBottomCell">
			<telerik:RadButton ID="transformButton" runat="server" Text="Apply" Enabled="false" AutoPostBack="false" EnableViewState="false" CssClass="rbFlatButton" />
		</td>
	</tr>
</table>
<script type="text/javascript">
	...
	if (typeof (Telerik.Web.UI.ImageEditor.DrawCircle) === "undefined")
	{
		Type.registerNamespace("Telerik.Web.UI.ImageEditor");
		(function ($, $T, $IE)
		{
			$IE.DrawCircle = function (imageEditor)
			{
				this._originalToolsPanelWidth = imageEditor.get_toolsPanel().get_width();
				
				$IE.DrawCircle.initializeBase(this, [imageEditor]);

				imageEditor.get_toolsPanel().set_width(190);
			}
			$IE.DrawCircle.prototype =
			{
				initialize: function ()
				{
					$IE.DrawCircle.callBaseMethod(this, "initialize");
				...
				dispose: function ()
				{
					...
					this.fillColorPicker.dispose();
					this.colorPicker.dispose();
					this.sizeCombo.dispose();
					this.transformButton.dispose();

					$IE.DrawCircle.callBaseMethod(this, "dispose");
				},
				...
				{
					this.drawTool = new $IE.DrawTools.CircleTool(this.get_imageEditor().getCanvasContext(), { coreFactory: new $IE.GraphicsCore.CanvasGraphicsCoreFactory() });
				...
				get_name: function () { return "DrawCircle"; },

				updateUI: function () {}
				/*=================================  Telerik.Web.UI.ImageEditor.IToolWidged members  ===============================*/
			};
			$IE.DrawCircle.registerClass("Telerik.Web.UI.ImageEditor.DrawCircle", $IE.ToolWidget, $IE.IToolWidget);
