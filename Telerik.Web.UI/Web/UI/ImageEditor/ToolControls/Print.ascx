...
<table id="Table" class="rieDialogsTable" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		<td class="rieRightAligned rieClearRightSpace" style="padding-top: 15px;">
			<telerik:RadButton ID="btnPrint" runat="server" Text="Print" ToolTip="Print" AutoPostBack="false"
				CausesValidation="false" EnableViewState="false" CommandName="Print" Primary="true" CssClass="rbFlatButton">
		...
		(function ($, $T, $IE)
		{
			...
			$IE.Print.prototype =
			{
				...
				printFromIframe: function() {
					...
					var canvas = this.get_imageEditor().getCanvasElement();
					var useImgeSrc = !canvas && (Browser.opera || Browser.chrome);
					//When browser is Opera or Chrome and CanvasMode="No", frameDocument.write does not work with uploaded images.
					var src = !useImgeSrc ? 'about:blank' : window.location.origin + "/" + this._getPrintImage().attr("src").replace(/^[\/\\]/, "");
					var style = "style='border:none; position:absolute; width:0; height:0; bottom0; left0;'";
					var iframe = $("<iframe src='" + src + "' " + style + " name='print-image-frame' frameborder='0' scrolling='no' id='print-image-frame'>").appendTo("body").get(0);
