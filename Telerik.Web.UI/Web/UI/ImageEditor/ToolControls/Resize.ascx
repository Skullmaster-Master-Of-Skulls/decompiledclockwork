...
<table id="Table" class="rieDialogsTable" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<td colspan="2">
			<asp:TextBox ID="TxtPercent" runat="server" ToolTip="Percentage" CssClass="rieInputDigits" />
			<span class="riePxLabel"><asp:Literal ID="Literal2" Text=" %" runat="server" /></span>
		</td>
	</tr>
	<tr>
		<td class="rieRightAligned rieClearRightSpace" colspan="3">
			<telerik:RadButton ID="BtnResize" runat="server" Text="Resize" ToolTip="Resize" AutoPostBack="false"
				CausesValidation="false" EnableViewState="false" CommandName="Resize" Primary="true" CssClass="rbFlatButton">
			</telerik:RadButton>
	...
	if (typeof (Telerik.Web.UI.ImageEditor.Resize) === "undefined")
	{
		...
		(function ($, $T, $IE)
		{
			...
			$IE.Resize.prototype =
			{
				...
				{
					if (!this._resizeBtn) {
						this._resizeBtn = $find(this.get_parentId() + "BtnResize");
					}
				...
				{
					if (!this._constraintBtn) {
						this._constraintBtn = $find(this.get_parentId() + "BtnConstraint");
					}
				...
				{
					if (!this._swapBtn) {
						this._swapBtn = $find(this.get_parentId() + "BtnSwap");
					}
				...
				{
					if (!this._comboSizes) {
						this._comboSizes = $find(this.get_parentId() + "PresetSizes");
					}
