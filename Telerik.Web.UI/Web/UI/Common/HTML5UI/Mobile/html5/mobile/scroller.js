...
}if(L.paneAxis.outOfBounds()){if(L.transition._started){L.transition.cancel();
L.velocity=Math.min(K.touch[L.axis].velocity*L.velocityMultiplier,n);
f.fn.start.call(L);
}else{L._snapBack();
...
}},_wheelScroll:function(L){if(L.ctrlKey){return;
}if(!this._wheel){this._wheel=true;
this._wheelY=0;
...
},scrollTo:function(K,L){if(this._native){m.scrollLeft(this.scrollElement,c(K));
