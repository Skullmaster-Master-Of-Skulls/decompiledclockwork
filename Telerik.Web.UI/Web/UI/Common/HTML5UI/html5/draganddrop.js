...
if(E.support.touch){ab.element.find(ab.options.filter).css("touch-action","none");
...
}},_start:function(aa){var ae=this,ad=ae.options,Z=ad.container?b(ad.container):null,ab=ad.hint;
...
}if(this.hint&&ab&&(af||ai)){if(ai){Z.top+=ad.y;
}if(af){Z.left+=ad.x;
}this.hint.css(Z);
...
return ab;
}}function P(){return b(E.support.browser.edge||E.support.browser.safari?k.body:k.documentElement);
