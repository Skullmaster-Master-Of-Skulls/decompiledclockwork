...
},_updateDocument:function(j){if(this.pdf&&this.pdf.loadingTask){this.pdf.loadingTask.destroy();
}this.file=j;
...
}(function(){(function(a,n){var i=".kendoPDFViewer",o=kendo.ui.Widget,c="change",h="keydown",d=kendo.support.click,m=480,g=kendo.attr,l=a.proxy,f=a.extend,e=".",b="aria-disabled";
var k={wrapperClass:"k-pager-wrap",iconFirst:"k-i-arrow-end-left",iconLast:"k-i-arrow-end-right",iconPrev:"k-i-arrow-60-left",iconNext:"k-i-arrow-60-right",first:"k-pager-first",last:"k-pager-last",nav:"k-pager-nav",disabled:"k-state-disabled"};
var j=o.extend({init:function(p,q){var r=this;
o.fn.init.call(r,p,q);
r.linkTemplate=kendo.template(r.options.linkTemplate);
r.element.addClass(k.wrapperClass);
r._renderLinks();
r._toggleDisabledClass();
r._attachEvents();
},options:{linkTemplate:'<a href="\\#" aria-label="#=text#" title="#=text#" class="k-button k-button-icon k-flat k-pager-nav #= wrapClass #" data-page="#=pageIdx#"><span class="k-icon #= iconClass #"></span></a>',previousNext:true,input:true,page:1,total:1,messages:{}},events:[c],_pagerLink:function(p,r,q,s){return this.linkTemplate({iconClass:p,text:r,wrapClass:s||"",pageIdx:q});
},_renderLinks:function(){var q=this,p=q.options;
if(p.previousNext){q.firstLink=a(q._pagerLink(k.iconFirst,p.messages.first,1,k.first));
q.prevLink=a(q._pagerLink(k.iconPrev,p.messages.previous,p.page-1));
q.nextLink=a(q._pagerLink(k.iconNext,p.messages.next,p.page+1));
q.lastLink=a(q._pagerLink(k.iconLast,p.messages.last,p.total,k.last));
}q.element.append(q.firstLink);
q.element.append(q.prevLink);
if(p.input){q._renderInput();
}q.element.append(q.nextLink);
q.element.append(q.lastLink);
},_toggleDisabledClass:function(){var q=this,p=q.options,r=!p.total;
if(q.nextLink&&q.prevLink){q.prevLink.toggleClass(k.disabled,r||p.page===1);
q.nextLink.toggleClass(k.disabled,r||p.page===p.total);
q.lastLink.toggleClass(k.disabled,r||p.page===p.total);
q.firstLink.toggleClass(k.disabled,r||p.page===1);
q.prevLink.attr(b,r||p.page===1);
q.firstLink.attr(b,r||p.page===1);
q.nextLink.attr(b,p.page===p.total);
q.lastLink.attr(b,p.page===p.total);
}if(q.input){q.input.toggleClass(k.disabled,p.total<=1);
q.input.attr(b,p.total<=1);
}},_attachEvents:function(){var p=this;
p.element.on(d+i,e+k.nav,l(p._click,p));
p.element.on(h+i,e+k.nav,function(q){if(q.keyCode===kendo.keys.ENTER){p._click(q);
q.preventDefault();
}});
if(p.input){p.input.on(h+i,l(p._keydown,p));
}},_click:function(p){var r=a(p.currentTarget);
var q=parseInt(r.attr(g("page")),10);
if(p.isDefaultPrevented()){return;
}this._change(q);
},_keydown:function(r){var t=r.keyCode,u=kendo.keys,s=a(r.target),v=parseInt(s.val(),10),w=t===u.UP||t===u.DOWN,p=t===u.RIGHT||t===u.LEFT||t===u.BACKSPACE||t===u.DELETE,q=w&&t===u.UP?1:-1;
if(w){v+=q;
}if(t===u.ENTER||w){if(isNaN(v)||v<1||v>this.options.total){v=this.options.page;
s.val(v);
return;
}this._change(v);
r.preventDefault();
}else{if(!r.key.match(/^\d+$/)&&!p){r.preventDefault();
}}},_change:function(p){var q=this;
if(p>=1&&p<=q.options.total){q.options.page=p;
if(q.input){q.input.val(p);
}q.prevLink.attr(g("page"),p-1);
q.nextLink.attr(g("page"),p+1);
q._toggleDisabledClass();
}q.trigger(c,{page:p});
},setOptions:function(p){var r=this,q=r.options.total;
p=a.extend(r.options,p);
o.fn.setOptions.call(r,p);
if(p.input){if(q!=p.total){r._renderInput();
r.input.on(h+i,l(r._keydown,r));
}else{r.input.val(p.total>0?p.page:1);
}}if(p.total>1){r.prevLink.attr(g("page"),p.page-1);
r.nextLink.attr(g("page"),p.page+1);
r.lastLink.attr(g("page"),p.total);
}r._toggleDisabledClass();
},_renderInput:function(){var t=this,u,q=t.options,s=t.element.parent().width()<=m,p,r=t.element.find(".k-pager-input");
if(t.input){t.input.off(i);
}u=q.total>0?kendo.format(q.messages.of,q.total):"";
if(!s){u+=q.total>1?q.messages.pages:q.messages.page;
}p='<input class="k-textbox" aria-label="'+q.page+u+'">'+u;
if(r.length){r.html(p);
}else{t.element.append('<span class="k-pager-input k-label">'+p+"</span>");
}t.input=t.element.find("input").val(q.total>0?q.page:1);
},destroy:function(){this.element.off(i);
if(this.input){this.input.off(i);
}o.fn.destroy.call(this);
}});
f(kendo.pdfviewer,{Pager:j});
}(window.kendo.jQuery));
return window.kendo;
},typeof define=="function"&&define.amd?define:function(a,b,c){(c||b)();
}));
...
var w=k.template('<select title="#=zoomLevel#">#for(var zoomIndex in zoomLevels){## var zoomLevel = zoomLevels[zoomIndex]; #<option value="#= zoomLevel.percent || (zoomLevel + "%") #">${zoomLevel.text ? zoomLevel.text : zoomLevel + "%"}</option>#}#</select>');
...
z=a(w({zoomLevels:D,zoomLevel:B.zoomLevel}));
...
p.element.append(n);
},load:function(n,o){var q=this,p=a.Deferred();
if(q._scale===n&&!o){return;
}else{if(q._scale&&q._scale!==n&&!o){q._scale=n;
q.render(n);
return p.resolve(q);
}}if(q.processor){q.processor.fetchPageData(q.pageNumber).then(function(r){q._page=r;
q._renderPromise=q.render(n).then(function(){q.viewer.trigger(k,{page:q});
});
p.resolve(q);
});
}q._scale=n;
...
if(F&&F.scrollTop>F.scrollHeight()){F._resize();
...
}if(v.updateComboBox&&A.toolbar){A.toolbar._updateZoomComboBox(y);
}return z;
...
return;
}p(v.viewer.pageContainer,true);
v._renderPrintContainer();
v._loadAllPages().then(q(v.processAfterRender,v));
...
var A=[];
var z=a.Deferred();
B._originalScale=B.viewer.zoom();
function v(C){A.push(C._renderPromise);
}for(var w=0;
w<y.length;
w++){x.push(y[w].load(3,true).then(v));
}Promise.all(x).then(function(){z.resolve(A);
});
return z;
},processAfterRender:function(v){var w=this;
Promise.all(v).then(function(){w._renderPrintPages();
setTimeout(function(){w._printDocument();
w.viewer.zoom(w._originalScale);
p(w.viewer.pageContainer,false);
delete w._originalScale;
},0);
});
...
if(!w){y.viewer._triggerError({message:y.errorMessages.popupBlocked});
return;
...
if(F.pdfjsProcessing||F.dplProcessing){G._initProcessor(F||{});
