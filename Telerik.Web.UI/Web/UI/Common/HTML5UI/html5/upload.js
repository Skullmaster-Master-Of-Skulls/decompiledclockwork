...
var S=Y.extend({init:function(aa,ac){var ad=this;
Y.fn.init.call(ad,aa,ac);
ad.name=aa.name;
ad.multiple=ad.options.multiple;
ad.directory=ad.options.directory;
ad.localization=ad.options.localization;
var Z=ad.element;
ad.wrapper=Z.closest(".k-upload");
if(ad.wrapper.length===0){ad.wrapper=ad._wrapInput(Z);
}ad._activeInput(Z);
ad.toggle(ad.options.enabled);
var ab=ad._ns=A+"-"+x.guid();
Z.closest("form").on("submit"+ab,b.proxy(ad._onParentFormSubmit,ad)).on("reset"+ab,b.proxy(ad._onParentFormReset,ad));
ad._initUploadModule();
ad._toggleDropZone();
ad.wrapper.on("click",".k-upload-action",b.proxy(ad._onFileAction,ad)).on("click",".k-clear-selected",b.proxy(ad._onClearSelected,ad)).on("click",".k-upload-selected",b.proxy(ad._onUploadSelected,ad));
if(ad.element.val()){ad._onInputChange({target:ad.element});
}},events:[K,T,O,i,h,f,g,D,G,C,I],options:{name:"Upload",enabled:true,multiple:true,directory:false,showFileList:true,template:"",files:[],async:{autoRetryAfter:0,bufferChunkSize:10000000,maxAutoRetries:1,removeVerb:"POST",autoUpload:true,withCredentials:true,accept:"*/*; q=0.5, application/json",useArrayBuffer:false},localization:{select:"Select files...",cancel:"Cancel",retry:"Retry",remove:"Remove",pause:"Pause",resume:"Resume",clearSelectedFiles:"Clear",uploadSelectedFiles:"Upload",dropFilesHere:"Drop files here to upload",invalidFiles:"Invalid file(s). Please check file upload requirements.",statusUploading:"uploading",statusUploaded:"uploaded",statusWarning:"warning",statusFailed:"failed",headerStatusUploading:"Uploading...",headerStatusPaused:"Paused",headerStatusUploaded:"Done",uploadSuccess:"File(s) uploaded successfully.",uploadFail:"File(s) failed to upload.",invalidMaxFileSize:"File size too large.",invalidMinFileSize:"File size too small.",invalidFileExtension:"File type not allowed."},validation:{allowedExtensions:[],maxFileSize:0,minFileSize:0},dropZone:""},_initUploadModule:function(){var ab=this,aa=ab.options;
if(aa.async.saveUrl){ab._module=ab._supportsFormData()?new j(ab):new r(ab);
ab._async=true;
var Z=aa.files;
if(Z.length>0){ab._renderInitialFiles(Z);
}}else{ab._module=new P(ab);
}},setOptions:function(aa){var ab=this,Z=ab.element;
...
var aa=ab._ns;
var Z=b(".k-dropzone",ab.wrapper).on("dragenter"+aa,M).on("dragover"+aa,function(ac){if(ac.originalEvent){ac.originalEvent.dataTransfer.dropEffect="copy";
}ac.preventDefault();
...
Z.on("dragenter"+aa,M).on("dragover"+aa,function(ac){if(ac.originalEvent){ac.originalEvent.dataTransfer.dropEffect="copy";
}ac.preventDefault();
}).on("drop"+aa,b.proxy(ab._onDrop,ab));
...
}function M(Z){if(Z.originalEvent){Z.originalEvent.dataTransfer.dropEffect="copy";
}Z.stopPropagation();
Z.preventDefault();
