...
},arrowClass:function(a2,a1){var a3="k-menu-expand-arrow k-icon";
...
var a2=a1.attr(S.attr("omit-wrap"));
if(a2){return;
}a1.addClass("k-item k-menu-item").children(M).addClass(L);
a1.children("a").addClass(X).children(M).addClass(L);
a1.filter(":not([disabled])").addClass(q);
a1.filter(".k-separator").removeClass("k-menu-item").addClass("k-menu-separator").empty().append("&nbsp;");
...
}if(!a1.children(Y).length){a1.contents().filter(function(){return !this.nodeName.match(x)&&!(this.nodeType==3&&!S.trim(this.nodeValue));
...
a3.append("<span class='k-menu-expand-arrow k-icon "+a2+"' />");
});
...
}function aw(a1){var a2=a1?"[data-group='"+a1+"']":"[data-group]";
return"ul"+a2+",div"+a2;
...
},_toggleScrollButtons:function(a6,a1,a3,a4){var a2=a4?S.scrollLeft(a6):a6.scrollTop();
var a7=a4?aH:aF;
var a5=a4?am:al;
a1.toggle(a2!==0);
...
var a1=this._insert(a2,a3,a3.length?this._childPopupElement(a3):null);
...
if(bl&&!bl.parentNode){return;
}if(b(bk).hasClass("k-menu-expand-arrow")){this._lastClickedElement=bl.parentElement;
...
return a1;
},_documentClick:function(a1){var a3=this;
var a2=b(a1.target).hasClass("k-menu-expand-arrow")?a3._lastClickedElement:a1.target;
if(m((a3._overflowWrapper()||a3.element)[0],a2)){a3._lastClickedElement=aU;
return;
}a3.clicked=false;
...
}if(a1.appendTo){a1.appendTo=b(a1.appendTo);
a1.appendTo.append(a2._popupsWrapper);
...
a3._preventProxy=function(){return false;
};
a3.userEvents.bind("hold",a3._showProxy);
}if(a1.filter){a2.on(a1.showOn+ak+a3._marker,a1.filter,a3._preventProxy||a3._showProxy);
}else{a2.on(a1.showOn+ak+a3._marker,a3._preventProxy||a3._showProxy);
