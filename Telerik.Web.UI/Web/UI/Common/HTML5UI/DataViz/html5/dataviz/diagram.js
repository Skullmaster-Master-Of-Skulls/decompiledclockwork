...
}(function(){(function(a,aE){var R=window.kendo,u=R.dataviz,B=u.diagram,j=R.Class,I=B.Group,ai=B.Rect,aj=B.Rectangle,aH=B.Utils,O=aH.isUndefined,ad=B.Point,i=B.Circle,ax=B.Ticker,v=R.deepExtend,Z=R.ui.Movable,aG=R.drawing.util,y=aG.defined,N=a.inArray,ah=a.proxy;
var t={arrow:"default",grip:"pointer",cross:"pointer",add:"pointer",move:"move",select:"pointer",south:"s-resize",east:"e-resize",west:"w-resize",north:"n-resize",rowresize:"row-resize",colresize:"col-resize"},K=10,e="Auto",aB="Top",al="Right",T="Left",f="Bottom",x=10,w=10,E="dragStart",C="drag",D="dragEnd",Q="itemRotate",P="itemBoundsChange",W=5,V=5,X="mouseEnter",Y="mouseLeave",aK="zoomStart",aJ="zoomEnd",ap=-20000,ao=20000,G=0.9,H=0.9300000000000001,aI=5,aD="transparent",ab="pan",am="rotated",au="source",av="target",J={"-1":au,"1":av};
B.Cursors=t;
var ag=R.Class.extend({init:function(aL){this.layoutState=aL;
...
aL++){this.subjects[aL].position(new ad(this.froms[aL].x+(this.tos[aL].x-this.froms[aL].x)*aM,this.froms[aL].y+(this.tos[aL].y-this.froms[aL].y)*aM));
}}});
var S=j.extend({init:function(aN,aM,aL){if(O(aL)){this.animate=false;
...
var k=j.extend({init:function(aL){this.units=[];
this.title="Composite unit";
if(aL!==aE){this.units.push(aL);
...
var o=j.extend({init:function(aL,aM,aN){this.item=aL;
this._redoSource=aM;
this._redoTarget=aN;
if(y(aM)){this._undoSource=aL.source();
}if(y(aN)){this._undoTarget=aL.target();
...
var n=j.extend({init:function(aL,aM,aN){this.item=aL;
this._undoSource=aM;
this._undoTarget=aN;
...
var z=j.extend({init:function(aL){this.connection=aL;
this.diagram=aL.diagram;
...
var A=j.extend({init:function(aL){this.shape=aL;
this.diagram=aL.diagram;
this.title="Deletion";
...
var aC=j.extend({init:function(aO,aP,aL){this.shapes=aO;
this.undoStates=aP;
...
var b=j.extend({init:function(aL,aM){this.connection=aL;
this.diagram=aM;
...
var c=j.extend({init:function(aM,aL){this.shape=aM;
this.diagram=aL;
...
var ac=j.extend({init:function(aN,aM,aL){this.initial=aN;
this.finalPos=aM;
this.diagram=aL;
this.title="Pan Unit";
...
var an=j.extend({init:function(aL,aO,aP){this.shapes=aO;
this.undoRotates=aP;
this.title="Rotation";
this.redoRotates=[];
...
var az=j.extend({init:function(aL,aN,aM){this.diagram=aL;
this.indices=aM;
this.items=aN;
...
var ay=j.extend({init:function(aL,aN,aM){this.diagram=aL;
this.indices=aM;
this.items=aN;
this.title="Rotate Unit";
},undo:function(){this.diagram._toIndex(this.items,this.indices);
...
var aF=R.Observable.extend({init:function(aL){R.Observable.fn.init.call(this,aL);
...
var F=j.extend({init:function(aL){this.toolService=aL;
},start:function(){},move:function(){},end:function(){},tryActivate:function(){return false;
},getCursor:function(){return t.arrow;
}});
var aq=F.extend({init:function(aQ){var aP=this;
var aN=R.support.mobileOS?H:G;
F.fn.init.call(aP,aQ);
var aM=aP.toolService.diagram,aL=aM.canvas;
var aO=aM.scroller=aP.scroller=a(aM.scrollable).kendoMobileScroller({friction:aN,velocityMultiplier:aI,mousewheelScrolling:false,zoom:false,scroll:ah(aP._move,aP)}).data("kendoMobileScroller");
if(aL.translate){aP.movableCanvas=new Z(aL.element);
}var aR=function(aS,aU,aT){aS.makeVirtual();
aS.virtualSize(aU||ap,aT||ao);
};
aR(aO.dimensions.x);
aR(aO.dimensions.y);
aO.disable();
...
var aL=aM.ctrlKey;
if(y(aN.key)){if(!aN.key||aN.key=="none"){aL=aa(aM)&&!y(aP.hoveredItem);
}else{aL=aM[aN.key+"Key"];
}}return aN!==false&&aL&&!y(aP.hoveredAdorner)&&!y(aP._hoveredConnector);
},start:function(){this.scroller.enable();
},move:function(){},_move:function(aL){var aP=this,aN=aP.toolService.diagram,aM=aN.canvas,aO=new ad(aL.scrollLeft,aL.scrollTop);
if(aM.translate){aN._storePan(aO.times(-1));
aP.movableCanvas.moveTo(aO);
aM.translate(aO.x,aO.y);
}else{aO=aO.plus(aN._pan.times(-1));
}aN.trigger(ab,{pan:aO});
},end:function(){this.scroller.disable();
},getCursor:function(){return t.move;
}});
var ae=j.extend({init:function(aL){this.toolService=aL;
...
}}if(this.adorner){if(!this.adorner.isDragHandle(this.handle)||!aL.trigger(E,{shapes:this.adorner.shapes,connections:[]})){this.adorner.start(aO);
}else{aP.startPoint=aO;
aP.end(aO);
}}},move:function(aL){if(this.adorner){this.adorner.move(this.handle,aL);
if(this.adorner.isDragHandle(this.handle)){this.toolService.diagram.trigger(C,{shapes:this.adorner.shapes,connections:[]});
}}},end:function(){var aM=this.toolService.diagram,aL=this.adorner,aN;
if(aL){if(!aL.isDragHandle(this.handle)||!aM.trigger(D,{shapes:aL.shapes,connections:[]})){aN=aL.stop();
if(aN){aM.undoRedoService.add(aN,false);
}}else{aL.cancel();
}}this.adorner=aE;
this.handle=aE;
},getCursor:function(aL){return this.toolService.hoveredItem?this.toolService.hoveredItem._getCursor(aL):t.arrow;
}});
var ar=j.extend({init:function(aL){this.toolService=aL;
...
}}return aL&&!y(aP.hoveredItem)&&!y(aP.hoveredAdorner);
...
var q=j.extend({init:function(aL){this.toolService=aL;
...
if(g(aL)&&!aN.trigger(E,{shapes:[],connections:[aL],connectionHandle:av})&&aN._addConnection(aL)){aQ._connectionManipulation(aL,aM._c.shape,true);
aQ._removeHover();
aQ.selectSingle(aQ.activeConnection,aO);
if(aO.type=="touchmove"){aN._cachedTouchTarget=aM.visual;
}}else{aL.source(null);
aQ.end(aP);
}},move:function(aM){var aN=this.toolService;
var aL=aN.activeConnection;
aL.target(aM);
aN.diagram.trigger(C,{shapes:[],connections:[aL],connectionHandle:av});
return true;
},end:function(aQ){var aS=this.toolService,aO=aS.diagram,aM=aS.activeConnection,aP=aS.hoveredItem,aN=aS._hoveredConnector,aR,aL=aO._cachedTouchTarget;
if(!aM){return;
}if(aN&&aN._c!=aM.sourceConnector){aR=aN._c;
}else{if(aP&&aP instanceof B.Shape){aR=aP.getConnector(e)||aP.getConnector(aQ);
}else{aR=aQ;
}}aM.target(aR);
if(!aO.trigger(D,{shapes:[],connections:[aM],connectionHandle:av})){aM.updateModel();
aO._syncConnectionChanges();
}else{aO.remove(aM,false);
aO.undoRedoService.pop();
}aS._connectionManipulation();
if(aL){aO._connectorsAdorner.visual.remove(aL);
aO._cachedTouchTarget=null;
}},getCursor:function(){return t.arrow;
}});
var m=j.extend({init:function(aL){this.toolService=aL;
this.type="ConnectionTool";
...
aP=J[aN];
}if(g(aM)&&aL&&!aR.diagram.trigger(E,{shapes:[],connections:[aM],connectionHandle:aP})){this.handle=aN;
this.handleName=aP;
aL.start(aQ);
}else{aR.startPoint=aQ;
aR.end(aQ);
}},move:function(aM){var aL=this._c.adorner;
if(g(this._c)&&aL){aL.move(this.handle,aM);
this.toolService.diagram.trigger(C,{shapes:[],connections:[this._c],connectionHandle:this.handleName});
return true;
}},end:function(aO){var aM=this._c;
var aL=aM.adorner;
var aP=this.toolService;
var aN=aP.diagram;
if(aL){if(g(aM)){var aQ=aL.stop(aO);
if(!aN.trigger(D,{shapes:[],connections:[aM],connectionHandle:this.handleName})){aN.undoRedoService.add(aQ,false);
aM.updateModel();
aN._syncConnectionChanges();
}else{aQ.undo();
}}}},getCursor:function(){return t.move;
}});
function aw(aL,aM){return aM.charCodeAt(0)==aL||aM.toUpperCase().charCodeAt(0)==aL;
}var aA=j.extend({init:function(aL){this.diagram=aL;
this.tools=[new aq(this),new m(this),new q(this),new ar(this),new ae(this)];
this.activeTool=aE;
},start:function(aM,aL){aL=v({},aL);
...
aN=v({ctrlKey:false,metaKey:false,altKey:false},aN);
if((aN.ctrlKey||aN.metaKey)&&!aN.altKey){if(aw(aM,"a")){aL.selectAll();
aL._destroyToolBar();
return true;
}else{if(aw(aM,"z")){aL.undo();
aL._destroyToolBar();
return true;
}else{if(aw(aM,"y")){aL.redo();
aL._destroyToolBar();
return true;
}else{if(aw(aM,"c")){aL.copy();
aL._destroyToolBar();
}else{if(aw(aM,"x")){aL.cut();
aL._destroyToolBar();
}else{if(aw(aM,"v")){aL.paste();
aL._destroyToolBar();
}else{if(aw(aM,"l")){aL.layout();
aL._destroyToolBar();
}else{if(aw(aM,"d")){aL._destroyToolBar();
aL.copy();
aL.paste();
...
}aQ=R.dataviz.round(Math.max(aO.zoomMin,Math.min(aO.zoomMax,aQ)),2);
aR.zoom=aQ;
aM.zoom(aQ,aR);
aM.trigger(aJ,aR);
return true;
...
var aL=this.activeTool?this.activeTool.getCursor(aN):this.hoveredAdorner?this.hoveredAdorner._getCursor(aN):this.hoveredItem?this.hoveredItem._getCursor(aN):t.arrow;
aM.css({cursor:aL});
...
var aL=this.diagram;
if(aM!=this.hoveredItem&&(!this.disabledShape||aM!=this.disabledShape)){if(this.hoveredItem){aL.trigger(Y,{item:this.hoveredItem});
this.hoveredItem._hover(false);
}if(aM&&aM.options.enable){aL.trigger(X,{item:aM});
this.hoveredItem=aM;
...
if(aO instanceof B.Connection){aQ.push(aO);
...
var aO;
if((!this.activeTool||this.activeTool.type!="ConnectionTool")&&aR&&aL&&!L(aR,aQ)){var aP=aN.mainLayer;
var aS=N(aR.visual,aP.children);
var aM=N(aL.visual,aP.children);
aO=aS>aM?aR:aL;
}return aO||aR||aL;
...
var p=R.Class.extend({init:function(){}});
var U=p.extend({init:function(aL){var aM=this;
p.fn.init.call(aM);
this.connection=aL;
},hitTest:function(aL){var aM=this.getBounds().inflate(K);
if(!aM.contains(aL)){return false;
}return B.Geometry.distanceToPolyline(aL,this.connection.allPoints())<K;
...
}return new ai(aO,aS,aQ-aO,aL-aS);
}});
var af=U.extend({init:function(aL){var aM=this;
U.fn.init.call(aM);
this.connection=aL;
},route:function(){}});
var h=U.extend({SAME_SIDE_DISTANCE_RATIO:5,init:function(aL){var aM=this;
U.fn.init.call(aM);
this.connection=aL;
...
},_sameSideDistance:function(aM){var aL=aM.shape.bounds(am);
...
var aQ=[];
var aO,aP;
if(aT===aB||aT==f){if(aW==aB||aW==f){if(aT==aW){if(aT==aB){aP=Math.min(aU.y,aN.y)-aR;
}else{aP=Math.max(aU.y,aN.y)+aR;
}aQ=[new ad(aU.x,aP),new ad(aN.x,aP)];
}else{aQ=[new ad(aU.x,aU.y+aM/2),new ad(aN.x,aU.y+aM/2)];
}}else{aQ=[new ad(aU.x,aN.y)];
}}else{if(aW==T||aW==al){if(aT==aW){if(aT==T){aO=Math.min(aU.x,aN.x)-aR;
}else{aO=Math.max(aU.x,aN.x)+aR;
}aQ=[new ad(aO,aU.y),new ad(aO,aN.y)];
}else{aQ=[new ad(aU.x+aL/2,aU.y),new ad(aU.x+aL/2,aU.y+aM)];
}}else{aQ=[new ad(aN.x,aU.y)];
}}return aQ;
...
aT=0;
}}aR[aP]=new ad(aR[aP-1].x+aS,aR[aP-1].y+aT);
}aP--;
if(aL&&aP%2!==0||!aL&&aP%2===0){aR[aQ-2]=new ad(aR[aQ-1].x,aR[aQ-2].y);
}else{aR[aQ-2]=new ad(aR[aQ-2].x,aR[aQ-1].y);
}return[aR[1],aR[2]];
},_startHorizontal:function(aO,aL,aN){var aM;
if(aN!==null&&(aN===al||aN===T)){aM=true;
}else{aM=Math.abs(aO.x-aL.x)>Math.abs(aO.y-aL.y);
}return aM;
}});
var d=j.extend({init:function(aL,aM){var aN=this;
aN.diagram=aL;
aN.options=v({},aN.options,aM);
aN.visual=new I();
aN.diagram._adorners.push(aN);
},refresh:function(){}});
var l=d.extend({init:function(aL,aN){var aP=this,aM;
aP.connection=aL;
...
aP.spVisual=new i(v(aP.options.handles,{center:aO}));
aP.epVisual=new i(v(aP.options.handles,{center:aQ}));
...
}else{if(aL&&aL instanceof B.Shape){aN=aL.getConnector(e)||aL.getConnector(aM);
}else{aN=aM;
...
var aN=this.options.handles.width/2+K;
var aO=this.options.handles.height/2+K;
var aR=aQ.distanceTo(aM);
var aU=aT.distanceTo(aM);
var aP=new ai(aQ.x,aQ.y).inflate(aN,aO).contains(aM);
var aS=new ai(aT.x,aT.y).inflate(aN,aO).contains(aM);
var aL=0;
if(aP&&(!aS||aR<aU)){aL=-1;
...
var r=d.extend({init:function(aL,aM){var aN=this;
...
aP._visible=true;
aP.shape=aO;
aP.diagram.bind(P,aP._refreshHandler);
aN=aO.connectors.length;
aP.connectors=[];
aP._clearVisual();
for(aM=0;
aM<aN;
aM++){aL=new s(aO.connectors[aM]);
aP.connectors.push(aL);
aP.visual.append(aL.visual);
}aP.visual.visible(true);
aP.refresh();
},_clearVisual:function(){var aL=this;
if(aL.diagram._cachedTouchTarget){aL._keepCachedTouchTarget();
}else{aL.visual.clear();
}},_keepCachedTouchTarget:function(){var aO=this,aP=aO.visual.children;
var aL=aP.length;
var aN=N(aO.diagram._cachedTouchTarget,aP);
for(var aM=aL-1;
aM>=0;
aM--){if(aM==aN){continue;
}aO.visual.remove(aP[aM]);
}},destroy:function(){var aL=this;
aL.diagram.unbind(P,aL._refreshHandler);
aL.shape=aE;
aL._visible=aE;
...
}var ak=d.extend({init:function(aL,aM){var aN=this;
d.fn.init.call(aN,aL,aM);
...
aN.diagram.bind(P,aN._refreshHandler).bind(Q,aN._rotatedHandler);
aN.refreshBounds();
aN.refresh();
},options:{handles:{fill:{color:"#fff"},stroke:{color:"#282828"},height:7,width:7,hover:{fill:{color:"#282828"},stroke:{color:"#282828"}}},selectable:{stroke:{color:"#778899",width:1,dashType:"dash"},fill:{color:aD}},offset:10},_initSelection:function(){var aO=this;
var aL=aO.diagram;
var aN=aL.options.selectable;
var aM=v({},aO.options.selectable,aN);
aO.rect=new aj(aM);
...
aN++){for(aO=-1;
aO<=1;
aO++){if(aN!==0||aO!==0){aM=new aj(aL);
aM.drawingElement._hover=ah(this._hover,this);
...
}}}if(this._bounds.contains(aR)){return new ad(0,0);
}},_getHandleBounds:function(aN){if(this._resizable()){var aM=this._handleOptions(),aP=aM.width,aL=aM.height,aO=new ai(0,0,aP,aL);
if(aN.x<0){aO.x=-aP/2;
...
if(aL){aL=360-aL;
aM.rotate(new ad(0,0),aL);
aM=new ad(Math.round(aM.x),Math.round(aM.y));
...
aL<aN.length;
aL++){aM=aN[aL];
if(aM instanceof B.Shape){aO.shapes.push(aM);
aM._rotationOffset=new ad();
...
if(aQ&&aH.isDefined(aO.stroke)){aP=v({},aP,aO.stroke);
...
}},angle:function(aL){if(y(aL)){this._angle=aL;
}return this._angle;
...
},move:function(aT,aY){var aQ,aR,aS=new ad(),aP=new ad(),aM,aN,a1,aU,aL,aV,aO=0,a2,aZ,a0;
...
this._lp=new ad(this._lp.x+a3.x,this._lp.y+a3.y);
}else{aQ=aY.minus(this._cp);
}if(this.isDragHandle(aT)){aP=aS=aQ;
aR=true;
}else{if(this._angle){aQ.rotate(new ad(0,0),this._angle);
}if(aT.x==-1){aS.x=aQ.x;
}else{if(aT.x==1){aP.x=aQ.x;
}}if(aT.y==-1){aS.y=aQ.y;
}else{if(aT.y==1){aP.y=aQ.y;
}}}if(!aR){a2=M(aT,this._innerBounds);
...
aV=new ai(aW.x-aV.width/2,aW.y-aV.height/2,aV.width,aV.height);
...
var aN=Math.max(aM.angle||w,V);
return aM?Math.floor(aL%360/aN)*aN:aL%360;
},_truncateDistance:function(aL){if(aL instanceof B.Point){return new B.Point(this._truncateDistance(aL.x),this._truncateDistance(aL.y));
}else{var aM=this.snapOptions()||{};
var aN=Math.max(aM.size||x,W);
...
},_displaceBounds:function(aL,aP,aN,aO){var aS=aL.topLeft().plus(aP),aM=aL.bottomRight().plus(aN),aQ=ai.fromPoints(aS,aM),aR;
if(!aO){aR=aQ.center();
aR.rotate(aL.center(),-this._angle);
aQ=new ai(aR.x-aQ.width/2,aR.y-aQ.height/2,aQ.width,aQ.height);
}return aQ;
},stop:function(){var aO,aM,aN;
if(this._cp!=this._sp){if(this._rotating){aO=new an(this,this.shapes,this.initialRotates);
...
aM++){aN=this.shapes[aM];
aN.updateModel();
}aO=new aC(this.shapes,this.shapeStates,this);
...
a.each(this.map,function(){aL=aO._getHandleBounds(new ad(this.x,this.y));
this.visual.position(aL.topLeft());
});
this.visual.position(aM.topLeft());
var aN=new ad(aM.width/2,aM.height/2);
...
this._rotationThumbBounds=new ai(aM.center().x,aM.y+aP.y,0,0).inflate(aP.width);
this.rotationThumb.redraw({x:aM.width/2-aP.width/2});
}}else{this.visual.visible(false);
}}});
var at=j.extend({init:function(aL){var aM=aL.options.selectable;
this.options=v({},this.options,aM);
this.visual=new aj(this.options);
this.diagram=aL;
},options:{stroke:{color:"#778899",width:1,dashType:"dash"},fill:{color:aD}},start:function(aL){this._sp=this._ep=aL;
this.refresh();
...
this.refresh();
},refresh:function(){if(this._sp){var aL=ai.fromPoints(this.diagram.modelToLayer(this._sp),this.diagram.modelToLayer(this._ep));
this.bounds(ai.fromPoints(this._sp,this._ep));
this.visual.position(aL.topLeft());
this.visual.redraw({height:aL.height+1,width:aL.width+1});
}}});
var s=j.extend({init:function(aL){this.options=v({},aL.options);
this._c=aL;
this.visual=new i(this.options);
this.refresh();
},_hover:function(aP){var aN=this.options,aM=aN.hover,aO=aN.stroke,aL=aN.fill;
if(aP&&aH.isDefined(aM.stroke)){aO=v({},aO,aM.stroke);
}if(aP&&aH.isDefined(aM.fill)){aL=aM.fill;
}this.visual.redraw({stroke:aO,fill:aL});
},refresh:function(){var aL=this._c.shape.diagram.modelToView(this._c.position()),aM=aL.minus(this._c.shape.bounds("transformed").topLeft()),aN=new ai(aL.x,aL.y,0,0);
aN.inflate(this.options.width/2,this.options.height/2);
this._visualBounds=aN;
this.visual.redraw({center:new ad(aM.x,aM.y)});
},_hitTest:function(aL){var aM=this._c.shape.diagram.modelToView(aL);
return this._visualBounds.contains(aM);
}});
function g(aM){var aL=aM.options.editable;
return aL&&aL.drag!==false;
}function L(aQ,aN){var aL,aO,aP;
for(var aM=0;
...
}}}function aa(aL){return aL.ctrlKey===false&&aL.altKey===false&&aL.shiftKey===false;
}v(B,{CompositeUnit:k,TransformUnit:aC,PanUndoUnit:ac,AddShapeUnit:c,AddConnectionUnit:b,DeleteShapeUnit:A,DeleteConnectionUnit:z,ConnectionEditAdorner:l,ConnectionTool:q,ConnectorVisual:s,UndoRedoService:aF,ResizingAdorner:ak,Selector:at,ToolService:aA,ConnectorsAdorner:r,LayoutUndoUnit:S,ConnectionEditUnit:o,ToFrontUnit:az,ToBackUnit:ay,ConnectionRouterBase:p,PolylineRouter:af,CascadingRouter:h,SelectionTool:ar,ScrollerTool:aq,PointerTool:ae,ConnectionEditTool:m,RotateUnit:an});
}(window.kendo.jQuery));
},typeof define=="function"&&define.amd?define:function(a,b,c){(c||b)();
}));
...
this.trigger("click",{item:bn,point:bp,meta:this._meta(Y.event)});
