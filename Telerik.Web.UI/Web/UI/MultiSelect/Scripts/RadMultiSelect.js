...
return'{"enabled":'+w.get_enabled()+',"selectedItems":'+(JSON.stringify(w._log.selectedItems)||"[]")+',"deselectedItems":'+(JSON.stringify(w._log.deselectedItems)||"[]")+',"value":'+JSON.stringify(w.get_value())+',"selectedDataItems":'+JSON.stringify(w.get_selectedDataItems().map(function(x){return x.toJSON();
}))+"}";
...
return w;
}a.registerControlProperties(c.RadMultiSelect,{skin:"",enabled:true,_skin:"",clientDataSourceID:"",_webServiceClientDataSource:"",itemsData:[],attributes:{},_pendingPostbackCommandArgs:null,_uniqueId:null});
