...
var g;
var j=this.get_templated()||(this.get_hasContentTemplate()&&this.get_expanded());
var i=(j&&!this.get_hasContentTemplate())||(j&&this.get_hasContentTemplate()&&this.get_expanded());
if(i&&this.get_focused()){g=a(this.get_element()).find(".rpTemplate");
var k="a,button,input:not([type=hidden]),textarea,select,details,[tabindex]";
f=g.find(k);
f=f.filter(function(l,e){return e.tabIndex>-1;
});
f=f.filter(function(l,e){return !e.hasAttribute("disabled");
});
f=f.filter(function(l,e){return a(e).is(":visible");
});
if(f.length>0){f.eq(0).focus();
h.preventDefault();
return false;
}}return true;
