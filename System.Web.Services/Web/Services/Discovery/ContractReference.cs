...
namespace System.Web.Services.Discovery
{
	...
	public class ContractReference : DiscoveryReference
	{
		...
		protected internal override void Resolve(string contentType, Stream stream)
		{
			...
			{
				string relUrl = (string)obj3;
				string text = DiscoveryReference.UriToString(this.Url, relUrl);
				if (base.ClientProtocol.Documents[text] == null)
				{
					string url = text;
					try
					{
						stream = base.ClientProtocol.Download(ref text, ref contentType);
						try
						{
							if (base.ClientProtocol.Documents[text] == null)
							{
								...
								if (ServiceDescription.CanRead(xmlTextReader))
								{
									...
									serviceDescription2.RetrievalUrl = text;
									base.ClientProtocol.Documents[text] = serviceDescription2;
									ContractReference contractReference = new ContractReference(text, null);
								...
								{
									base.ClientProtocol.Documents[text] = XmlSchema.Read(xmlTextReader, null);
					...
					catch (Exception ex3)
					{
						...
						throw new InvalidDocumentContentsException(Res.GetString("TheWSDLDocumentContainsLinksThatCouldNotBeResolved", new object[]
						{
							text
						}), ex3);
					}
