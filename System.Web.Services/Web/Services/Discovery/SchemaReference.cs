...
namespace System.Web.Services.Discovery
{
	...
	public sealed class SchemaReference : DiscoveryReference
	{
		...
		protected internal override void Resolve(string contentType, Stream stream)
		{
			...
			{
				base.ClientProtocol.Errors[this.Url] = new InvalidContentTypeException(Res.GetString("WebInvalidContentType", new object[]
				{
					contentType
				}), contentType);
			}
			...
			foreach (XmlSchemaObject xmlSchemaObject in xmlSchema.Includes)
			{
				...
				catch (Exception ex)
				{
					...
					throw new InvalidDocumentContentsException(Res.GetString("TheSchemaDocumentContainsLinksThatCouldNotBeResolved", new object[]
					{
						text
					}), ex);
				}
