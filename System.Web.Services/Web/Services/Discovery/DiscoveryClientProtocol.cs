	...
	public class DiscoveryClientProtocol : HttpWebClientProtocol
	{
		...
		public DiscoveryDocument DiscoverAny(string url)
		{
			...
			if (discoveryReference == null)
			{
				...
				if (flag2)
				{
					throw new InvalidOperationException(Res.GetString("TheDocumentWasNotRecognizedAsAKnownDocumentType", new object[]
					{
						arrayList[0]
					}));
				}
				StringBuilder stringBuilder2 = new StringBuilder(Res.GetString("WebMissingResource", new object[]
				{
					url
				}));
				foreach (object obj in this.Errors)
				{
					...
					stringBuilder2.Append("\n- ").Append(Res.GetString("WebDiscoRefReport", new object[]
					{
						text,
						ex5.Message
					}));
		...
		public Stream Download(ref string url, ref string contentType)
		{
			...
			catch (Exception ex)
			{
				...
				throw new WebException(Res.GetString("ThereWasAnErrorDownloading0", new object[]
				{
					url
				}), ex);
			}
			...
			if (httpWebResponse != null && httpWebResponse.StatusCode != HttpStatusCode.OK)
			{
				...
				{
					url
				}), new WebException(message, null, WebExceptionStatus.ProtocolError, webResponse));
			}
			...
			try
			{
				...
				if (webResponse.ResponseUri.Scheme == Uri.UriSchemeFtp || webResponse.ResponseUri.Scheme == Uri.UriSchemeFile)
				{
					...
					string a;
					if (num != -1 && (a = webResponse.ResponseUri.AbsolutePath.Substring(num + 1).ToLower(CultureInfo.InvariantCulture)) != null && (a == "xml" || a == "wsdl" || a == "xsd" || a == "disco"))
					{
						contentType = "text/xml";
					}
				}
				result = RequestResponseUtils.StreamToMemoryStream(responseStream);
			}
			finally
			{
		...
		private static string GetUniqueFilename(Hashtable filenames, string path)
		{
			if (DiscoveryClientProtocol.IsFilenameInUse(filenames, path))
			{
				...
				string str = path.Substring(0, path.Length - extension.Length);
				int num = 0;
				do
				{
					path = str + num.ToString(CultureInfo.InvariantCulture) + extension;
					num++;
				}
		...
		public DiscoveryClientResultCollection ReadAll(string topLevelFilename)
		{
			...
			try
			{
				...
				for (int i = 0; i < discoveryClientResultsFile.Results.Count; i++)
				{
					...
					{
						throw new InvalidOperationException(Res.GetString("WebRefInvalidAttribute", new object[]
						{
							"referenceType"
						}));
					}
		...
		public DiscoveryClientResultCollection WriteAll(string directory, string topLevelFilename)
		{
			...
			foreach (DictionaryEntry dictionaryEntry3 in array)
			{
				...
				if (value != null)
				{
					DiscoveryReference discoveryReference = this.References[url];
					string text2 = (discoveryReference == null) ? DiscoveryReference.FilenameFromUrl(base.Url) : discoveryReference.DefaultFilename;
					text2 = DiscoveryClientProtocol.GetUniqueFilename(filenames, Path.GetFullPath(Path.Combine(directory, text2)));
					discoveryClientResultsFile.Results.Add(new DiscoveryClientResult((discoveryReference == null) ? null : discoveryReference.GetType(), url, DiscoveryClientProtocol.GetRelativePath(text2, text)));
