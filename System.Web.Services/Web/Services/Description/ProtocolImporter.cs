...
namespace System.Web.Services.Description
{
	...
	public abstract class ProtocolImporter
	{
			...
			try
			{
				...
				while (i < this.portType.Operations.Count)
				{
					...
					catch (Exception ex)
					{
						...
						throw new InvalidOperationException(Res.GetString("UnableToImportOperation1", new object[]
						{
							this.operation.Name
						}), ex);
					}
				...
				if (flag && num > 0 && this.Style == ServiceDescriptionImportStyle.Client)
				{
					...
					CodeMemberMethod codeMemberMethod2 = WebCodeGenerator.AddMethod(this.CodeTypeDeclaration, text2, new CodeFlags[1], new string[]
					{
						typeof(object).FullName
					}, new string[]
					{
						"userState"
					}, typeof(void).FullName, metadata, CodeFlags.IsPublic | ((text != text2) ? ((CodeFlags)0) : CodeFlags.IsNew));
				...
				if (!extensions.IsHandled(obj))
				{
					...
					if (text != null)
					{
						if (extensions.IsRequired(obj))
						{
							...
							ProtocolImporter.AddWarningComment(comments, Res.GetString("WebServiceDescriptionIgnoredRequired", new object[]
							{
								text,
								text2
							}));
						...
						else
						{
							...
							ProtocolImporter.AddWarningComment(comments, Res.GetString("WebServiceDescriptionIgnoredOptional", new object[]
							{
								text,
								text2
							}));
		...
		{
			comments.Add(new CodeCommentStatement(Res.GetString("CodegenWarningDetails", new object[]
			{
				text
			})));
		}
		...
		protected virtual void EndNamespace()
		{
			if (this.classes != null)
			{
				foreach (object obj in this.classes)
				{
					...
					this.codeNamespace.Types.Add(value);
				}
			}
