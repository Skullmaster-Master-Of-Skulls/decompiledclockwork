...
namespace System.Web.Services.Description
{
	...
	public class ServiceDescriptionImporter
	{
		...
		private ProtocolImporter FindImporterByName(string protocolName)
		{
			...
			throw new ArgumentException(Res.GetString("ProtocolWithNameIsNotRecognized1", new object[]
			{
				protocolName
			}), "protocolName");
		}
		...
		public static StringCollection GenerateWebReferences(WebReferenceCollection webReferences, CodeDomProvider codeProvider, CodeCompileUnit codeCompileUnit, WebReferenceOptions options)
		{
			...
			foreach (object obj in webReferences)
			{
				...
				foreach (string typeName in options.SchemaImporterExtensions)
				{
					serviceDescriptionImporter.Extensions.Add(Type.GetType(typeName, true));
				}
			...
			if (options.Verbose)
			{
				foreach (object obj4 in hashtable.Values)
				{
					...
					foreach (string value in importContext2.Warnings)
					{
		...
		private ServiceDescriptionImportWarnings Import(CodeNamespace codeNamespace, ImportContext importContext, Hashtable exportContext, StringCollection warnings)
		{
			...
			foreach (object obj2 in this.serviceDescriptions)
			{
				...
				{
					XmlSchema schema2 = (XmlSchema)obj3;
			...
			foreach (object obj4 in this.serviceDescriptions)
			{
				...
				foreach (object obj5 in serviceDescription2.Messages)
				{
					...
					foreach (object obj6 in message.Parts)
					{
						...
						bool flag;
						bool isLiteral;
						this.FindUse(messagePart, out flag, out isLiteral);
						if (messagePart.Element != null && !messagePart.Element.IsEmpty)
						{
							if (flag)
							{
								throw new InvalidOperationException(Res.GetString("CanTSpecifyElementOnEncodedMessagePartsPart", new object[]
								{
									messagePart.Name,
									message.Name
								}));
							...
							{
								ServiceDescriptionImporter.AddSchema(xmlSchemaElement.Parent as XmlSchema, flag, isLiteral, this.abstractSchemas, this.concreteSchemas, hashtable);
								if (xmlSchemaElement.SchemaTypeName != null && !xmlSchemaElement.SchemaTypeName.IsEmpty)
								{
									...
									{
										ServiceDescriptionImporter.AddSchema(xmlSchemaType.Parent as XmlSchema, flag, isLiteral, this.abstractSchemas, this.concreteSchemas, hashtable);
									}
								}
							}
						}
						if (messagePart.Type != null && !messagePart.Type.IsEmpty)
						{
							...
							{
								ServiceDescriptionImporter.AddSchema(xmlSchemaType2.Parent as XmlSchema, flag, isLiteral, this.abstractSchemas, this.concreteSchemas, hashtable);
							}
						}
					}
				}
			...
			{
				this.abstractSchemas,
				this.concreteSchemas
			})
			{
				hashtable2 = new Hashtable();
				foreach (object obj7 in xmlSchemas)
				{
					XmlSchema schema3 = (XmlSchema)obj7;
					this.AddImport(schema3, hashtable2);
				}
				foreach (object obj8 in hashtable2.Keys)
				{
					XmlSchema xmlSchema = (XmlSchema)obj8;
					if (hashtable[xmlSchema] == null && !xmlSchemas.Contains(xmlSchema))
					{
			...
			foreach (object obj9 in this.allSchemas)
			{
				...
				if (!this.abstractSchemas.Contains(schema4) && !this.concreteSchemas.Contains(schema4))
				{
					this.AddImport(schema4, hashtable2);
				}
			}
			foreach (object obj10 in hashtable2.Keys)
			{
				XmlSchema xmlSchema2 = (XmlSchema)obj10;
				if (hashtable[xmlSchema2] == null)
				{
					if (!this.abstractSchemas.Contains(xmlSchema2))
					{
						this.abstractSchemas.Add(xmlSchema2);
					}
					if (!this.concreteSchemas.Contains(xmlSchema2))
					{
						this.concreteSchemas.Add(xmlSchema2);
					}
				}
			}
			if (this.abstractSchemas.Count > 0)
			{
				...
				{
					XmlSchema schema5 = (XmlSchema)obj11;
					this.abstractSchemas.AddReference(schema5);
				}
			...
			if (this.concreteSchemas.Count > 0)
			{
				...
				{
					XmlSchema schema6 = (XmlSchema)obj12;
					this.concreteSchemas.AddReference(schema6);
				}
				StringCollection stringCollection2 = SchemaCompiler.Compile(this.concreteSchemas);
				foreach (string value2 in stringCollection2)
				{
					warnings.Add(value2);
				}
			}
