	...
	public class SoapProtocolImporter : ProtocolImporter
	{
		...
		protected override void EndNamespace()
		{
			...
			foreach (object obj4 in this.codeClasses)
			{
				...
				{
					CodeAttributeDeclaration value = (CodeAttributeDeclaration)obj5;
					codeTypeDeclaration.CustomAttributes.Add(value);
				}
				foreach (object obj6 in this.soapExporter.IncludeMetadata)
				{
					CodeAttributeDeclaration value2 = (CodeAttributeDeclaration)obj6;
					codeTypeDeclaration.CustomAttributes.Add(value2);
				}
			}
			foreach (object obj7 in base.ExtraCodeClasses)
			{
				CodeTypeDeclaration value3 = (CodeTypeDeclaration)obj7;
				base.CodeNamespace.Types.Add(value3);
			}
		...
		protected override bool IsBindingSupported()
		{
			...
			{
				base.UnsupportedBindingWarning(Res.GetString("ThereIsNoSoapTransportImporterThatUnderstands1", new object[]
				{
		...
		private void EndMetadata(CodeAttributeDeclarationCollection metadata, Type attributeType, string parameter)
		{
			...
			WebCodeGenerator.AddCustomAttribute(metadata, attributeType, parameters, (string[])this.propertyNames.ToArray(typeof(string)), (CodeExpression[])this.propertyValues.ToArray(typeof(CodeExpression)));
		}
			...
			while (i < 2)
			{
				...
				foreach (SoapHeaderBinding soapHeaderBinding in array)
				{
					if (soapHeaderBinding.MapToProperty)
					{
						...
						{
							throw new InvalidOperationException(Res.GetString("WebUnknownEncodingStyle", new object[]
							{
						...
						{
							GlobalSoapHeader globalSoapHeader = (GlobalSoapHeader)this.classHeaders[key];
							if (globalSoapHeader == null)
							{
								...
								this.classHeaders.Add(key, globalSoapHeader);
								if (this.headers[key] == null)
								{
									this.headers.Add(key, globalSoapHeader);
								}
							}
							hashtable.Add(key, new LocalSoapHeader
							{
		...
		protected override CodeMemberMethod GenerateMethod()
		{
			...
			this.GenerateHeaders(metadata, soapBodyBinding.Use, soapBindingStyle == SoapBindingStyle.Rpc, input, messageBinding);
			...
			if (message != null)
			{
				parts = this.GetMessageParts(message, soapBodyBinding2);
				bool flag2;
				if (!this.CheckMessageStyles(base.MethodName, parts, soapBodyBinding2, soapBindingStyle, out flag2))
				{
					return null;
				}
				if (flag != flag2)
			...
			if (message != null)
			{
				xmlMembersMapping2 = this.ImportMessage(text, parts, soapBodyBinding2, soapBindingStyle, flag);
			...
			foreach (object obj in soapParameters.Parameters)
			{
				...
				{
					base.UnsupportedOperationWarning(Res.GetString("CodeGenSupportReferenceParameters", new object[]
					{
						base.ServiceImporter.CodeGenerator.GetType().Name
					}));
					return null;
				}
			...
			if (soapBindingStyle == SoapBindingStyle.Rpc)
			{
				...
				this.EndMetadata(metadata, typeof(SoapRpcMethodAttribute), soapOperationBinding.SoapAction);
			}
			else
			{
				this.AddMetadataProperty("Use", new CodeFieldReferenceExpression(new CodeTypeReferenceExpression(typeof(SoapBindingUse).FullName), Enum.Format(typeof(SoapBindingUse), soapBodyBinding.Use, "G")));
				...
				this.EndMetadata(metadata, typeof(SoapDocumentMethodAttribute), soapOperationBinding.SoapAction);
			}
			...
			foreach (object obj2 in soapParameters.Parameters)
			{
				...
				if (soapBodyBinding.Use == SoapBindingUse.Encoded)
				{
					this.soapExporter.AddMappingMetadata(array[num], soapParameter2.mapping, soapParameter2.name != soapParameter2.mapping.MemberName);
				}
				else
				{
					string ns = (soapBindingStyle == SoapBindingStyle.Rpc) ? soapParameter2.mapping.Namespace : (soapParameter2.IsOut ? xmlMembersMapping2.Namespace : xmlMembersMapping.Namespace);
					bool forceUseMemberName = soapParameter2.name != soapParameter2.mapping.MemberName;
					this.xmlExporter.AddMappingMetadata(array[num], soapParameter2.mapping, ns, forceUseMemberName);
					if (soapParameter2.mapping.CheckSpecified)
					{
						num++;
						array[num] = new CodeAttributeDeclarationCollection();
						this.xmlExporter.AddMappingMetadata(array[num], soapParameter2.mapping, ns, soapParameter2.specifiedName != soapParameter2.mapping.MemberName + "Specified");
						array[num].Add(value);
					}
				}
				if (array[num].Count > 0 && !base.ServiceImporter.CodeGenerator.Supports(GeneratorSupport.ParameterAttributes))
				{
					base.UnsupportedOperationWarning(Res.GetString("CodeGenSupportParameterAttributes", new object[]
					{
						base.ServiceImporter.CodeGenerator.GetType().Name
					}));
					return null;
			...
			if (soapParameters.Return != null)
			{
				...
				{
					base.UnsupportedOperationWarning(Res.GetString("CodeGenSupportReturnTypeAttributes", new object[]
					{
						base.ServiceImporter.CodeGenerator.GetType().Name
					}));
					return null;
			...
			if (base.Style == ServiceDescriptionImportStyle.Client)
			{
				bool flag5 = (base.ServiceImporter.CodeGenerationOptions & CodeGenerationOptions.GenerateOldAsync) != CodeGenerationOptions.None;
				bool flag6 = (base.ServiceImporter.CodeGenerationOptions & CodeGenerationOptions.GenerateNewAsync) != CodeGenerationOptions.None && base.ServiceImporter.CodeGenerator.Supports(GeneratorSupport.DeclareEvents) && base.ServiceImporter.CodeGenerator.Supports(GeneratorSupport.DeclareDelegates);
				CodeExpression[] array2 = new CodeExpression[2];
				this.CreateInvokeParams(array2, text4, soapParameters.InParameters, soapParameters.InCheckSpecifiedCount);
				CodeMethodInvokeExpression codeMethodInvokeExpression = new CodeMethodInvokeExpression(new CodeThisReferenceExpression(), "Invoke", array2);
				this.WriteReturnMappings(codeMemberMethod, codeMethodInvokeExpression, soapParameters, resultsName);
				if (flag5)
				{
					...
					CodeMemberMethod codeMemberMethod2 = WebCodeGenerator.AddMethod(base.CodeTypeDeclaration, "Begin" + text4, parameterFlags, array3, array4, typeof(IAsyncResult).FullName, null, CodeFlags.IsPublic);
					...
					codeMethodInvokeExpression = new CodeMethodInvokeExpression(new CodeThisReferenceExpression(), "BeginInvoke", array2);
					...
					CodeMemberMethod codeMemberMethod3 = WebCodeGenerator.AddMethod(base.CodeTypeDeclaration, "End" + text4, array7, array5, array6, (soapParameters.Return == null) ? typeof(void).FullName : WebCodeGenerator.FullTypeName(soapParameters.Return, base.ServiceImporter.CodeGenerator), null, CodeFlags.IsPublic);
					...
					this.WriteReturnMappings(codeMemberMethod3, codeMethodInvokeExpression, soapParameters, resultsName);
				}
				if (flag6)
				{
					string key = ProtocolImporter.MethodSignature(text4, text5, codeFlags, typeFullNames);
					DelegateInfo delegateInfo = (DelegateInfo)base.ExportContext[key];
					if (delegateInfo == null)
					{
						string handlerType = base.ClassNames.AddUnique(text4 + "CompletedEventHandler", text4);
						string handlerArgs = base.ClassNames.AddUnique(text4 + "CompletedEventArgs", text4);
						delegateInfo = new DelegateInfo(handlerType, handlerArgs);
					}
					string handlerName = base.MethodNames.AddUnique(text4 + "Completed", text4);
					string methodName = base.MethodNames.AddUnique(text4 + "Async", text4);
					string text6 = base.MethodNames.AddUnique(text4 + "OperationCompleted", text4);
					string callbackName = base.MethodNames.AddUnique("On" + text4 + "OperationCompleted", text4);
					...
					string text7 = ProtocolImporter.UniqueName("userState", names);
					CodeMemberMethod codeMemberMethod4 = WebCodeGenerator.AddAsyncMethod(base.CodeTypeDeclaration, methodName, SoapParameter.GetTypeFullNames(soapParameters.InParameters, soapParameters.InCheckSpecifiedCount, base.ServiceImporter.CodeGenerator), names, text6, callbackName, text7);
					array2 = new CodeExpression[4];
					this.CreateInvokeParams(array2, text4, soapParameters.InParameters, soapParameters.InCheckSpecifiedCount);
					array2[2] = new CodeFieldReferenceExpression(new CodeThisReferenceExpression(), text6);
					array2[3] = new CodeArgumentReferenceExpression(text7);
					codeMethodInvokeExpression = new CodeMethodInvokeExpression(new CodeThisReferenceExpression(), "InvokeAsync", array2);
					codeMemberMethod4.Statements.Add(codeMethodInvokeExpression);
					bool flag7 = soapParameters.Return != null || soapParameters.OutParameters.Count > 0;
					WebCodeGenerator.AddCallbackImplementation(base.CodeTypeDeclaration, callbackName, handlerName, delegateInfo.handlerArgs, flag7);
					if (base.ExportContext[key] == null)
					{
						WebCodeGenerator.AddDelegate(base.ExtraCodeClasses, delegateInfo.handlerType, flag7 ? delegateInfo.handlerArgs : typeof(AsyncCompletedEventArgs).FullName);
						if (flag7)
						{
		...
		private bool CheckMessageStyles(string messageName, MessagePart[] parts, SoapBodyBinding soapBodyBinding, SoapBindingStyle soapBindingStyle, out bool hasWrapper)
		{
			...
			else if (soapBodyBinding.Use == SoapBindingUse.Encoded)
			{
				...
				{
					base.UnsupportedOperationBindingWarning(Res.GetString("TheEncodingIsNotSupported1", new object[]
					{
