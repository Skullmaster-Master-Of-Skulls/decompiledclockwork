	...
	internal class SchemaCompiler
	{
		...
		private static void AddImports(XmlSchemas schemas)
		{
			foreach (object obj in schemas)
			{
				...
				SchemaCompiler.AddImport(schema, "http://schemas.xmlsoap.org/soap/encoding/");
				SchemaCompiler.AddImport(schema, "http://schemas.xmlsoap.org/wsdl/");
			}
		}
		...
		internal static string WarningDetails(XmlSchemaException exception, string message)
		{
			...
			{
				result = SchemaCompiler.GetSchemaItem(xmlSchemaObject, null, message);
			}
			else
			{
				string text = null;
				...
				result = Res.GetString("SchemaSyntaxErrorDetails", new object[]
				{
					text,
					message,
		...
		private static string GetSchemaItem(XmlSchemaObject o, string ns, string details)
		{
			...
			else if (o is XmlSchemaContent)
			{
				...
				@string = Res.GetString("XmlSchemaContentDef", new object[]
				{
					parentName3.Name,
		...
		{
			SchemaCompiler.Warnings.Add(Res.GetString((args.Severity == XmlSeverityType.Error) ? "SchemaValidationError" : "SchemaValidationWarning", new object[]
			{
				SchemaCompiler.WarningDetails(args.Exception, args.Message)
			}));
		}
