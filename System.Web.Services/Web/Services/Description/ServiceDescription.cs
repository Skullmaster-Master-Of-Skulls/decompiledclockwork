...
namespace System.Web.Services.Description
{
	...
	public sealed class ServiceDescription : NamedItem
	{
		...
		public static XmlSerializer Serializer
		{
			get
			{
				if (ServiceDescription.serializer == null)
				{
					...
					WebServicesSection.LoadXmlFormatExtensions(webServicesSection.GetAllFormatExtensionTypes(), overrides, xmlSerializerNamespaces);
					ServiceDescription.namespaces = xmlSerializerNamespaces;
					if (webServicesSection.ServiceDescriptionExtended)
					{
						ServiceDescription.serializer = new XmlSerializer(typeof(ServiceDescription), overrides);
					}
					else
					{
		...
		public static ServiceDescription Read(string fileName, bool validate)
		{
			...
			try
			{
				result = ServiceDescription.Read(streamReader, validate);
			}
			finally
			{
				streamReader.Close();
