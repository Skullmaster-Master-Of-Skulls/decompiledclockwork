...
namespace System.Web.Services.Configuration
{
	...
	public sealed class WebServicesSection : ConfigurationSection
	{
		...
		private static object ClassSyncObject
		{
			get
			{
				if (WebServicesSection.classSyncObject == null)
				{
					...
					Interlocked.CompareExchange(ref WebServicesSection.classSyncObject, value, null);
				}
					...
					lock (WebServicesSection.ClassSyncObject)
					{
						if (this.discoveryDocumentSerializer == null)
						{
							...
							foreach (Type type in this.DiscoveryReferenceTypes)
							{
								...
								{
									throw new InvalidOperationException(Res.GetString("WebMissingCustomAttribute", new object[]
									{
										type.FullName,
										"XmlRoot"
									}));
		...
		private static XmlFormatExtensionPointAttribute GetExtensionPointAttribute(Type type)
		{
			...
			if (customAttributes.Length == 0)
			{
				throw new ArgumentException(Res.GetString("TheSyntaxOfTypeMayNotBeExtended1", new object[]
				{
					type.FullName
				}), "type");
			}
		...
		internal static void LoadXmlFormatExtensions(Type[] extensionTypes, XmlAttributeOverrides overrides, XmlSerializerNamespaces namespaces)
		{
			...
			foreach (Type type in extensionTypes)
			{
				if (hashtable2[type] == null)
				{
					...
					if (customAttributes.Length == 0)
					{
						throw new ArgumentException(Res.GetString("RequiredXmlFormatExtensionAttributeIsMissing1", new object[]
						{
							type.FullName
						}), "extensionTypes");
					}
					XmlFormatExtensionAttribute xmlFormatExtensionAttribute = (XmlFormatExtensionAttribute)customAttributes[0];
					foreach (Type key in xmlFormatExtensionAttribute.ExtensionPoints)
					{
			...
			{
				Type type2 = (Type)obj;
				XmlFormatExtensionPointAttribute extensionPointAttribute = WebServicesSection.GetExtensionPointAttribute(type2);
				XmlAttributes xmlAttributes2 = (XmlAttributes)hashtable[type2];
				if (extensionPointAttribute.AllowElements)
				{
					xmlAttributes2.XmlAnyElements.Add(new XmlAnyElementAttribute());
				}
				overrides.Add(type2, extensionPointAttribute.MemberName, xmlAttributes2);
			}
		}
		...
		private Type[] returnWriterTypes = new Type[]
		{
			typeof(XmlReturnWriter)
		};
