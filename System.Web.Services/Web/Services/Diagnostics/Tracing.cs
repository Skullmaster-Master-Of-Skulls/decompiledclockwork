...
namespace System.Web.Services.Diagnostics
{
	...
	internal static class Tracing
	{
		...
		private static object InternalSyncObject
		{
			get
			{
				if (Tracing.internalSyncObject == null)
				{
					...
					Interlocked.CompareExchange(ref Tracing.internalSyncObject, value, null);
				}
		...
		private static void UnhandledExceptionHandler(object sender, UnhandledExceptionEventArgs args)
		{
			...
			Tracing.ExceptionCatch(TraceEventType.Error, sender, "UnhandledExceptionHandler", e);
		}
		...
		private static void StackTrace(TraceEventType eventType, Exception e)
		{
			...
			{
				Tracing.TraceEvent(eventType, Res.GetString("TraceExceptionDetails", new object[]
				{
					e.ToString()
				}));
			}
		...
		private static string GetHostByAddress(string ipAddress)
		{
			...
			try
			{
				result = Dns.GetHostByAddress(ipAddress).HostName;
			}
			catch
			{
				result = null;
		...
		internal static List<string> Details(HttpRequest request)
		{
			...
			list.Add(Res.GetString("TraceUserHostAddress", new object[]
			{
			...
			{
				list.Add(Res.GetString("TraceUserHostName", new object[]
				{
					text
				}));
			}
			list.Add(Res.GetString("TraceUrl", new object[]
			{
				request.HttpMethod,
				request.Url
			}));
			...
			{
				list.Add(Res.GetString("TraceUrlReferrer", new object[]
				{
					request.UrlReferrer
				}));
			}
		...
		internal static void Enter(string callId, TraceMethod caller, TraceMethod callDetails, List<string> details)
		{
			...
			string text = (callDetails == null) ? Res.GetString("TraceCallEnter", new object[]
			{
				callId,
				caller
			}) : Res.GetString("TraceCallEnterDetails", new object[]
			{
				callId,
				caller,
				callDetails
		...
		internal static void Exit(string callId, TraceMethod caller)
		{
			...
			Tracing.TraceEvent(TraceEventType.Information, Res.GetString("TraceCallExit", new object[]
			{
				callId,
				caller
			}));
		...
		internal static void OnUnknownElement(object sender, XmlElementEventArgs e)
		{
			...
			Tracing.TraceEvent(TraceEventType.Warning, Res.GetString(name, new object[]
			{
				text,
				e.ExpectedElements
			}));
