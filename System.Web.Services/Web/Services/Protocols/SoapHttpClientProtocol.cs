...
namespace System.Web.Services.Protocols
{
	...
	public class SoapHttpClientProtocol : HttpWebClientProtocol
	{
		...
		public void Discover()
		{
			...
			{
				throw new InvalidOperationException(Res.GetString("DiscoveryIsNotPossibleBecauseTypeIsMissing1", new object[]
				{
		...
		protected object[] Invoke(string methodName, object[] parameters)
		{
			...
			try
			{
				...
				try
				{
					...
					result = this.ReadResponse(soapClientMessage, webResponse, stream, false);
				}
		...
		{
			SoapHttpClientProtocol.InvokeAsyncState internalAsyncState = new SoapHttpClientProtocol.InvokeAsyncState(methodName, parameters);
		...
		protected object[] EndInvoke(IAsyncResult asyncResult)
		{
			...
			try
			{
				WebResponse response = base.EndSend(asyncResult, ref obj, ref stream);
				SoapHttpClientProtocol.InvokeAsyncState invokeAsyncState = (SoapHttpClientProtocol.InvokeAsyncState)obj;
				result = this.ReadResponse(invokeAsyncState.Message, response, stream, true);
			}
			catch (XmlException innerException)
			{
				throw new InvalidOperationException(Res.GetString("WebResponseBadXml"), innerException);
		...
		private void InvokeAsyncCallback(IAsyncResult result)
		{
			...
			if (webClientAsyncResult.Request != null)
			{
				...
				try
				{
					WebResponse response = base.EndSend(webClientAsyncResult, ref obj, ref stream);
					SoapHttpClientProtocol.InvokeAsyncState invokeAsyncState = (SoapHttpClientProtocol.InvokeAsyncState)obj;
					parameters = this.ReadResponse(invokeAsyncState.Message, response, stream, true);
				}
			...
			base.OperationCompleted(userToken.UserState, parameters, e, false);
		}
		...
		protected void InvokeAsync(string methodName, object[] parameters, SendOrPostCallback callback, object userState)
		{
			...
			WebClientAsyncResult webClientAsyncResult = new WebClientAsyncResult(this, internalAsyncState, null, new AsyncCallback(this.InvokeAsyncCallback), asyncOperation);
			try
			{
		...
		private SoapClientMessage BeforeSerialize(WebRequest request, string methodName, object[] parameters)
		{
			...
			SoapReflectedExtension[] reflectedExtensions = (SoapReflectedExtension[])SoapHttpClientProtocol.CombineExtensionsHelper(this.clientType.HighPriExtensions, method.extensions, this.clientType.LowPriExtensions, typeof(SoapReflectedExtension));
			object[] extensionInitializers = (object[])SoapHttpClientProtocol.CombineExtensionsHelper(this.clientType.HighPriExtensionInitializers, method.extensionInitializers, this.clientType.LowPriExtensionInitializers, typeof(object));
		...
		private void Serialize(SoapClientMessage message)
		{
			...
			if (Tracing.On)
			{
				Tracing.Enter(Tracing.TraceId("TraceWriteRequest"), caller, new TraceMethod(method.parameterSerializer, "Serialize", new object[]
				{
					writerForMessage,
					parameterValues,
					null,
		...
		private object[] ReadResponse(SoapClientMessage message, WebResponse response, Stream responseStream, bool asyncCall)
		{
			...
			if (!flag || (flag && httpWebResponse != null && httpWebResponse.ContentLength == 0L))
			{
				...
				throw new InvalidOperationException(Res.GetString("WebResponseContent", new object[]
				{
					message.ContentType,
					this.HttpContentType
				}) + Environment.NewLine + RequestResponseUtils.CreateResponseExceptionString(response, responseStream));
			}
			else
			{
				...
				{
					bufferSize = RequestResponseUtils.GetBufferSize((int)httpWebResponse.ContentLength);
				}
				...
				else
				{
					...
					if (Tracing.On)
					{
						Tracing.Enter(Tracing.TraceId("TraceReadResponse"), caller, new TraceMethod(method.returnSerializer, "Deserialize", new object[]
						{
							readerForMessage,
				...
				while (reader.NodeType != XmlNodeType.EndElement && reader.NodeType != XmlNodeType.None)
				{
					if (reader.NamespaceURI == "http://schemas.xmlsoap.org/soap/envelope/" || reader.NamespaceURI == "http://www.w3.org/2003/05/soap-envelope" || reader.NamespaceURI == null || reader.NamespaceURI.Length == 0)
					{
						if (reader.LocalName == "faultcode" || reader.LocalName == "Code")
						{
							if (flag)
							{
								code = this.ReadSoap12FaultCode(reader, out subCode);
							}
							else
							{
		...
		private XmlQualifiedName ReadFaultCode(XmlReader reader)
		{
			...
			if (num >= 0)
			{
				...
				if (text2 == null)
				{
					throw new InvalidOperationException(Res.GetString("WebQNamePrefixUndefined", new object[]
					{
						text3
					}));
				}
