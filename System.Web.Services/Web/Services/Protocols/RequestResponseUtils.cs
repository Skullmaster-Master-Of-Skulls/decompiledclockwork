	...
	internal class RequestResponseUtils
	{
		...
		internal static string CreateResponseExceptionString(WebResponse response, Stream stream)
		{
			if (response is HttpWebResponse)
			{
				...
				{
					return Res.GetString("WebResponseKnownError", new object[]
					{
						statusCode,
		...
		private static class HttpUtility
		{
			...
			internal static string HtmlDecode(string s)
			{
				...
				RequestResponseUtils.HttpUtility.HtmlDecode(s, output);
				return stringBuilder.ToString();
			}
				...
				while (i < length)
				{
					...
					if (text.Length > 1 && text[0] == '#')
					{
						...
						catch (FormatException e)
						{
							...
							if (Tracing.On)
							{
								Tracing.ExceptionCatch(TraceEventType.Warning, typeof(RequestResponseUtils.HttpUtility), "HtmlDecode", e);
							}
							goto IL_14C;
						}
						catch (ArgumentException e2)
						{
							...
							if (Tracing.On)
							{
								Tracing.ExceptionCatch(TraceEventType.Warning, typeof(RequestResponseUtils.HttpUtility), "HtmlDecode", e2);
							}
							goto IL_14C;
						}
					}
