...
namespace System.Web.Services.Protocols
{
	...
	internal class WebServiceHandler
	{
		...
		private void Invoke()
		{
			...
			string stringBuffer;
			if (!this.protocol.IsOneWay && RemoteDebugger.IsServerCallInEnabled(this.protocol, out stringBuffer))
			{
				remoteDebugger = new RemoteDebugger();
				remoteDebugger.NotifyServerCallEnter(this.protocol, stringBuffer);
			}
			try
			{
		...
		protected void CoreProcessRequest()
		{
			try
			{
				...
				if (this.protocol.IsOneWay)
				{
					...
					if (Tracing.On)
					{
						Tracing.Information("TracePostWorkItemIn", new object[]
						{
							traceMethod
						});
					}
					...
					if (Tracing.On)
					{
						Tracing.Information("TracePostWorkItemOut", new object[]
						{
							traceMethod
						});
					}
		...
		{
			HttpContext result = HttpContext.Current;
		...
		private void OneWayInvoke()
		{
			...
			{
				this.HandleOneWayException(e, "OneWayInvoke");
			}
			finally
			{
				if (httpContext != null)
		...
		private void OneWayInvokeTransacted()
		{
			...
			{
				this.HandleOneWayException(e, "OneWayInvokeTransacted");
			}
			finally
			{
				if (httpContext != null)
		...
		private void OneWayAsyncInvoke()
		{
			...
			{
				this.HandleOneWayException(e, "OneWayAsyncInvoke");
			}
			finally
			{
				if (httpContext != null)
