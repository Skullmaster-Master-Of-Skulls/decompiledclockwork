...
namespace System.Web.Services.Protocols
{
	...
	internal class RemoteDebugger : INotifySource2
	{
		...
		private static object InternalSyncObject
		{
			get
			{
				if (RemoteDebugger.s_InternalSyncObject == null)
				{
					...
					Interlocked.CompareExchange(ref RemoteDebugger.s_InternalSyncObject, value, null);
				}
		...
		internal static bool IsClientCallOutEnabled()
		{
			...
			try
			{
				result = (!CompModSwitches.DisableRemoteDebugging.Enabled && Debugger.IsAttached && RemoteDebugger.Connection != null);
			}
			catch (Exception ex)
			{
			...
			try
			{
				if (this.NotifySink != null)
				{
					...
					if (!(intPtr == IntPtr.Zero))
					{
						...
						request.Headers.Add(RemoteDebugger.debuggerHeader, value);
					}
				}
			}
			catch (Exception ex)
		...
		internal void NotifyServerCallExit(HttpResponse response)
		{
			try
			{
				...
				string value = Convert.ToBase64String(array);
				response.AddHeader(RemoteDebugger.debuggerHeader, value);
			}
			catch (Exception ex)
			{
				if (ex is ThreadAbortException || ex is StackOverflowException || ex is OutOfMemoryException)
