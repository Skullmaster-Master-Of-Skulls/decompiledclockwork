	...
	public sealed class SoapServerType : ServerType
	{
		...
		public SoapServerType(Type type, WebServiceProtocols protocolsSupported) : base(type)
		{
			...
			for (int j = 0; j < array.Length; j++)
			{
				...
				SoapReflectedMethod soapReflectedMethod = SoapReflector.ReflectMethod(methodInfo, false, xmlReflectionImporter, soapReflectionImporter, attribute.Namespace);
			...
			TraceMethod caller = Tracing.On ? new TraceMethod(this, ".ctor", new object[]
			{
				type,
				protocolsSupported
			}) : null;
			if (Tracing.On)
			{
				Tracing.Enter(Tracing.TraceId("TraceCreateSerializer"), caller, new TraceMethod(typeof(XmlSerializer), "FromMappings", new object[]
				{
					array3,
					base.Evidence
				}));
			...
			for (int k = 0; k < array2.Length; k++)
			{
				...
				{
					throw new SoapException(Res.GetString("TheMethodDoesNotHaveARequestElementEither1", new object[]
					{
