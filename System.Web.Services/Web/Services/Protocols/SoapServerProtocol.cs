	...
	public class SoapServerProtocol : ServerProtocol
	{
		...
		internal override bool Initialize()
		{
			...
			if (ex == null)
			{
				try
				{
					SoapReflectedExtension[] reflectedExtensions = (SoapReflectedExtension[])SoapServerProtocol.CombineExtensionsHelper(this.serverMethod.extensions, this.serverType.LowPriExtensions, typeof(SoapReflectedExtension));
					object[] extensionInitializers = (object[])SoapServerProtocol.CombineExtensionsHelper(this.serverMethod.extensionInitializers, this.serverType.LowPriExtensionInitializers, typeof(object));
		...
		internal override object[] ReadParameters()
		{
			...
			try
			{
				...
				string text = new SoapHeaderHandling().ReadHeaders(xmlReader, this.serverMethod.inHeaderSerializer, this.message.Headers, this.serverMethod.inHeaderMappings, SoapHeaderDirection.In, this.helper.EnvelopeNs, (this.serverMethod.use == SoapBindingUse.Encoded) ? this.helper.EncodingNs : null, checkRequiredHeaders);
				if (text != null)
				{
					throw new SoapHeaderException(Res.GetString("WebMissingHeader", new object[]
					{
						text
					}), new XmlQualifiedName("MustUnderstand", "http://schemas.xmlsoap.org/soap/envelope/"));
				}
				...
				if (Tracing.On)
				{
					Tracing.Enter(Tracing.TraceId("TraceReadRequest"), caller, new TraceMethod(this.serverMethod.parameterSerializer, "Deserialize", new object[]
					{
						xmlReader,
						(this.serverMethod.use == SoapBindingUse.Encoded) ? this.helper.EncodingNs : null
					}));
				}
				bool flag2 = !flag && (WebServicesSection.Current.SoapEnvelopeProcessing.IsStrict || Tracing.On);
				object[] array;
				if (flag2)
				{
					XmlDeserializationEvents events = Tracing.On ? Tracing.GetDeserializationEvents() : RuntimeUtils.GetDeserializationEvents();
					array = (object[])this.serverMethod.parameterSerializer.Deserialize(xmlReader, null, events);
				}
				else
				{
					array = (object[])this.serverMethod.parameterSerializer.Deserialize(xmlReader, flag ? this.helper.EncodingNs : null);
				}
				if (Tracing.On)
				{
		...
		internal override void WriteReturns(object[] returnValues, Stream outputStream)
		{
			...
			if (Tracing.On)
			{
				Tracing.Enter(Tracing.TraceId("TraceWriteResponse"), caller, new TraceMethod(this.serverMethod.returnSerializer, "Serialize", new object[]
				{
					writerForMessage,
					returnValues,
					null,
		...
		internal override bool WriteException(Exception e, Stream outputStream)
		{
			...
			base.Response.Clear();
			HttpStatusCode statusCode = this.helper.SetResponseErrorCode(base.Response, ex);
			bool flag = false;
