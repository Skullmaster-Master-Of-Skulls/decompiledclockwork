...
namespace System.Web.Services.Protocols
{
	...
	public abstract class SoapMessage
	{
		...
		public object GetInParameterValue(int index)
		{
			...
			{
				throw new IndexOutOfRangeException(Res.GetString("indexMustBeBetweenAnd0Inclusive", new object[]
				{
		...
		protected void EnsureStage(SoapMessageStage stage)
		{
			...
			{
				throw new InvalidOperationException(Res.GetString("WebCannotAccessValueStage", new object[]
				{
					this.stage.ToString()
				}));
			}
		...
		internal void RunExtensions(SoapExtension[] extensions, bool throwOnException)
		{
			...
			TraceMethod traceMethod = Tracing.On ? new TraceMethod(this, "RunExtensions", new object[]
			{
				extensions,
				throwOnException
			}) : null;
			if ((this.stage & (SoapMessageStage)12) != (SoapMessageStage)0)
			{
				for (int i = 0; i < extensions.Length; i++)
				{
					if (Tracing.On)
					{
						Tracing.Enter("SoapExtension", traceMethod, new TraceMethod(extensions[i], "ProcessMessage", new object[]
						{
							this.stage
						}));
					}
			...
			for (int j = extensions.Length - 1; j >= 0; j--)
			{
				if (Tracing.On)
				{
					Tracing.Enter("SoapExtension", traceMethod, new TraceMethod(extensions[j], "ProcessMessage", new object[]
					{
						this.stage
					}));
				}
