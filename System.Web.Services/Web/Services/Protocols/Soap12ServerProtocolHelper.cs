	...
	internal class Soap12ServerProtocolHelper : SoapServerProtocolHelper
	{
		...
		internal override SoapServerMethod RouteRequest()
		{
			...
			if (text != null && text.Length > 0)
			{
				...
				if (Tracing.On)
				{
					Tracing.Enter("RouteRequest", caller, new TraceMethod(base.ServerType, "GetMethod", new object[]
					{
						text
					}), Tracing.Details(base.ServerProtocol.Request));
				}
			...
			if (!flag)
			{
				throw new SoapException(Res.GetString("UnableToHandleRequestActionNotRecognized1", new object[]
				{
					text
				}), Soap12FaultCodes.SenderFaultCode);
			}
			if (flag2)
			{
