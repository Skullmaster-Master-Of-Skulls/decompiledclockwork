...
namespace System.Web.Services.Protocols
{
	...
	internal static class SoapReflector
	{
		...
		internal static void IncludeTypes(LogicalMethodInfo[] methods, SoapReflectionImporter importer)
		{
			...
			{
				SoapReflector.IncludeTypes(method, importer);
			}
		}
		...
		internal static string GetSoapMethodBinding(LogicalMethodInfo method)
		{
			...
			if (customAttributes.Length == 0)
			{
				...
				{
					text = ((SoapRpcMethodAttribute)customAttributes[0]).Binding;
				}
			}
			else
			{
		...
		internal static SoapReflectedMethod ReflectMethod(LogicalMethodInfo methodInfo, bool client, XmlReflectionImporter xmlImporter, SoapReflectionImporter soapImporter, string defaultNs)
		{
			...
			try
			{
				...
				bool serviceDefaultIsEncoded = SoapReflector.ServiceDefaultIsEncoded(soapServiceAttribute);
				...
				if (methodAttribute.binding.Length > 0)
				{
					if (client)
					{
						throw new InvalidOperationException(Res.GetString("WebInvalidBindingPlacement", new object[]
						{
							obj.GetType().Name
						}));
					}
				...
				soapReflectedMethod.requestMappings = SoapReflector.ImportMembersMapping(xmlImporter, soapImporter, serviceDefaultIsEncoded, soapReflectedMethod.rpc, soapReflectedMethod.use, soapReflectedMethod.paramStyle, text, text2, methodAttribute.requestNs == null, array3, true, false, key, client);
				...
				if (!soapReflectedMethod.oneWay)
				{
					...
					soapReflectedMethod.responseMappings = SoapReflector.ImportMembersMapping(xmlImporter, soapImporter, serviceDefaultIsEncoded, soapReflectedMethod.rpc, soapReflectedMethod.use, soapReflectedMethod.paramStyle, text3, text4, methodAttribute.responseNs == null, array3, false, false, key + ":Response", !client);
				}
				...
				soapReflectedMethod.inHeaderMappings = SoapReflector.ImportMembersMapping(xmlImporter, soapImporter, serviceDefaultIsEncoded, false, soapReflectedMethod.use, SoapParameterStyle.Bare, text + "InHeaders", defaultNs, true, (XmlReflectionMember[])arrayList.ToArray(typeof(XmlReflectionMember)), false, true, key + ":InHeaders", client);
				if (!soapReflectedMethod.oneWay)
				{
					soapReflectedMethod.outHeaderMappings = SoapReflector.ImportMembersMapping(xmlImporter, soapImporter, serviceDefaultIsEncoded, false, soapReflectedMethod.use, SoapParameterStyle.Bare, text3 + "OutHeaders", defaultNs, true, (XmlReflectionMember[])arrayList2.ToArray(typeof(XmlReflectionMember)), false, true, key + ":OutHeaders", !client);
				}
				result = soapReflectedMethod;
			}
			catch (Exception ex)
			{
		...
		private static XmlMembersMapping ImportMembersMapping(XmlReflectionImporter xmlImporter, SoapReflectionImporter soapImporter, bool serviceDefaultIsEncoded, bool rpc, SoapBindingUse use, SoapParameterStyle paramStyle, string elementName, string elementNamespace, bool nsIsDefault, XmlReflectionMember[] members, bool validate, bool openModel, string key, bool writeAccess)
		{
			...
			{
				string ns = (!rpc && paramStyle != SoapParameterStyle.Bare && nsIsDefault) ? SoapReflector.GetEncodedNamespace(elementNamespace, serviceDefaultIsEncoded) : elementNamespace;
				xmlMembersMapping = soapImporter.ImportMembersMapping(elementName, ns, members, rpc || paramStyle != SoapParameterStyle.Bare, rpc, validate, writeAccess ? XmlMappingAccess.Write : XmlMappingAccess.Read);
			}
			else
			{
				string ns2 = nsIsDefault ? SoapReflector.GetLiteralNamespace(elementNamespace, serviceDefaultIsEncoded) : elementNamespace;
				xmlMembersMapping = xmlImporter.ImportMembersMapping(elementName, ns2, members, paramStyle != SoapParameterStyle.Bare, rpc, openModel, writeAccess ? XmlMappingAccess.Write : XmlMappingAccess.Read);
			}
		...
		{
			return new Exception(Res.GetString(description, new object[]
			{
				declaringType.Name,
				memberName
			}));
