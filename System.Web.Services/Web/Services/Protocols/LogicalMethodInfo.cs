...
namespace System.Web.Services.Protocols
{
	...
	public sealed class LogicalMethodInfo
	{
		...
		internal LogicalMethodInfo(MethodInfo methodInfo, WebMethod webMethod)
		{
			...
			{
				throw new InvalidOperationException(Res.GetString("WebMethodStatic", new object[]
				{
					methodInfo.Name
				}));
			}
		...
		internal static LogicalMethodInfo[] Create(MethodInfo[] methodInfos, LogicalMethodTypes types, Hashtable declarations)
		{
			...
			for (int k = 0; k < num; k++)
			{
				...
				if (methodInfo3 == null)
				{
					...
					{
						methodInfo2.DeclaringType.FullName,
						methodInfo2.Name,
						text
					}));
				}
		...
		internal string GetKey()
		{
			...
			if (text.Length > 1024)
			{
				byte[] inArray = LogicalMethodInfo.HashAlgorithm.ComputeHash(Encoding.UTF8.GetBytes(text));
