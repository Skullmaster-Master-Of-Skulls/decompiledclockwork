		...
		internal DiscoveryServerType(Type type, string uri) : base(typeof(DiscoveryServerProtocol))
		{
			...
			foreach (object obj in serviceDescriptionReflector.ServiceDescription.Services)
			{
				...
				foreach (object obj2 in service.Ports)
				{
					...
					{
						System.Web.Services.Discovery.SoapBinding soapBinding = new System.Web.Services.Discovery.SoapBinding();
			...
			foreach (object obj in schemas)
			{
				...
				if (xmlSchema != null)
				{
					if (xmlSchema.Id == null || xmlSchema.Id.Length == 0)
					{
						...
						int num2;
						num = (num2 = num + 1);
						xmlSchema2.Id = str + num2.ToString(CultureInfo.InvariantCulture);
					}
					string location = uri + "?schema=" + xmlSchema.Id;
					foreach (object obj2 in descriptions)
					{
						...
						if (serviceDescription.Types.Schemas.Count == 0)
						{
							...
							this.AddExternal(xmlSchema3, xmlSchema.TargetNamespace, location);
							serviceDescription.Types.Schemas.Add(xmlSchema3);
						}
						else
						{
							this.AddExternal(serviceDescription.Types.Schemas[0], xmlSchema.TargetNamespace, location);
						}
					}
