		...
		internal static WebServiceBindingAttribute GetAttribute(Type type)
		{
			while (type != null)
			{
				...
				if (customAttributes.Length != 0)
				{
					...
					{
						throw new ArgumentException(Res.GetString("OnlyOneWebServiceBindingAttributeMayBeSpecified1", new object[]
						{
							type.FullName
						}), "type");
					}
			...
			else
			{
				...
				foreach (WebServiceBindingAttribute webServiceBindingAttribute2 in customAttributes)
				{
					if (webServiceBindingAttribute2.Name == binding)
					{
						...
						{
							throw new ArgumentException(Res.GetString("MultipleBindingsWithSameName2", new object[]
							{
				...
				{
					throw new ArgumentException(Res.GetString("TypeIsMissingWebServiceBindingAttributeThat2", new object[]
					{
						declaringType.FullName,
						binding
					}), "methodInfo");
