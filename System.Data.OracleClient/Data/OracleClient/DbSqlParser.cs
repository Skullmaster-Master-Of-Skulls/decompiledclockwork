...
namespace System.Data.OracleClient
{
	...
	{
		// Token: 0x0600009F RID: 159 RVA: 0x000564F4 File Offset: 0x000558F4
		...
		// Token: 0x060000A0 RID: 160 RVA: 0x00056524 File Offset: 0x00055924
		...
		// (get) Token: 0x060000A1 RID: 161 RVA: 0x000565C4 File Offset: 0x000559C4
		...
		// (get) Token: 0x060000A2 RID: 162 RVA: 0x000565F4 File Offset: 0x000559F4
		...
		// (get) Token: 0x060000A3 RID: 163 RVA: 0x00056614 File Offset: 0x00055A14
		...
		// (get) Token: 0x060000A4 RID: 164 RVA: 0x00056634 File Offset: 0x00055A34
		private static Regex SqlTokenParser
		{
			get
			{
				Regex sqlTokenParser = DbSqlParser._sqlTokenParser;
				if (sqlTokenParser == null)
				{
					sqlTokenParser = DbSqlParser.GetSqlTokenParser();
				}
		...
		// (get) Token: 0x060000A5 RID: 165 RVA: 0x00056654 File Offset: 0x00055A54
		...
		// Token: 0x060000A6 RID: 166 RVA: 0x00056684 File Offset: 0x00055A84
		...
		// Token: 0x060000A7 RID: 167 RVA: 0x000566D4 File Offset: 0x00055AD4
		...
		// Token: 0x060000A8 RID: 168 RVA: 0x00056714 File Offset: 0x00055B14
		private void CompleteSchemaInformation()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				DbSqlParserColumnCollection columns2 = this.GatherTableColumns(dbSqlParserTable);
				dbSqlParserTable.Columns = columns2;
			}
			for (int j = 0; j < num; j++)
			{
				...
				if (!dbSqlParserColumn.IsExpression)
				{
					if ("*" == dbSqlParserColumn.ColumnName)
					{
						...
						if (dbSqlParserColumn.TableName.Length != 0)
						{
							...
							int count2 = columns3.Count;
							for (int k = 0; k < count2; k++)
							{
								columns.Insert(j + k, columns3[k]);
							}
							num += count2 - 1;
							j += count2 - 1;
						}
						else
						{
							for (int l = 0; l < count; l++)
							{
								...
								DbSqlParserColumnCollection columns4 = dbSqlParserTable2.Columns;
								int count3 = columns4.Count;
								for (int m = 0; m < count3; m++)
								{
									columns.Insert(j + m, columns4[m]);
								}
								num += count3 - 1;
								j += count3;
							}
			...
			for (int n = 0; n < count; n++)
			{
				...
				this.GatherKeyColumns(table);
			}
		}

		// Token: 0x060000A9 RID: 169 RVA: 0x000568B4 File Offset: 0x00055CB4
		...
		// Token: 0x060000AA RID: 170 RVA: 0x00056904 File Offset: 0x00055D04
		...
		// Token: 0x060000AB RID: 171 RVA: 0x000569D4 File Offset: 0x00055DD4
		...
		// Token: 0x060000AC RID: 172 RVA: 0x00056A04 File Offset: 0x00055E04
		...
		// Token: 0x060000AD RID: 173 RVA: 0x00056A84 File Offset: 0x00055E84
		...
		// Token: 0x060000AE RID: 174 RVA: 0x00056AC4 File Offset: 0x00055EC4
		...
		// Token: 0x060000AF RID: 175 RVA: 0x00056AE4 File Offset: 0x00055EE4
		...
		// Token: 0x060000B0 RID: 176 RVA: 0x00057144 File Offset: 0x00056544
		internal static DbSqlParser.Token TokenFromMatch(Match match)
		{
			...
			if (match.Groups[DbSqlParser._otherGroup].Success)
			{
				...
				DbSqlParser.TokenType type = DbSqlParser.TokenType.Other;
				switch (text[0])
				{
				case '(':
					type = DbSqlParser.TokenType.Other_LeftParen;
					break;
				case ')':
					type = DbSqlParser.TokenType.Other_RightParen;
					break;
				case '*':
					type = DbSqlParser.TokenType.Other_Star;
					break;
				case ',':
					type = DbSqlParser.TokenType.Other_Comma;
					break;
				case '.':
					type = DbSqlParser.TokenType.Other_Period;
					break;
				}
				return new DbSqlParser.Token(type, match.Groups[DbSqlParser._otherGroup].Value);
			}
			if (match.Groups[DbSqlParser._keywordGroup].Success)
			{
				...
				DbSqlParser.TokenType tokenType = DbSqlParser.TokenType.Keyword;
				switch (length)
				{
				case 2:
					if ("as" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_AS;
					}
					else if ("on" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_ON;
					}
					break;
				case 3:
					if ("for" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_FOR;
					}
					else if ("all" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_ALL;
					}
					else if ("top" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_TOP;
					}
					break;
				case 4:
					if ("from" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_FROM;
					}
					else if ("into" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_INTO;
					}
					else if ("join" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_JOIN;
					}
					else if ("left" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_LEFT;
					}
					break;
				case 5:
					if ("where" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_WHERE;
					}
					else if ("group" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_GROUP;
					}
					else if ("order" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_ORDER;
					}
					else if ("right" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_RIGHT;
					}
					else if ("outer" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_OUTER;
					}
					else if ("using" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_USING;
					}
					else if ("cross" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_CROSS;
					}
					else if ("union" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_UNION;
					}
					else if ("minus" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_MINUS;
					}
					else if ("inner" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_INNER;
					}
					break;
				case 6:
					if ("select" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_SELECT;
					}
					else if ("having" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_HAVING;
					}
					break;
				case 7:
					if ("compute" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_COMPUTE;
					}
					else if ("natural" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_NATURAL;
					}
					break;
				case 8:
					if ("distinct" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_DISTINCT;
					}
					break;
				case 9:
					if ("intersect" == text2)
					{
						tokenType = DbSqlParser.TokenType.Keyword_INTERSECT;
					}
					break;
				}
				if (DbSqlParser.TokenType.Keyword != tokenType)
				{
					return new DbSqlParser.Token(tokenType, text2);
				}
			}
			return DbSqlParser.Token.Null;
		}
		...
		internal struct Token
		{
			...
			// (get) Token: 0x060000B4 RID: 180 RVA: 0x00057584 File Offset: 0x00056984
			...
			// (get) Token: 0x060000B5 RID: 181 RVA: 0x000575A4 File Offset: 0x000569A4
			...
			// Token: 0x060000B6 RID: 182 RVA: 0x000575C4 File Offset: 0x000569C4
