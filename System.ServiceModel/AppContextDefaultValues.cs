	...
	internal static class AppContextDefaultValues
	{
		...
		private static void PopulateDefaultValuesPartial(string platformIdentifier, string profile, int version)
		{
			if (platformIdentifier == ".NETCore" || platformIdentifier == ".NETFramework")
			{
				...
				if (version <= 40602)
				{
					LocalAppContext.DefineSwitchDefault("Switch.System.ServiceModel.DisableUsingServicePointManagerSecurityProtocols", true);
				}
				if (version <= 40700)
				{
					LocalAppContext.DefineSwitchDefault("Switch.System.ServiceModel.UseSha1InPipeConnectionGetHashAlgorithm", true);
					LocalAppContext.DefineSwitchDefault("Switch.System.ServiceModel.DisableAddressHeaderCollectionValidation", true);
					LocalAppContext.DefineSwitchDefault("Switch.System.ServiceModel.UseSha1InMsmqEncryptionAlgorithm", true);
					LocalAppContext.DefineSwitchDefault("Switch.System.ServiceModel.DontEnableSystemDefaultTlsVersions", true);
				}
			}
		}
	}
