...
namespace System.DirectoryServices
{
	...
	public class SearchResultCollection : MarshalByRefObject, ICollection, IEnumerable, IDisposable
	{
		...
		internal unsafe byte[] RetrieveDirectorySynchronizationCookie()
		{
			...
			AdsSearchColumn* value = &adsSearchColumn;
			this.SearchObject.GetColumn(this.Handle, this.AdsDirsynCookieName, (IntPtr)((void*)value));
			byte[] result;
			try
			{
			...
			finally
			{
				try
				{
					this.SearchObject.FreeColumn((IntPtr)((void*)value));
				}
				catch (COMException)
				{
				}
		...
		internal unsafe DirectoryVirtualListView RetrieveVLVResponse()
		{
			...
			AdsSearchColumn adsSearchColumn = default(AdsSearchColumn);
			AdsSearchColumn* value = &adsSearchColumn;
			this.SearchObject.GetColumn(this.Handle, this.AdsVLVResponseName, (IntPtr)((void*)value));
			DirectoryVirtualListView result;
			try
			{
				AdsValue* pADsValues = adsSearchColumn.pADsValues;
				DirectoryVirtualListView directoryVirtualListView = (DirectoryVirtualListView)new AdsValueHelper(*pADsValues).GetVlvValue();
				result = directoryVirtualListView;
			}
			finally
			{
				try
		...
		private class ResultsEnumerator : IEnumerator
		{
			...
			private unsafe SearchResult GetCurrentResult()
			{
				...
				for (int nextColumnName = this.results.SearchObject.GetNextColumnName(this.results.Handle, (IntPtr)((void*)(&intPtr))); nextColumnName == 0; nextColumnName = this.results.SearchObject.GetNextColumnName(this.results.Handle, (IntPtr)((void*)(&intPtr))))
				{
					try
					{
						AdsSearchColumn adsSearchColumn = default(AdsSearchColumn);
						AdsSearchColumn* value = &adsSearchColumn;
						this.results.SearchObject.GetColumn(this.results.Handle, intPtr, (IntPtr)((void*)value));
						try
						{
							int dwNumValues = adsSearchColumn.dwNumValues;
							AdsValue* ptr = adsSearchColumn.pADsValues;
							object[] array = new object[dwNumValues];
							for (int i = 0; i < dwNumValues; i++)
							{
								array[i] = new AdsValueHelper(*ptr).GetValue();
								ptr++;
							}
						...
						finally
						{
							try
							{
								this.results.SearchObject.FreeColumn((IntPtr)((void*)value));
							}
							catch (COMException)
							{
							}
			...
			public bool MoveNext()
			{
				...
				if (!this.initialized)
				{
					...
					if (firstRow != 20498)
					{
						...
						{
							throw new ArgumentException(Res.GetString("DSInvalidSearchFilter", new object[]
							{
								this.results.Filter
							}));
						}
