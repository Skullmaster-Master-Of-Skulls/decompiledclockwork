...
namespace System.DirectoryServices
{
	...
	public class DirectorySearcher : System.ComponentModel.Component
	{
		...
		public DirectorySearcher() : this(null, "(objectClass=*)", null, SearchScope.Subtree)
		{
		...
		[DirectoryServicesPermission(SecurityAction.Demand, Unrestricted = true)]
		public DirectorySearcher(DirectoryEntry searchRoot) : this(searchRoot, "(objectClass=*)", null, SearchScope.Subtree)
		{
			this.scopeSpecified = false;
		}
		...
		private SearchResultCollection FindAll(bool findMoreThanOne)
		{
			...
			{
				throw new NotSupportedException(Res.GetString("DSSearchUnsupported", new object[]
				{
			...
			if (array != null)
			{
				directorySearch.ExecuteSearch(this.Filter, array, array.Length, out searchHandle);
			}
			else
			{
				directorySearch.ExecuteSearch(this.Filter, null, -1, out searchHandle);
				array = new string[0];
			}
			SearchResultCollection result = new SearchResultCollection(directoryEntry, searchHandle, array, this);
			...
			try
			{
				...
				for (int j = 0; j < prefs.Length; j++)
				{
					...
					if (num2 != 0)
					{
						...
						throw new InvalidOperationException(Res.GetString("DSSearchPreferencesNotAccepted", new object[]
						{
							text
						}));
					}
