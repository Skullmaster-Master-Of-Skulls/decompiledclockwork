	...
	public class ReplicationCursorCollection : ReadOnlyCollectionBase
	{
		...
		internal void AddHelper(string partition, object cursors, bool advanced, IntPtr info)
		{
			...
			for (int i = 0; i < cNumCursors; i++)
			{
				if (advanced)
				{
					ptr = Utils.AddToIntPtr(info, Marshal.SizeOf(typeof(int)) * 2 + i * Marshal.SizeOf(typeof(DS_REPL_CURSOR_3)));
					DS_REPL_CURSOR_3 ds_REPL_CURSOR_ = new DS_REPL_CURSOR_3();
					Marshal.PtrToStructure(ptr, ds_REPL_CURSOR_);
					ReplicationCursor cursor = new ReplicationCursor(this.server, partition, ds_REPL_CURSOR_.uuidSourceDsaInvocationID, ds_REPL_CURSOR_.usnAttributeFilter, ds_REPL_CURSOR_.ftimeLastSyncSuccess, ds_REPL_CURSOR_.pszSourceDsaDN);
					this.Add(cursor);
				}
				else
				{
					ptr = Utils.AddToIntPtr(info, Marshal.SizeOf(typeof(int)) * 2 + i * Marshal.SizeOf(typeof(DS_REPL_CURSOR)));
					DS_REPL_CURSOR ds_REPL_CURSOR = new DS_REPL_CURSOR();
					Marshal.PtrToStructure(ptr, ds_REPL_CURSOR);
