...
namespace System.DirectoryServices.ActiveDirectory
{
	...
	public class AdamInstance : DirectoryServer
	{
		...
		public static AdamInstance GetAdamInstance(DirectoryContext context)
		{
			...
			if (!context.isServer())
			{
				throw new ActiveDirectoryObjectNotFoundException(Res.GetString("AINotFound", new object[]
				{
					context.Name
				}), typeof(AdamInstance), context.Name);
			}
		...
		{
			AdamInstanceCollection result = null;
			if (context == null)
			{
				throw new ArgumentNullException("context");
			}
			...
			try
			{
				result = ConfigurationSet.FindAdamInstances(context, partitionName, null);
			}
			catch (ActiveDirectoryObjectNotFoundException)
			{
				result = new AdamInstanceCollection(new ArrayList());
			}
			return result;
		}
		...
		public override ReplicationCursorCollection GetReplicationCursors(string partition)
		{
			...
			this.GetADAMHandle();
			info = base.GetReplicationInfoHelper(this.ADAMHandle, 8, 1, partition, ref advanced, context, DirectoryContext.ADAMHandle);
			return base.ConstructReplicationCursors(this.ADAMHandle, advanced, info, partition, this, DirectoryContext.ADAMHandle);
		}
		...
		public override ReplicationOperationInformation GetReplicationOperationInformation()
		{
			...
			this.GetADAMHandle();
			info = base.GetReplicationInfoHelper(this.ADAMHandle, 5, 5, null, ref flag, 0, DirectoryContext.ADAMHandle);
			return base.ConstructPendingOperations(info, this, DirectoryContext.ADAMHandle);
		}
		...
		public override ReplicationNeighborCollection GetReplicationNeighbors(string partition)
		{
			...
			this.GetADAMHandle();
			info = base.GetReplicationInfoHelper(this.ADAMHandle, 0, 0, partition, ref flag, 0, DirectoryContext.ADAMHandle);
			return base.ConstructNeighbors(info, this, DirectoryContext.ADAMHandle);
		}
		...
		public override ReplicationNeighborCollection GetAllReplicationNeighbors()
		{
			...
			this.GetADAMHandle();
			info = base.GetReplicationInfoHelper(this.ADAMHandle, 0, 0, null, ref flag, 0, DirectoryContext.ADAMHandle);
			return base.ConstructNeighbors(info, this, DirectoryContext.ADAMHandle);
		}
		...
		public override ActiveDirectoryReplicationMetadata GetReplicationMetadata(string objectPath)
		{
			...
			this.GetADAMHandle();
			info = base.GetReplicationInfoHelper(this.ADAMHandle, 9, 2, objectPath, ref advanced, 0, DirectoryContext.ADAMHandle);
			return base.ConstructMetaData(advanced, info, this, DirectoryContext.ADAMHandle);
		}
		...
		public string DefaultPartition
		{
			...
			set
			{
				...
				else
				{
					...
					{
						throw new ArgumentException(Res.GetString("ServerNotAReplica", new object[]
						{
							value
						}), "value");
					}
		...
		internal Guid NtdsaObjectGuid
		{
			get
			{
				...
				{
					DirectoryEntry cachedDirectoryEntry = this.directoryEntryMgr.GetCachedDirectoryEntry(this.NtdsaObjectName);
					byte[] b = (byte[])PropertyManager.GetPropertyValue(this.context, cachedDirectoryEntry, PropertyManager.ObjectGuid);
					this.cachedNtdsaObjectGuid = new Guid(b);
				}
		...
		private ReplicationFailureCollection GetReplicationFailures(DS_REPL_INFO_TYPE type)
		{
			...
			this.GetADAMHandle();
			info = base.GetReplicationInfoHelper(this.ADAMHandle, (int)type, (int)type, null, ref flag, 0, DirectoryContext.ADAMHandle);
			return base.ConstructFailures(info, this, DirectoryContext.ADAMHandle);
		}
