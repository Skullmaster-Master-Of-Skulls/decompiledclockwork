	...
	public class ActiveDirectoryInterSiteTransport : IDisposable
	{
		...
		public static ActiveDirectoryInterSiteTransport FindByTransportType(DirectoryContext context, ActiveDirectoryTransportType transport)
		{
			...
			try
			{
				...
				string text = "CN=Inter-Site Transports,CN=Sites," + str;
			...
			catch (COMException ex)
			{
				...
				throw new ActiveDirectoryObjectNotFoundException(Res.GetString("TransportNotFound", new object[]
				{
					transport.ToString()
				}), typeof(ActiveDirectoryInterSiteTransport), transport.ToString());
			}
		...
		public ReadOnlySiteLinkCollection SiteLinks
		{
			get
			{
				...
				if (!this.linkRetrieved)
				{
					...
					ADSearcher adsearcher = new ADSearcher(this.cachedEntry, "(&(objectClass=siteLink)(objectCategory=SiteLink))", new string[]
					{
						"cn"
		...
		public ReadOnlySiteLinkBridgeCollection SiteLinkBridges
		{
			get
			{
				...
				if (!this.bridgeRetrieved)
				{
					...
					ADSearcher adsearcher = new ADSearcher(this.cachedEntry, "(&(objectClass=siteLinkBridge)(objectCategory=SiteLinkBridge))", new string[]
					{
						"cn"
					}, SearchScope.OneLevel);
					SearchResultCollection searchResultCollection = null;
					...
					try
					{
						foreach (object obj in searchResultCollection)
						{
							SearchResult searchResult = (SearchResult)obj;
							DirectoryEntry directoryEntry = searchResult.GetDirectoryEntry();
							string bridgeName = (string)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.Cn);
							ActiveDirectorySiteLinkBridge activeDirectorySiteLinkBridge = new ActiveDirectorySiteLinkBridge(this.context, bridgeName, this.transport, true);
