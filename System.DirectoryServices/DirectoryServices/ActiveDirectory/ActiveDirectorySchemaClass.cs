...
namespace System.DirectoryServices.ActiveDirectory
{
	...
	public class ActiveDirectorySchemaClass : IDisposable
	{
				...
				if (this.mandatoryProperties == null)
				{
					if (this.isBound)
					{
						...
						else
						{
							...
							this.mandatoryProperties = new ActiveDirectorySchemaPropertyCollection(this.context, this, true, PropertyManager.MustContain, this.GetProperties(this.GetPropertyValuesRecursively(propertyNames)));
						}
					}
					else
					{
				...
				if (this.optionalProperties == null)
				{
					if (this.isBound)
					{
						...
						else
						{
							...
							ArrayList arrayList2 = new ArrayList();
							foreach (object obj2 in this.GetPropertyValuesRecursively(propertyNames))
							{
		...
		internal static Hashtable GetPropertiesFromSchemaContainer(DirectoryContext context, DirectoryEntry schemaEntry, string name, bool isDefunctOnServer)
		{
			...
			try
			{
				result = Utils.GetValuesWithRangeRetrieval(schemaEntry, stringBuilder.ToString(), arrayList, arrayList2, SearchScope.OneLevel);
			}
			catch (COMException ex)
			{
		...
		private ArrayList GetClasses(ICollection ldapDisplayNames)
		{
			...
			try
			{
				...
				foreach (object obj in ldapDisplayNames)
				{
					...
					stringBuilder.Append(PropertyManager.LdapDisplayName);
					stringBuilder.Append("=");
					stringBuilder.Append(Utils.GetEscapedFilterValue(filterValue));
					stringBuilder.Append(")");
				}
				if (ldapDisplayNames.Count > 1)
				{
				...
				ADSearcher adsearcher = new ADSearcher(this.schemaEntry, filter, propertiesToLoad, SearchScope.OneLevel);
				...
				foreach (object obj2 in searchResultCollection)
				{
					...
					ActiveDirectorySchemaClass value = new ActiveDirectorySchemaClass(this.context, text, directoryEntry, this.schemaEntry);
					arrayList.Add(value);
				}
			}
			catch (COMException e)
		...
		private ArrayList GetProperties(ICollection ldapDisplayNames)
		{
			...
			try
			{
				...
				foreach (object obj2 in searchResultCollection)
				{
					...
					directoryEntry.Username = this.context.UserName;
					directoryEntry.Password = this.context.Password;
					ActiveDirectorySchemaProperty value = new ActiveDirectorySchemaProperty(this.context, text, directoryEntry, this.schemaEntry);
					arrayList.Add(value);
				}
			}
			catch (COMException e)
		...
		private ArrayList GetPropertyValuesRecursively(string[] propertyNames)
		{
			...
			{
				foreach (object obj6 in this.GetValuesFromCache(propertyName))
				{
					string text6 = (string)obj6;
					if (!arrayList.Contains(text6))
					{
						arrayList.Add(text6);
					}
				}
