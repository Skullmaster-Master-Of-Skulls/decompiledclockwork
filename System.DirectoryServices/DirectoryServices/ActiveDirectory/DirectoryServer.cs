	...
	public abstract class DirectoryServer : IDisposable
	{
		...
		public void MoveToAnotherSite(string siteName)
		{
			...
			if (Utils.Compare(this.SiteName, siteName) != 0)
			{
				...
				try
				{
					...
					string text = (string)PropertyManager.GetPropertyValue(this.context, cachedDirectoryEntry, PropertyManager.DistinguishedName);
		...
		internal ArrayList GetPartitions()
		{
			...
			try
			{
				...
				directoryEntry2 = DirectoryEntryManager.GetDirectoryEntry(this.context, dn);
		...
		internal IntPtr GetReplicationInfoHelper(IntPtr dsHandle, int type, int secondaryType, string partition, ref bool advanced, int context, LoadLibrarySafeHandle libHandle)
		{
			...
			if (procAddress == (IntPtr)0)
			{
				...
				num = dsReplicaGetInfoW(dsHandle, secondaryType, partition, (IntPtr)0, ref result);
				advanced = false;
				flag = false;
			}
			else
			{
				...
				num = dsReplicaGetInfo2W(dsHandle, type, partition, (IntPtr)0, null, null, 0, context, ref result);
			}
			if (flag && num == 50)
			{
				procAddress = UnsafeNativeMethods.GetProcAddress(libHandle, "DsReplicaGetInfoW");
				...
				num = dsReplicaGetInfoW2(dsHandle, secondaryType, partition, (IntPtr)0, ref result);
				advanced = false;
			}
			if (num != 0)
			{
		...
		internal ActiveDirectoryReplicationMetadata ConstructMetaData(bool advanced, IntPtr info, DirectoryServer server, LoadLibrarySafeHandle libHandle)
		{
			...
			if (advanced)
			{
				try
				{
					if (info != (IntPtr)0)
					{
						...
						int cNumEntries = ds_REPL_OBJ_META_DATA_.cNumEntries;
						if (cNumEntries > 0)
						{
							activeDirectoryReplicationMetadata.AddHelper(cNumEntries, info, true);
						}
					}
			...
			try
			{
				...
				int cNumEntries = ds_REPL_OBJ_META_DATA.cNumEntries;
				if (cNumEntries > 0)
				{
					activeDirectoryReplicationMetadata.AddHelper(cNumEntries, info, false);
				}
			}
			finally
			{
		...
		internal ReplicationConnectionCollection GetInboundConnectionsHelper()
		{
			if (this.inbound == null)
			{
				...
				ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=nTDSConnection)(objectCategory=nTDSConnection))", new string[]
				{
					"cn"
				...
				try
				{
					...
					foreach (object obj in searchResultCollection)
					{
						...
						ReplicationConnection value = new ReplicationConnection(newDirectoryContext, searchResult.GetDirectoryEntry(), (string)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.Cn));
						this.inbound.Add(value);
					}
				}
				catch (COMException e)
				{
		...
		internal ReplicationConnectionCollection GetOutboundConnectionsHelper()
		{
			if (this.outbound == null)
			{
				...
				DirectoryEntry directoryEntry = DirectoryEntryManager.GetDirectoryEntry(Utils.GetNewDirectoryContext(this.Name, DirectoryContextType.DirectoryServer, this.context), dn);
				string str = (this is DomainController) ? ((DomainController)this).ServerObjectName : ((AdamInstance)this).ServerObjectName;
				ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=nTDSConnection)(objectCategory=nTDSConnection)(fromServer=CN=NTDS Settings," + str + "))", new string[]
				{
					"objectClass",
					"cn"
				}, SearchScope.Subtree);
				...
				try
				{
					...
					{
						SearchResult searchResult = (SearchResult)obj;
						ReplicationConnection value = new ReplicationConnection(newDirectoryContext, searchResult.GetDirectoryEntry(), (string)searchResult.Properties["cn"][0]);
						this.outbound.Add(value);
					}
				}
				catch (COMException e)
				{
