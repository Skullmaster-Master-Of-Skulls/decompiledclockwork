...
namespace System.DirectoryServices.ActiveDirectory
{
	...
	public class ActiveDirectorySchemaProperty : IDisposable
	{
				...
				if (this.isBound && !this.linkedPropertyInitialized)
				{
					...
					if (num != -1)
					{
						...
						try
						{
							...
							ReadOnlyActiveDirectorySchemaPropertyCollection allProperties = ActiveDirectorySchema.GetAllProperties(this.context, this.schemaEntry, filter);
							if (allProperties.Count != 1)
							{
								throw new ActiveDirectoryObjectNotFoundException(Res.GetString("LinkedPropertyNotFound", new object[]
								{
									num2
								}), typeof(ActiveDirectorySchemaProperty), null);
							}
		...
		private object GetValueFromCache(string propertyName, bool mustExist)
		{
			...
			try
			{
				...
				if (resultPropertyValueCollection == null || resultPropertyValueCollection.Count < 1)
				{
					if (mustExist)
					{
						throw new ActiveDirectoryOperationException(Res.GetString("PropertyNotFound", new object[]
						{
							propertyName
						}));
					}
				...
				{
					result = resultPropertyValueCollection[0];
				}
			}
			catch (COMException e)
			{
		...
		internal static SearchResult GetPropertiesFromSchemaContainer(DirectoryContext context, DirectoryEntry schemaEntry, string name, bool isDefunctOnServer)
		{
			...
			ADSearcher adsearcher = new ADSearcher(schemaEntry, stringBuilder.ToString(), propertiesToLoad, SearchScope.OneLevel, false, false);
			try
			{
		...
		private ActiveDirectorySyntax MapSyntax(string syntaxId, int oMID, OMObjectClass oMObjectClass)
		{
			...
			throw new ActiveDirectoryOperationException(Res.GetString("UnknownSyntax", new object[]
			{
				this.ldapDisplayName
			}));
		}
		...
		private static OMObjectClass dnOMObjectClass = new OMObjectClass(new byte[]
		{
			43,
			12,
			2,
		...
		private static OMObjectClass dNWithStringOMObjectClass = new OMObjectClass(new byte[]
		{
			42,
			134,
			72,
		...
		private static OMObjectClass dNWithBinaryOMObjectClass = new OMObjectClass(new byte[]
		{
			42,
			134,
			72,
		...
		private static OMObjectClass replicaLinkOMObjectClass = new OMObjectClass(new byte[]
		{
			42,
			134,
			72,
		...
		private static OMObjectClass presentationAddressOMObjectClass = new OMObjectClass(new byte[]
		{
			43,
			12,
			2,
		...
		private static OMObjectClass accessPointDnOMObjectClass = new OMObjectClass(new byte[]
		{
			43,
			12,
			2,
		...
		private static OMObjectClass oRNameOMObjectClass = new OMObjectClass(new byte[]
		{
			86,
			6,
			1,
