	...
	public class ActiveDirectorySiteLinkBridge : IDisposable
	{
		...
		public ActiveDirectorySiteLinkBridge(DirectoryContext context, string bridgeName, ActiveDirectoryTransportType transport)
		{
			...
			try
			{
				...
				this.cachedEntry = directoryEntry.Children.Add(originalPath, "siteLinkBridge");
			}
		...
		public static ActiveDirectorySiteLinkBridge FindByName(DirectoryContext context, string bridgeName, ActiveDirectoryTransportType transport)
		{
			...
			ActiveDirectorySiteLinkBridge result;
			try
			{
				ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=siteLinkBridge)(objectCategory=SiteLinkBridge)(name=" + Utils.GetEscapedFilterValue(bridgeName) + "))", new string[]
				{
					"distinguishedName"
				...
				{
					Exception ex = new ActiveDirectoryObjectNotFoundException(Res.GetString("DSNotFound"), typeof(ActiveDirectorySiteLinkBridge), bridgeName);
					throw ex;
				}
				DirectoryEntry directoryEntry2 = searchResult.GetDirectoryEntry();
				result = new ActiveDirectorySiteLinkBridge(context, bridgeName, transport, true)
				{
		...
		private void GetLinks()
		{
			...
			for (int i = 0; i < arrayList2.Count; i++)
			{
				...
				ActiveDirectorySiteLink link = new ActiveDirectorySiteLink(this.context, text3, this.transport, true, directoryEntry);
				this.links.Add(link);
			}
		}
