	...
	public class ActiveDirectorySiteLink : IDisposable
	{
		...
		public ActiveDirectorySiteLink(DirectoryContext context, string siteLinkName, ActiveDirectoryTransportType transport, ActiveDirectorySchedule schedule)
		{
			...
			try
			{
				...
				this.cachedEntry = directoryEntry.Children.Add(originalPath, "siteLink");
		...
		public static ActiveDirectorySiteLink FindByName(DirectoryContext context, string siteLinkName, ActiveDirectoryTransportType transport)
		{
			...
			ActiveDirectorySiteLink result;
			try
			{
				ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=siteLink)(objectCategory=SiteLink)(name=" + Utils.GetEscapedFilterValue(siteLinkName) + "))", new string[]
				{
					"distinguishedName"
				...
				{
					Exception ex = new ActiveDirectoryObjectNotFoundException(Res.GetString("DSNotFound"), typeof(ActiveDirectorySiteLink), siteLinkName);
					throw ex;
				}
				...
				result = activeDirectorySiteLink;
			}
		...
		public TimeSpan ReplicationInterval
		{
			get
			{
				...
				try
				{
					...
					{
						int minutes = (int)this.cachedEntry.Properties["replInterval"][0];
		...
		private void GetSites()
		{
			...
			for (int i = 0; i < arrayList2.Count; i++)
			{
				...
				ActiveDirectorySite site = new ActiveDirectorySite(this.context, text2, true);
				this.sites.Add(site);
			}
		}
