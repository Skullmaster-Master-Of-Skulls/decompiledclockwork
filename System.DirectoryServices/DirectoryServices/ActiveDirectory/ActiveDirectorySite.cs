...
namespace System.DirectoryServices.ActiveDirectory
{
	...
	public class ActiveDirectorySite : IDisposable
	{
		...
		public static ActiveDirectorySite FindByName(DirectoryContext context, string siteName)
		{
			...
			ActiveDirectorySite result;
			try
			{
				ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=site)(objectCategory=site)(name=" + Utils.GetEscapedFilterValue(siteName) + "))", new string[]
				{
					"distinguishedName"
		...
		public ActiveDirectorySite(DirectoryContext context, string siteName)
		{
			...
			try
			{
				...
				string originalPath = "cn=" + this.name;
				originalPath = Utils.GetEscapedPath(originalPath);
				this.cachedEntry = directoryEntry.Children.Add(originalPath, "site");
			}
			catch (COMException e)
			{
				throw ExceptionHelper.GetExceptionFromCOMException(context, e);
		...
		public static ActiveDirectorySite GetComputerSite()
		{
			...
			if (num == 0)
			{
				...
				try
				{
					string siteName = Marshal.PtrToStringUni(intPtr);
					...
					ActiveDirectorySite activeDirectorySite = ActiveDirectorySite.FindByName(newDirectoryContext, siteName);
					result = activeDirectorySite;
				}
				finally
				{
			...
			get
			{
				...
				if (this.existing && this.topologyGenerator == null && !this.topologyTouched)
				{
					...
					if (flag)
					{
						...
						try
						{
							text = (string)PropertyManager.GetPropertyValue(this.context, directoryEntry.Parent, PropertyManager.DnsHostName);
						}
						catch (COMException ex)
						{
							if (ex.ErrorCode == -2147016656)
						...
						this.topologyGenerator = new DomainController(Utils.GetNewDirectoryContext(text, DirectoryContextType.DirectoryServer, this.context), text);
					}
				}
				IL_142:
		...
		public string Location
		{
			get
			{
				...
				try
				{
					...
					{
						result = (string)this.cachedEntry.Properties["location"][0];
					}
					else
					{
						result = null;
			...
			get
			{
				if (this.ntdsEntry == null)
				{
					...
					catch (COMException ex)
					{
						if (ex.ErrorCode == -2147016656)
						{
							string @string = Res.GetString("NTDSSiteSetting", new object[]
							{
								this.name
							});
				...
				if (this.existing)
				{
					if (this.topologyTouched)
					{
						try
						{
							...
							this.NTDSSiteEntry.Properties["interSiteTopologyGenerator"].Value = value;
						}
						catch (COMException e)
						{
							throw ExceptionHelper.GetExceptionFromCOMException(this.context, e);
		...
		private ReadOnlyDirectoryServerCollection GetBridgeheadServers()
		{
			...
			if (this.existing)
			{
				...
				try
				{
					ADSearcher adsearcher = new ADSearcher(directoryEntry, "(|(objectCategory=server)(objectCategory=NTDSConnection))", new string[]
					{
						"fromServer",
						"distinguishedName",
						"dNSHostName",
					...
					try
					{
						foreach (object obj in searchResultCollection)
						{
							...
							string s = (string)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.ObjectCategory);
							if (Utils.Compare(s, 0, "CN=Server".Length, "CN=Server", 0, "CN=Server".Length) == 0)
							{
								hashtable3.Add((string)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.DistinguishedName), (string)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.DnsHostName));
							}
						}
						foreach (object obj2 in searchResultCollection)
						{
							SearchResult res2 = (SearchResult)obj2;
							string s2 = (string)PropertyManager.GetSearchResultPropertyValue(res2, PropertyManager.ObjectCategory);
							if (Utils.Compare(s2, 0, "CN=Server".Length, "CN=Server", 0, "CN=Server".Length) != 0)
							{
								string distinguishedName = (string)PropertyManager.GetSearchResultPropertyValue(res2, PropertyManager.FromServer);
								string text = Utils.GetPartialDN(distinguishedName, 3);
								adsPathname.Set(text, 4);
								text = adsPathname.Retrieve(11);
								text = text.Substring(3);
								string partialDN = Utils.GetPartialDN((string)PropertyManager.GetSearchResultPropertyValue(res2, PropertyManager.DistinguishedName), 2);
								if (!hashtable.Contains(partialDN))
								{
									string value = (string)hashtable3[partialDN];
									if (!hashtable2.Contains(partialDN))
									{
										hashtable2.Add(partialDN, value);
									}
									if (Utils.Compare((string)PropertyManager.GetPropertyValue(this.context, this.cachedEntry, PropertyManager.Cn), text) != 0)
									{
										hashtable.Add(partialDN, value);
				...
				if (hashtable2.Count != 0)
				{
					...
					ADSearcher adsearcher2 = new ADSearcher(directoryEntry2, "(&(objectClass=nTDSConnection)(objectCategory=NTDSConnection)" + stringBuilder.ToString() + ")", new string[]
					{
						"fromServer",
						"distinguishedName"
					}, SearchScope.Subtree);
					...
					try
					{
						...
						{
							SearchResult res3 = (SearchResult)obj4;
							string text2 = (string)PropertyManager.GetSearchResultPropertyValue(res3, PropertyManager.FromServer);
							string key = text2.Substring(17);
							if (hashtable2.Contains(key))
							{
								string text3 = Utils.GetPartialDN((string)PropertyManager.GetSearchResultPropertyValue(res3, PropertyManager.DistinguishedName), 4);
								adsPathname.Set(text3, 4);
								text3 = adsPathname.Retrieve(11);
								text3 = text3.Substring(3);
								if (Utils.Compare(text3, (string)PropertyManager.GetPropertyValue(this.context, this.cachedEntry, PropertyManager.Cn)) != 0)
								{
				...
				foreach (object obj5 in hashtable)
				{
					...
					string text4 = (string)dictionaryEntry2.Value;
					...
					if (this.IsADAM)
					{
						...
						string adamInstanceName = text4;
						if (num != 389)
						{
							adamInstanceName = text4 + ":" + num;
						}
					...
					{
						server = new DomainController(Utils.GetNewDirectoryContext(text4, DirectoryContextType.DirectoryServer, this.context), text4);
					}
					readOnlyDirectoryServerCollection.Add(server);
				}
			}
		...
		private void GetSubnets()
		{
			...
			ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=subnet)(objectCategory=subnet)(siteObject=" + Utils.GetEscapedFilterValue((string)PropertyManager.GetPropertyValue(this.context, this.cachedEntry, PropertyManager.DistinguishedName)) + "))", new string[]
			{
				"cn",
				"location"
			...
			try
			{
				foreach (object obj in searchResultCollection)
				{
					...
					string subnetName = (string)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.Cn);
					ActiveDirectorySubnet activeDirectorySubnet = new ActiveDirectorySubnet(this.context, subnetName, null, true);
		...
		{
			DirectoryEntry directoryEntry = DirectoryEntryManager.GetDirectoryEntry(this.context, WellKnownDN.RootDSE);
			string str = (string)directoryEntry.Properties["configurationNamingContext"][0];
			string dn = "CN=Inter-Site Transports,CN=Sites," + str;
			directoryEntry = DirectoryEntryManager.GetDirectoryEntry(this.context, dn);
			ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=siteLink)(objectCategory=SiteLink)(siteList=" + Utils.GetEscapedFilterValue((string)PropertyManager.GetPropertyValue(this.context, this.cachedEntry, PropertyManager.DistinguishedName)) + "))", new string[]
			{
				"cn",
				"distinguishedName"
			}, SearchScope.Subtree);
			...
			try
			{
				...
				{
					SearchResult searchResult = (SearchResult)obj;
					string distinguishedName = (string)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.DistinguishedName);
					string siteLinkName = (string)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.Cn);
					...
					try
					{
						activeDirectorySiteLink = new ActiveDirectorySiteLink(this.context, siteLinkName, transport, true, searchResult.GetDirectoryEntry());
		...
		private void GetLinks()
		{
			...
			try
			{
				foreach (object obj in searchResultCollection)
				{
					...
					ActiveDirectorySiteLink link;
					if (string.Compare(value, "IP", StringComparison.OrdinalIgnoreCase) == 0)
					{
						link = new ActiveDirectorySiteLink(this.context, siteLinkName, ActiveDirectoryTransportType.Rpc, true, directoryEntry2);
					}
					else
					{
						if (string.Compare(value, "SMTP", StringComparison.OrdinalIgnoreCase) != 0)
						...
						link = new ActiveDirectorySiteLink(this.context, siteLinkName, ActiveDirectoryTransportType.Smtp, true, directoryEntry2);
					}
					this.links.Add(link);
				}
			}
			...
			if (!this.IsADAM)
			{
				...
				try
				{
					...
					if (cItems > 0)
					{
						...
						{
							ptr = Utils.AddToIntPtr(rItems, Marshal.SizeOf(typeof(DS_NAME_RESULT_ITEM)) * i);
		...
		{
			ADSearcher adsearcher = new ADSearcher(this.cachedEntry, "(&(objectClass=server)(objectCategory=server))", new string[]
			{
				"dNSHostName"
			}, SearchScope.Subtree);
			SearchResultCollection searchResultCollection = null;
			...
			try
			{
				foreach (object obj in searchResultCollection)
				{
					...
					{
						server = new DomainController(Utils.GetNewDirectoryContext(text, DirectoryContextType.DirectoryServer, this.context), text);
					}
		...
		private void GetPreferredBridgeheadServers(ActiveDirectoryTransportType transport)
		{
			...
			DirectoryEntry directoryEntry = DirectoryEntryManager.GetDirectoryEntry(this.context, dn);
			ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=server)(objectCategory=Server)(bridgeheadTransportList=" + Utils.GetEscapedFilterValue(filterValue) + "))", new string[]
			{
				"dNSHostName",
				"distinguishedName"
			}, SearchScope.OneLevel);
