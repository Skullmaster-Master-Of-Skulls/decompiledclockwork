...
namespace System.DirectoryServices.ActiveDirectory
{
	...
	public class ActiveDirectoryReplicationMetadata : DictionaryBase
	{
		...
		{
			IntPtr info2 = (IntPtr)0;
			for (int i = 0; i < count; i++)
			{
				if (advanced)
				{
					info2 = Utils.AddToIntPtr(info, Marshal.SizeOf(typeof(int)) * 2 + i * Marshal.SizeOf(typeof(DS_REPL_ATTR_META_DATA_2)));
					AttributeMetadata attributeMetadata = new AttributeMetadata(info2, true, this.server, this.nameTable);
					this.Add(attributeMetadata.Name, attributeMetadata);
				}
				else
				{
					info2 = Utils.AddToIntPtr(info, Marshal.SizeOf(typeof(int)) * 2 + i * Marshal.SizeOf(typeof(DS_REPL_ATTR_META_DATA)));
					AttributeMetadata attributeMetadata2 = new AttributeMetadata(info2, false, this.server, this.nameTable);
