	...
	internal class TrustHelper
	{
		...
		internal static bool GetTrustedDomainInfoStatus(DirectoryContext context, string sourceName, string targetName, TRUST_ATTRIBUTE attribute, bool isForest)
		{
			...
			try
			{
				try
				{
					PolicySafeHandle handle = new PolicySafeHandle(Utils.GetPolicyHandle(policyServerName));
					...
					int num = UnsafeNativeMethods.LsaQueryTrustedDomainInfoByName(handle, lsa_UNICODE_STRING, TRUSTED_INFORMATION_CLASS.TrustedDomainInformationEx, ref intPtr);
					if (num != 0)
					{
			...
			try
			{
				try
				{
					...
					else
					{
						...
						num = UnsafeNativeMethods.LsaSetTrustedDomainInfoByName(handle, lsa_UNICODE_STRING, TRUSTED_INFORMATION_CLASS.TrustedDomainInformationEx, intPtr2);
						if (num != 0)
						{
		...
		internal static void DeleteTrust(DirectoryContext sourceContext, string sourceName, string targetName, bool isForest)
		{
			...
			try
			{
				try
				{
					...
					int num = UnsafeNativeMethods.LsaQueryTrustedDomainInfoByName(handle, lsa_UNICODE_STRING, TRUSTED_INFORMATION_CLASS.TrustedDomainInformationEx, ref intPtr2);
					if (num != 0)
					{
						int num2 = UnsafeNativeMethods.LsaNtStatusToWinError(num);
						if (num2 != TrustHelper.STATUS_OBJECT_NAME_NOT_FOUND)
					...
					else
					{
						try
						{
							...
							TrustHelper.ValidateTrustAttribute(trusted_DOMAIN_INFORMATION_EX, isForest, sourceName, targetName);
							num = UnsafeNativeMethods.LsaDeleteTrustedDomain(handle, trusted_DOMAIN_INFORMATION_EX.Sid);
							if (num != 0)
							{
								int num2 = UnsafeNativeMethods.LsaNtStatusToWinError(num);
								throw ExceptionHelper.GetExceptionFromErrorCode(num2, policyServerName);
		...
		internal static void VerifyTrust(DirectoryContext context, string sourceName, string targetName, bool isForest, TrustDirection direction, bool forceSecureChannelReset, string preferredTargetServer)
		{
			...
			try
			{
				try
				{
					...
					TrustHelper.ValidateTrust(handle, lsa_UNICODE_STRING, sourceName, targetName, isForest, (int)direction, policyServerName);
		...
		internal static void CreateTrust(DirectoryContext sourceContext, string sourceName, DirectoryContext targetContext, string targetName, bool isForest, TrustDirection direction, string password)
		{
			...
			try
			{
				try
				{
					...
					flag = Utils.Impersonate(sourceContext);
					PolicySafeHandle handle = new PolicySafeHandle(Utils.GetPolicyHandle(policyServerName));
					int num = UnsafeNativeMethods.LsaCreateTrustedDomainEx(handle, trusted_DOMAIN_INFORMATION_EX, trusted_DOMAIN_AUTH_INFORMATION, TrustHelper.TRUSTED_SET_POSIX | TrustHelper.TRUSTED_SET_AUTH, out intPtr4);
					if (num != 0)
					{
						...
						if (isForest)
						{
							throw new ActiveDirectoryObjectExistsException(Res.GetString("AlreadyExistingForestTrust", new object[]
							{
								sourceName,
								targetName
							}));
						}
						throw new ActiveDirectoryObjectExistsException(Res.GetString("AlreadyExistingDomainTrust", new object[]
						{
							sourceName,
							targetName
						}));
		...
		internal static string UpdateTrust(DirectoryContext context, string sourceName, string targetName, string password, bool isForest)
		{
			...
			try
			{
				try
				{
					...
					int num = UnsafeNativeMethods.LsaQueryTrustedDomainInfoByName(handle, lsa_UNICODE_STRING, TRUSTED_INFORMATION_CLASS.TrustedDomainFullInformation, ref intPtr);
					if (num != 0)
					{
						int num2 = UnsafeNativeMethods.LsaNtStatusToWinError(num);
						if (num2 != TrustHelper.STATUS_OBJECT_NAME_NOT_FOUND)
					...
					else
					{
						...
						num = UnsafeNativeMethods.LsaSetTrustedDomainInfoByName(handle, lsa_UNICODE_STRING, TRUSTED_INFORMATION_CLASS.TrustedDomainFullInformation, intPtr2);
						if (num != 0)
						{
							throw ExceptionHelper.GetExceptionFromErrorCode(UnsafeNativeMethods.LsaNtStatusToWinError(num), policyServerName);
						}
						result = policyServerName;
					}
				}
				finally
				{
		...
		private static void ValidateTrustAttribute(TRUSTED_DOMAIN_INFORMATION_EX domainInfo, bool isForest, string sourceName, string targetName)
		{
			...
			else
			{
				if ((domainInfo.TrustAttributes & TRUST_ATTRIBUTE.TRUST_ATTRIBUTE_FOREST_TRANSITIVE) != (TRUST_ATTRIBUTE)0)
				{
					throw new ActiveDirectoryObjectNotFoundException(Res.GetString("WrongForestTrust", new object[]
					{
						sourceName,
						targetName
					}), typeof(TrustRelationshipInformation), null);
		...
		private static IntPtr GetTrustedDomainInfo(DirectoryContext targetContext, string targetName, bool isForest)
		{
			...
			try
			{
				try
				{
					text = Utils.GetPolicyServerName(targetContext, isForest, false, targetName);
					flag = Utils.Impersonate(targetContext);
					try
					{
						handle = new PolicySafeHandle(Utils.GetPolicyHandle(text));
					}
					...
					int num = UnsafeNativeMethods.LsaQueryInformationPolicy(handle, TrustHelper.PolicyDnsDomainInformation, out intPtr);
					if (num != 0)
					{
