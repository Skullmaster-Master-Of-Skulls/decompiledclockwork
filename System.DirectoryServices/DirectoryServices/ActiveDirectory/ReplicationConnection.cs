	...
	public class ReplicationConnection : IDisposable
	{
		...
		public static ReplicationConnection FindByName(DirectoryContext context, string name)
		{
			...
			try
			{
				...
				ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=nTDSConnection)(objectCategory=NTDSConnection)(name=" + Utils.GetEscapedFilterValue(name) + "))", new string[]
				{
					"distinguishedName"
				...
				result = new ReplicationConnection(context, directoryEntry2, name);
			}
			finally
			{
		...
		public ReplicationConnection(DirectoryContext context, string name, DirectoryServer sourceServer, ActiveDirectorySchedule schedule, ActiveDirectoryTransportType transport)
		{
			...
			try
			{
				...
				string dn = "CN=NTDS Settings," + str;
				directoryEntry = DirectoryEntryManager.GetDirectoryEntry(context, dn);
				string text = "cn=" + this.connectionName;
				text = Utils.GetEscapedPath(text);
				this.cachedDirectoryEntry = directoryEntry.Children.Add(text, "nTDSConnection");
				...
				this.cachedDirectoryEntry.Properties["fromServer"].Add(text2);
				if (schedule != null)
				{
			...
			get
			{
				...
				if (this.sourceServerName == null)
				{
					...
					if (this.IsADAM)
					{
						...
						string arg = (string)PropertyManager.GetPropertyValue(this.context, directoryEntry.Parent, PropertyManager.DnsHostName);
						if (num != 389)
						{
							this.sourceServerName = arg + ":" + num;
						}
					}
					else
					{
		...
		public string DestinationServer
		{
			get
			{
				...
				if (this.destinationServerName == null)
				{
					...
					string arg = (string)PropertyManager.GetPropertyValue(this.context, directoryEntry2, PropertyManager.DnsHostName);
					if (this.IsADAM)
					{
						int num = (int)PropertyManager.GetPropertyValue(this.context, directoryEntry, PropertyManager.MsDSPortLDAP);
						if (num != 389)
						{
							this.destinationServerName = arg + ":" + num;
						}
						else
						{
							this.destinationServerName = arg;
						}
					}
					else
					{
		...
		public bool Enabled
		{
			get
			{
				...
				try
				{
					...
					{
						result = (bool)this.cachedDirectoryEntry.Properties["enabledConnection"][0];
					}
					else
					{
						result = false;
		...
		public ReplicationSpan ReplicationSpan
		{
			get
			{
				...
				string distinguishedName2 = (string)PropertyManager.GetPropertyValue(this.context, directoryEntry, PropertyManager.ServerName);
