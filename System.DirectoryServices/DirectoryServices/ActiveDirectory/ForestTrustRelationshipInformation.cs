		...
		public void Save()
		{
			...
			try
			{
				try
				{
					...
					int num4 = UnsafeNativeMethods.LsaSetForestTrustInformation(handle, lsa_UNICODE_STRING, intPtr3, 1, out intPtr4);
					if (num4 != 0)
					{
					...
					num4 = UnsafeNativeMethods.LsaSetForestTrustInformation(handle, lsa_UNICODE_STRING, intPtr3, 0, out intPtr4);
					if (num4 != 0)
					{
		...
		private void GetForestTrustInfoHelper()
		{
			...
			try
			{
				try
				{
					...
					flag = Utils.Impersonate(this.context);
					PolicySafeHandle handle = new PolicySafeHandle(Utils.GetPolicyHandle(policyServerName));
					int num = UnsafeNativeMethods.LsaQueryForestTrustInformation(handle, lsa_UNICODE_STRING, ref intPtr);
					if (num != 0)
					{
					...
					try
					{
						if (intPtr != (IntPtr)0)
						{
							...
							for (int i = 0; i < recordCount; i++)
							{
								...
								{
									IntPtr ptr = Utils.AddToIntPtr(intPtr3, 16);
								...
								{
									IntPtr ptr2 = Utils.AddToIntPtr(intPtr3, 16);
