...
namespace System.DirectoryServices.ActiveDirectory
{
	...
	internal sealed class Utils
	{
		...
		internal static string GetDnsNameFromDN(string distinguishedName)
		{
			...
			int num = dsCrackNames(IntPtr.Zero, 1, 1, 7, 1, intPtr2, out zero);
			if (num == 0)
			{
				try
				{
					...
					if (dsNameResult.itemCount >= 1 && dsNameResult.items != IntPtr.Zero)
					{
						...
						{
							result = dsNameResultItem.name.Substring(0, dsNameResultItem.name.Length - 1);
						}
						else
						{
		...
		internal static string GetDNFromDnsName(string dnsName)
		{
			...
			IntPtr intPtr2 = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(IntPtr)));
			Marshal.WriteIntPtr(intPtr2, intPtr);
			int num = dsCrackNames(IntPtr.Zero, 1, 7, 1, 1, intPtr2, out zero);
			if (num == 0)
			{
				try
				{
		...
		internal static string GetDnsHostNameFromNTDSA(DirectoryContext context, string dn)
		{
			...
			try
			{
				result = (string)PropertyManager.GetPropertyValue(context, directoryEntry, PropertyManager.DnsHostName);
			}
			finally
			{
		...
		internal static string GetAdamDnsHostNameFromNTDSA(DirectoryContext context, string dn)
		{
			...
			try
			{
				...
				{
					throw new ActiveDirectoryOperationException(Res.GetString("NoHostNameOrPortNumber", new object[]
					{
						dn
					}));
				}
				foreach (object obj in searchResultCollection)
				{
					...
					string text3 = (string)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.ObjectCategory);
					if (text3.Length >= text2.Length && Utils.Compare(text3, 0, text2.Length, text2, 0, text2.Length) == 0)
					{
						num = (int)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.MsDSPortLDAP);
					}
					else
					{
						text = (string)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.DnsHostName);
					}
				}
			}
			finally
		...
		internal static string GetAdamHostNameAndPortsFromNTDSA(DirectoryContext context, string dn)
		{
			...
			try
			{
				...
				foreach (object obj in searchResultCollection)
				{
					...
					{
						num = (int)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.MsDSPortLDAP);
						num2 = (int)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.MsDSPortSSL);
					}
					else
					{
						text = (string)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.DnsHostName);
		...
		internal static IntPtr GetAuthIdentity(DirectoryContext context, LoadLibrarySafeHandle libHandle)
		{
			string user;
			string domain;
			Utils.GetDomainAndUsername(context, out user, out domain);
			...
			IntPtr result;
			int num = dsMakePasswordCredentials(user, domain, context.Password, out result);
			if (num != 0)
			{
				throw ExceptionHelper.GetExceptionFromErrorCode(num);
			}
		...
		internal static IntPtr GetDSHandle(string domainControllerName, string domainName, IntPtr authIdentity, LoadLibrarySafeHandle libHandle)
		{
			...
			IntPtr result;
			int num = dsBindWithCred(domainControllerName, domainName, authIdentity, out result);
			if (num != 0)
			{
		...
		internal static bool CheckCapability(DirectoryEntry rootDSE, Capability capability)
		{
			...
			if (rootDSE != null)
			{
				...
				if (capability == Capability.ActiveDirectoryApplicationMode)
				{
					using (IEnumerator enumerator2 = rootDSE.Properties[PropertyManager.SupportedCapabilities].GetEnumerator())
					{
						while (enumerator2.MoveNext())
						{
							...
							if (string.Compare(strA2, SupportedCapability.ADAMOid, StringComparison.OrdinalIgnoreCase) == 0)
							{
								result = true;
								break;
							}
				...
				if (capability == Capability.ActiveDirectoryOrADAM)
				{
					...
					{
						string strA3 = (string)obj3;
						if (string.Compare(strA3, SupportedCapability.ADAMOid, StringComparison.OrdinalIgnoreCase) == 0 || string.Compare(strA3, SupportedCapability.ADOid, StringComparison.OrdinalIgnoreCase) == 0)
						{
							result = true;
							break;
						}
		...
		internal static DirectoryEntry GetCrossRefEntry(DirectoryContext context, DirectoryEntry partitionsEntry, string partitionName)
		{
			...
			ADSearcher adsearcher = new ADSearcher(partitionsEntry, filter, new string[]
			{
				PropertyManager.DistinguishedName
			...
			string text = (string)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.DistinguishedName);
		...
		internal static ActiveDirectoryTransportType GetTransportTypeFromDN(string DN)
		{
			...
			string @string = Res.GetString("UnknownTransport", new object[]
			{
				value
			});
		...
		internal static string GetServerNameFromInvocationID(string serverObjectDN, Guid invocationID, DirectoryServer server)
		{
			...
			if (serverObjectDN == null)
			{
				...
				if (num == 0)
				{
					...
					goto IL_7D;
					IL_92:
					ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=nTDSDSA)(invocationID=" + str + "))", new string[]
					{
						"distinguishedName"
					}, SearchScope.Subtree, false, false);
					try
					{
		...
		internal static bool Impersonate(DirectoryContext context)
		{
			...
			string lpszUsername;
			string lpszDomain;
			Utils.GetDomainAndUsername(context, out lpszUsername, out lpszDomain);
			if (UnsafeNativeMethods.LogonUserW(lpszUsername, lpszDomain, context.Password, Utils.LOGON32_LOGON_NEW_CREDENTIALS, Utils.LOGON32_PROVIDER_WINNT50, ref intPtr) == 0)
			{
				throw ExceptionHelper.GetExceptionFromErrorCode(Marshal.GetLastWin32Error());
			}
			try
		...
		internal static string GetPolicyServerName(DirectoryContext context, bool isForest, bool needPdc, string source)
		{
			...
			if (context.isDomain())
			{
				...
				result = Locator.GetDomainControllerInfo(null, source, null, (long)privateLocatorFlags).DomainControllerName.Substring(2);
			}
			else if (isForest)
			{
				if (needPdc)
		...
		internal static IntPtr GetPolicyHandle(string serverName)
		{
			...
			try
			{
				int num = UnsafeNativeMethods.LsaOpenPolicy(lsa_UNICODE_STRING, objectAttributes, policy_VIEW_LOCAL_INFORMATION, out intPtr);
				if (num != 0)
				{
		...
		internal static Hashtable GetValuesWithRangeRetrieval(DirectoryEntry searchRootEntry, string filter, ArrayList propertiesWithRangeRetrieval, ArrayList propertiesWithoutRangeRetrieval, SearchScope searchScope)
		{
			...
			for (;;)
			{
				foreach (object obj3 in arrayList2)
				{
					...
					hashtable2.Add(text5.ToLower(CultureInfo.InvariantCulture), value);
				}
				arrayList2.Clear();
				SearchResult searchResult = adsearcher.FindOne();
				if (searchResult == null)
				...
				using (IEnumerator enumerator4 = searchResult.Properties.PropertyNames.GetEnumerator())
				{
					while (enumerator4.MoveNext())
					{
						...
						if (hashtable2.Contains(text7) || arrayList.Contains(text7))
						{
							...
							arrayList3.AddRange(searchResult.Properties[text6]);
							if (hashtable2.Contains(text7))
							{
								string text8 = (string)hashtable2[text7];
								if (text6.Length >= text8.Length && Utils.Compare(text8, 0, text8.Length, text6, 0, text8.Length) != 0)
								{
									arrayList2.Add(text7);
									num += searchResult.Properties[text6].Count;
								}
							}
						}
					}
		...
		internal static ArrayList GetReplicaList(DirectoryContext context, string partitionName, string siteName, bool isDefaultNC, bool isADAM, bool isGC)
		{
			...
			if (flag)
			{
				...
				try
				{
					...
					if (isADAM)
					{
						name = Utils.GetAdamDnsHostNameFromNTDSA(context, (string)PropertyManager.GetPropertyValue(context, directoryEntry, PropertyManager.FsmoRoleOwner));
					}
					else
					{
						name = Utils.GetDnsHostNameFromNTDSA(context, (string)PropertyManager.GetPropertyValue(context, directoryEntry, PropertyManager.FsmoRoleOwner));
					}
					...
					try
					{
						hashtable3 = Utils.GetValuesWithRangeRetrieval(directoryEntry2, filter, arrayList3, SearchScope.OneLevel);
					}
					...
					foreach (object obj in arrayList4)
					{
						...
						stringBuilder.Append("=");
						stringBuilder.Append(Utils.GetEscapedFilterValue(text2));
						stringBuilder.Append(")");
						stringBuilder2.Append("(");
						stringBuilder2.Append(PropertyManager.DistinguishedName);
						stringBuilder2.Append("=");
						stringBuilder2.Append(Utils.GetEscapedFilterValue(Utils.GetPartialDN(text2, 1)));
						stringBuilder2.Append(")");
					}
					foreach (object obj2 in arrayList5)
					{
						...
						stringBuilder3.Append("=");
						stringBuilder3.Append(Utils.GetEscapedFilterValue(text3));
						stringBuilder3.Append(")");
						stringBuilder4.Append("(");
						stringBuilder4.Append(PropertyManager.DistinguishedName);
						stringBuilder4.Append("=");
						stringBuilder4.Append(Utils.GetEscapedFilterValue(Utils.GetPartialDN(text3, 1)));
						stringBuilder4.Append(")");
					}
				}
			...
			try
			{
				...
				if (siteName != null)
				{
					dn2 = "CN=Servers,CN=" + siteName + ",CN=Sites," + text;
				}
				else
				{
					dn2 = "CN=Sites," + text;
				}
				directoryEntry3 = DirectoryEntryManager.GetDirectoryEntry(context, dn2);
				string filter2;
				if (stringBuilder.ToString().Length == 0)
				{
					if (isDefaultNC)
					{
						filter2 = string.Concat(new string[]
						{
							"(|(&(",
							PropertyManager.ObjectCategory,
							"=nTDSDSA)(",
				...
				ADSearcher adsearcher = new ADSearcher(directoryEntry3, filter2, new string[0], SearchScope.Subtree);
				SearchResultCollection searchResultCollection = null;
				bool flag2 = false;
				ArrayList arrayList6 = new ArrayList();
				int num = 0;
				string text4 = PropertyManager.MsDSHasInstantiatedNCs + ";range=0-*";
				...
				adsearcher.PropertiesToLoad.Add(text4);
				...
				try
				{
					string text5 = "CN=NTDS-DSA";
					string text6 = "CN=NTDS-DSA-RO";
					using (SearchResultCollection searchResultCollection = adsearcher.FindAll())
					{
						foreach (object obj3 in searchResultCollection)
						{
							...
							string text7 = (string)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.ObjectCategory);
							if (text7.Length >= text5.Length && Utils.Compare(text7, 0, text5.Length, text5, 0, text5.Length) == 0)
							{
								string text8 = (string)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.DistinguishedName);
								if (flag)
								{
									if (text7.Length < text6.Length || Utils.Compare(text7, 0, text6.Length, text6, 0, text6.Length) != 0)
									{
										string text9 = null;
										if (!searchResult.Properties.Contains(text4))
										{
											using (IEnumerator enumerator2 = searchResult.Properties.PropertyNames.GetEnumerator())
											{
												while (enumerator2.MoveNext())
												{
													...
													if (text10.Length >= PropertyManager.MsDSHasInstantiatedNCs.Length && Utils.Compare(text10, 0, PropertyManager.MsDSHasInstantiatedNCs.Length, PropertyManager.MsDSHasInstantiatedNCs, 0, PropertyManager.MsDSHasInstantiatedNCs.Length) == 0)
													{
														text9 = text10;
														break;
													}
										...
										bool flag3 = false;
										int num2 = 0;
										foreach (object obj5 in searchResult.Properties[text9])
										{
											string text11 = (string)obj5;
											if (text11.Length - 13 >= partitionName.Length && Utils.Compare(text11, 13, partitionName.Length, partitionName, 0, partitionName.Length) == 0)
											{
												flag3 = true;
												if (string.Compare(text11, 10, "0", 0, 1, StringComparison.OrdinalIgnoreCase) == 0)
												{
													arrayList.Add(text8);
													if (isADAM)
													{
														hashtable2.Add(text8, (int)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.MsDSPortLDAP));
														break;
													}
													break;
												}
										...
										if (!flag3 && text9.Length >= text4.Length && Utils.Compare(text9, 0, text4.Length, text4, 0, text4.Length) != 0)
										{
											flag2 = true;
					...
					if (flag2)
					{
						do
						{
							...
							{
								string filterValue = (string)obj6;
								stringBuilder5.Append("(");
								stringBuilder5.Append(PropertyManager.NCName);
								stringBuilder5.Append("=");
								stringBuilder5.Append(Utils.GetEscapedFilterValue(filterValue));
								stringBuilder5.Append(")");
							}
							if (arrayList6.Count > 1)
							{
							...
							string text12 = string.Concat(new object[]
							{
							...
							adsearcher.PropertiesToLoad.Clear();
							adsearcher.PropertiesToLoad.Add(text12);
							adsearcher.PropertiesToLoad.Add(PropertyManager.DistinguishedName);
							...
							try
							{
								foreach (object obj7 in searchResultCollection2)
								{
									...
									string text13 = (string)PropertyManager.GetSearchResultPropertyValue(searchResult2, PropertyManager.DistinguishedName);
									string text14 = null;
									if (!searchResult2.Properties.Contains(text12))
									{
										using (IEnumerator enumerator2 = searchResult2.Properties.PropertyNames.GetEnumerator())
										{
											while (enumerator2.MoveNext())
											{
												...
												string text15 = (string)obj8;
												if (string.Compare(text15, 0, PropertyManager.MsDSHasInstantiatedNCs, 0, PropertyManager.MsDSHasInstantiatedNCs.Length, StringComparison.OrdinalIgnoreCase) == 0)
												{
													text14 = text15;
													break;
												}
									...
									bool flag4 = false;
									int num3 = 0;
									foreach (object obj9 in searchResult2.Properties[text14])
									{
										string text16 = (string)obj9;
										if (text16.Length - 13 >= partitionName.Length && Utils.Compare(text16, 13, partitionName.Length, partitionName, 0, partitionName.Length) == 0)
										{
											flag4 = true;
											if (string.Compare(text16, 10, "0", 0, 1, StringComparison.OrdinalIgnoreCase) == 0)
											{
												arrayList.Add(text13);
												if (isADAM)
												{
													hashtable2.Add(text13, (int)PropertyManager.GetSearchResultPropertyValue(searchResult2, PropertyManager.MsDSPortLDAP));
													break;
												}
												break;
											}
									...
									if (!flag4 && text14.Length >= text12.Length && Utils.Compare(text14, 0, text12.Length, text12, 0, text12.Length) != 0)
									{
										flag2 = true;
										arrayList6.Add(text13);
										num += num3;
										continue;
									}
									continue;
				...
				{
					if (ex.ErrorCode == -2147016656 && siteName != null)
					{
						return arrayList2;
					}
			...
			{
				string text17 = (string)obj10;
				string text18 = (string)hashtable[text17];
				if (text18 == null)
				{
					if (isADAM)
					{
					...
					throw new ActiveDirectoryOperationException(Res.GetString("NoHostName", new object[]
					{
						text17
					}));
				}
				...
				{
					if (isADAM && hashtable2[text17] == null)
					{
						throw new ActiveDirectoryOperationException(Res.GetString("NoHostNameOrPortNumber", new object[]
						{
							text17
					...
					if (isADAM)
					{
						arrayList2.Add(text18 + ":" + (int)hashtable2[text17]);
					}
					else
					{
		...
		internal static string GetEscapedFilterValue(string filterValue)
		{
			...
			int num = filterValue.IndexOfAny(anyOf);
			if (num != -1)
			{
		...
		internal static string GetNtAuthorityString()
		{
			if (Utils.NTAuthorityString == null)
			{
				...
				Utils.NTAuthorityString = ntaccount.Value.Substring(0, length);
			}
