...
namespace System.DirectoryServices.ActiveDirectory
{
	...
	internal class DirectoryEntryManager
	{
		...
		internal void RemoveIfExists(string distinguishedName)
		{
			...
			if (this.directoryEntries.ContainsKey(key))
			{
				DirectoryEntry directoryEntry = (DirectoryEntry)this.directoryEntries[key];
		...
		private DirectoryEntry GetNewDirectoryEntry(string dn)
		{
			...
			return DirectoryEntryManager.Bind(this.bindingPrefix + str, this.context.UserName, this.context.Password, this.context.useServerBind());
		}
		...
		internal string ExpandWellKnownDN(WellKnownDN dn)
		{
			...
			case WellKnownDN.DefaultNamingContext:
			{
				...
				result = (string)PropertyManager.GetPropertyValue(this.context, cachedDirectoryEntry, PropertyManager.DefaultNamingContext);
				break;
			}
			case WellKnownDN.SchemaNamingContext:
			{
				DirectoryEntry cachedDirectoryEntry2 = this.GetCachedDirectoryEntry("RootDSE");
				result = (string)PropertyManager.GetPropertyValue(this.context, cachedDirectoryEntry2, PropertyManager.SchemaNamingContext);
				break;
			}
			case WellKnownDN.ConfigurationNamingContext:
			{
				DirectoryEntry cachedDirectoryEntry3 = this.GetCachedDirectoryEntry("RootDSE");
				result = (string)PropertyManager.GetPropertyValue(this.context, cachedDirectoryEntry3, PropertyManager.ConfigurationNamingContext);
				break;
			}
			case WellKnownDN.PartitionsContainer:
				result = "CN=Partitions," + this.ExpandWellKnownDN(WellKnownDN.ConfigurationNamingContext);
				break;
			case WellKnownDN.SitesContainer:
				result = "CN=Sites," + this.ExpandWellKnownDN(WellKnownDN.ConfigurationNamingContext);
				break;
			case WellKnownDN.SystemContainer:
				result = "CN=System," + this.ExpandWellKnownDN(WellKnownDN.DefaultNamingContext);
				break;
			case WellKnownDN.RidManager:
				result = "CN=RID Manager$," + this.ExpandWellKnownDN(WellKnownDN.SystemContainer);
				break;
			case WellKnownDN.Infrastructure:
				result = "CN=Infrastructure," + this.ExpandWellKnownDN(WellKnownDN.DefaultNamingContext);
				break;
			case WellKnownDN.RootDomainNamingContext:
			{
				DirectoryEntry cachedDirectoryEntry4 = this.GetCachedDirectoryEntry("RootDSE");
				result = (string)PropertyManager.GetPropertyValue(this.context, cachedDirectoryEntry4, PropertyManager.RootDomainNamingContext);
				break;
			}
			default:
		...
		internal static DirectoryEntry GetDirectoryEntry(DirectoryContext context, string dn)
		{
			...
			string str2 = adsPathname.Retrieve(7);
			return DirectoryEntryManager.Bind(str + str2, context.UserName, context.Password, context.useServerBind());
		}
		...
		internal static string ExpandWellKnownDN(DirectoryContext context, WellKnownDN dn)
		{
			...
			IL_EF:
			result = "CN=Partitions," + DirectoryEntryManager.ExpandWellKnownDN(context, WellKnownDN.ConfigurationNamingContext);
			return result;
		}
