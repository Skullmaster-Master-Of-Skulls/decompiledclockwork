	...
	public class DirectoryContext
	{
			...
			if (osversion.Platform == PlatformID.Win32NT && osversion.Version.Major >= 5)
			{
				if (osversion.Version.Major == 5 && osversion.Version.Minor == 0)
				{
					...
					if (!NativeMethods.GetVersionEx(osversionInfoEx))
					{
						...
						throw new SystemException(Res.GetString("VersionFailure", new object[]
						{
							lastError
						}));
					}
		...
		internal static bool IsContextValid(DirectoryContext context, DirectoryContextType contextType)
		{
			...
			else
			{
				...
				{
					string text2;
					string str = Utils.SplitServerNameAndPortNumber(context.Name, out text2);
					DirectoryEntry directoryEntry = new DirectoryEntry("WinNT://" + str + ",computer", context.UserName, context.Password, Utils.DefaultAuthType);
					try
					{
						try
						{
		...
		internal static string GetLoggedOnDomain()
		{
			...
			int num = NativeMethods.LsaConnectUntrusted(out lsaHandle);
			if (num == 0)
			{
				negotiateCallerNameRequest.messageType = 1;
				int num2;
				int num3;
				num = NativeMethods.LsaCallAuthenticationPackage(lsaHandle, 0, negotiateCallerNameRequest, submitBufferLength, out zero, out num2, out num3);
				try
				{
					if (num == 0 && num3 == 0)
					{
						Marshal.PtrToStructure(zero, negotiateCallerNameResponse);
						int length = negotiateCallerNameResponse.callerName.IndexOf('\\');
						text = negotiateCallerNameResponse.callerName.Substring(0, length);
					}
					else
					{
						if (num == -1073741756)
						...
						if (num != 0 || UnsafeNativeMethods.LsaNtStatusToWinError(num3) != 1312)
						{
							throw ExceptionHelper.GetExceptionFromErrorCode(UnsafeNativeMethods.LsaNtStatusToWinError((num != 0) ? num : num3));
						}
						WindowsIdentity current = WindowsIdentity.GetCurrent();
						int length2 = current.Name.IndexOf('\\');
						text = current.Name.Substring(0, length2);
					}
					goto IL_109;
				}
				finally
