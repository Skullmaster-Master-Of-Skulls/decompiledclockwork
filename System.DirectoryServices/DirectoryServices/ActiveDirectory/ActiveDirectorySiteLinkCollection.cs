	...
	public class ActiveDirectorySiteLinkCollection : CollectionBase
	{
		...
		public ActiveDirectorySiteLink this[int index]
		{
			...
			set
			{
				...
				if (!value.existing)
				{
					throw new InvalidOperationException(Res.GetString("SiteLinkNotCommitted", new object[]
					{
						value.Name
					}));
				}
		...
		protected override void OnInsertComplete(int index, object value)
		{
			if (this.initialized)
			{
				...
				try
				{
					this.de.Properties["siteLinkList"].Add(value2);
				}
				catch (COMException e)
				{
					throw ExceptionHelper.GetExceptionFromCOMException(this.context, e);
		...
		protected override void OnRemoveComplete(int index, object value)
		{
			...
			try
			{
				this.de.Properties["siteLinkList"].Remove(value2);
			}
			catch (COMException e)
			{
				throw ExceptionHelper.GetExceptionFromCOMException(this.context, e);
		...
		protected override void OnSetComplete(int index, object oldValue, object newValue)
		{
			...
			try
			{
				this.de.Properties["siteLinkList"][index] = value;
			}
			catch (COMException e)
			{
				throw ExceptionHelper.GetExceptionFromCOMException(this.context, e);
		...
		protected override void OnValidate(object value)
		{
			...
			if (!((ActiveDirectorySiteLink)value).existing)
			{
				throw new InvalidOperationException(Res.GetString("SiteLinkNotCommitted", new object[]
				{
					((ActiveDirectorySiteLink)value).Name
				}));
			}
		}
