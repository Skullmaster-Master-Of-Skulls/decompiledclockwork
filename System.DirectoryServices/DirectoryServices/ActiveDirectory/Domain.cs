...
namespace System.DirectoryServices.ActiveDirectory
{
	...
	public class Domain : ActiveDirectoryPartition
	{
		...
		public static Domain GetDomain(DirectoryContext context)
		{
			...
			if (context.Name == null || context.isDomain() || context.isServer())
			{
				...
				try
				{
					...
					distinguishedName = (string)PropertyManager.GetPropertyValue(context, cachedDirectoryEntry, PropertyManager.DefaultNamingContext);
				}
				...
				return new Domain(context, Utils.GetDnsNameFromDN(distinguishedName), directoryEntryManager);
			}
			if (context.ContextType == DirectoryContextType.Domain)
			{
				throw new ActiveDirectoryObjectNotFoundException(Res.GetString("DomainNotFound"), typeof(Domain), context.Name);
		...
		public DomainControllerCollection FindAllDiscoverableDomainControllers()
		{
			long dcFlags = 4096L;
			base.CheckIfDisposed();
			return new DomainControllerCollection(Locator.EnumerateDomainControllers(this.context, base.Name, null, dcFlags));
		}
		...
		public DomainControllerCollection FindAllDiscoverableDomainControllers(string siteName)
		{
			...
			return new DomainControllerCollection(Locator.EnumerateDomainControllers(this.context, base.Name, siteName, dcFlags));
		}
		...
		public TrustRelationshipInformation GetTrustRelationship(string targetDomainName)
		{
			...
			{
				throw new ActiveDirectoryObjectNotFoundException(Res.GetString("DomainTrustDoesNotExist", new object[]
				{
					base.Name,
					targetDomainName
		...
		public void CreateTrustRelationship(Domain targetDomain, TrustDirection direction)
		{
			...
			TrustHelper.CreateTrust(this.context, base.Name, targetDomain.GetDirectoryContext(), targetDomain.Name, false, direction, password);
			int num = 0;
			if ((direction & TrustDirection.Inbound) != (TrustDirection)0)
			{
				num |= 2;
			...
			TrustHelper.CreateTrust(targetDomain.GetDirectoryContext(), targetDomain.Name, this.context, base.Name, false, (TrustDirection)num, password);
		}
		...
		public void UpdateTrustRelationship(Domain targetDomain, TrustDirection newTrustDirection)
		{
			...
			string password = TrustHelper.CreateTrustPassword();
			TrustHelper.UpdateTrustDirection(this.context, base.Name, targetDomain.Name, password, false, newTrustDirection);
			...
			TrustHelper.UpdateTrustDirection(targetDomain.GetDirectoryContext(), targetDomain.Name, base.Name, password, false, trustDirection);
		}
		...
		public Forest Forest
		{
			get
			{
				...
				if (this.cachedForest == null)
				{
					...
					string distinguishedName = (string)PropertyManager.GetPropertyValue(this.context, cachedDirectoryEntry, PropertyManager.RootDomainNamingContext);
		...
		private DomainMode GetDomainMode()
		{
			...
			try
			{
				...
				case 0:
					...
					{
						result = DomainMode.Windows2000NativeDomain;
					}
					else
					{
						result = DomainMode.Windows2000MixedDomain;
					}
					break;
				case 1:
					result = DomainMode.Windows2003InterimDomain;
					break;
				case 2:
					result = DomainMode.Windows2003Domain;
					break;
				case 3:
					result = DomainMode.Windows2008Domain;
					break;
				case 4:
					result = DomainMode.Windows2008R2Domain;
					break;
				default:
		...
		private Domain GetParent()
		{
			...
			if (this.trustParent != null)
			{
				...
				try
				{
					...
					context = Utils.GetNewDirectoryContext(text, DirectoryContextType.Domain, this.context);
				}
				finally
				{
					directoryEntry.Dispose();
				}
				return new Domain(context, text);
			}
			return null;
		}
					...
					for (int k = 0; k < arrayList2.Count; k++)
					{
						...
						if (k != num2 && trustObject2.TrustType != TrustType.Kerberos)
						{
							...
							else if ((trustObject2.Flags & 1) != 0)
							{
								...
								{
									string distinguishedName = this.directoryEntryMgr.ExpandWellKnownDN(WellKnownDN.RootDomainNamingContext);
									string dnsNameFromDN = Utils.GetDnsNameFromDN(distinguishedName);
		...
		private void RepairTrustHelper(Domain targetDomain, TrustDirection direction)
		{
			...
			string preferredTargetServer2 = TrustHelper.UpdateTrust(this.context, base.Name, targetDomain.Name, password, false);
			if ((direction & TrustDirection.Outbound) != (TrustDirection)0)
			{
				try
				{
					TrustHelper.VerifyTrust(this.context, base.Name, targetDomain.Name, false, TrustDirection.Outbound, true, preferredTargetServer);
				}
				catch (ActiveDirectoryObjectNotFoundException)
				{
					throw new ActiveDirectoryObjectNotFoundException(Res.GetString("WrongTrustDirection", new object[]
			...
			if ((direction & TrustDirection.Inbound) != (TrustDirection)0)
			{
				try
				{
					TrustHelper.VerifyTrust(targetDomain.GetDirectoryContext(), targetDomain.Name, base.Name, false, TrustDirection.Outbound, true, preferredTargetServer2);
				}
				catch (ActiveDirectoryObjectNotFoundException)
				{
					throw new ActiveDirectoryObjectNotFoundException(Res.GetString("WrongTrustDirection", new object[]
