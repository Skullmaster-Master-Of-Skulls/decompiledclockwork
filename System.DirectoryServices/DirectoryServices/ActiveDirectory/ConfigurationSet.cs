	...
	public class ConfigurationSet
	{
		...
		private static DirectoryEntry GetSearchRootEntry(Forest forest)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					result = new DirectoryEntry("GC://" + directoryContext.GetServerName(), directoryContext.UserName, directoryContext.Password, authenticationTypes);
				}
				else
				{
					result = new DirectoryEntry("LDAP://" + directoryContext.GetServerName(), directoryContext.UserName, directoryContext.Password, authenticationTypes);
				}
			}
			else
			{
				result = new DirectoryEntry("GC://" + forest.Name, directoryContext.UserName, directoryContext.Password, authenticationTypes);
			}
			return result;
		}
		...
		internal static AdamInstance FindAnyAdamInstance(DirectoryContext context)
		{
			if (context.ContextType == DirectoryContextType.ConfigurationSet)
			{
				...
				try
				{
					...
					ADSearcher adsearcher = new ADSearcher(searchRootEntry, filter, new string[]
					{
					...
					try
					{
						foreach (object obj in searchResultCollection)
						{
							...
							foreach (object obj2 in searchResult.Properties[PropertyManager.ServiceBindingInformation])
							{
								...
								if (text3.Length > text2.Length && string.Compare(text3.Substring(0, text2.Length), text2, StringComparison.OrdinalIgnoreCase) == 0)
								{
									arrayList.Add(text3.Substring(text2.Length));
								}
							}
						}
					}
		...
		internal static AdamInstance FindAliveAdamInstance(string configSetName, DirectoryContext context, ArrayList adamInstanceNames)
		{
			...
			foreach (object obj in adamInstanceNames)
			{
				...
				try
				{
					...
					result = new AdamInstance(newDirectoryContext, text, directoryEntryManager, true);
					flag = true;
				}
				catch (COMException ex)
				{
					...
					{
						throw new ActiveDirectoryObjectNotFoundException(Res.GetString("ADAMInstanceNotFoundInConfigSet", new object[]
						{
							(configSetName != null) ? configSetName : context.Name
		...
		private ArrayList GetSites()
		{
			...
			ADSearcher adsearcher = new ADSearcher(cachedDirectoryEntry, filter, new string[]
			{
				PropertyManager.Cn
			...
			try
			{
				...
				foreach (object obj in searchResultCollection)
				{
					...
					arrayList.Add(new ActiveDirectorySite(this.context, (string)PropertyManager.GetSearchResultPropertyValue(res, PropertyManager.Cn), true));
				}
			}
			catch (COMException e)
			{
		...
		private ArrayList GetApplicationPartitions()
		{
			...
			stringBuilder.Append(")))");
			string filter = stringBuilder.ToString();
			ADSearcher adsearcher = new ADSearcher(cachedDirectoryEntry2, filter, new string[]
			{
			...
			{
				searchResultCollection = adsearcher.FindAll();
				string value = (string)PropertyManager.GetPropertyValue(this.context, cachedDirectoryEntry, PropertyManager.SchemaNamingContext);
				string value2 = (string)PropertyManager.GetPropertyValue(this.context, cachedDirectoryEntry, PropertyManager.ConfigurationNamingContext);
				foreach (object obj in searchResultCollection)
				{
					SearchResult searchResult = (SearchResult)obj;
					string text = (string)PropertyManager.GetSearchResultPropertyValue(searchResult, PropertyManager.NCName);
