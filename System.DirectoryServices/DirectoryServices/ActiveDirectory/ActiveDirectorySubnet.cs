...
namespace System.DirectoryServices.ActiveDirectory
{
	...
	public class ActiveDirectorySubnet : IDisposable
	{
		...
		public static ActiveDirectorySubnet FindByName(DirectoryContext context, string subnetName)
		{
			...
			ActiveDirectorySubnet result;
			try
			{
				ADSearcher adsearcher = new ADSearcher(directoryEntry, "(&(objectClass=subnet)(objectCategory=subnet)(name=" + Utils.GetEscapedFilterValue(subnetName) + "))", new string[]
				{
					"distinguishedName"
				...
				{
					Exception ex = new ActiveDirectoryObjectNotFoundException(Res.GetString("DSNotFound"), typeof(ActiveDirectorySubnet), subnetName);
					throw ex;
				}
				string text = null;
				...
				{
					activeDirectorySubnet = new ActiveDirectorySubnet(context, subnetName, text, true);
				}
				activeDirectorySubnet.cachedEntry = directoryEntry2;
				result = activeDirectorySubnet;
			}
		...
		public ActiveDirectorySubnet(DirectoryContext context, string subnetName)
		{
			...
			try
			{
				...
				this.cachedEntry = directoryEntry.Children.Add(originalPath, "subnet");
			}
			catch (COMException e)
			{
		...
		public ActiveDirectorySubnet(DirectoryContext context, string subnetName, string siteName) : this(context, subnetName)
		{
			...
			{
				throw new ArgumentException(Res.GetString("SiteNotExist", new object[]
				{
					siteName
				}), "siteName");
			}
		...
		public ActiveDirectorySite Site
		{
			...
			set
			{
				...
				{
					throw new InvalidOperationException(Res.GetString("SiteNotCommitted", new object[]
					{
						value
					}));
				}
