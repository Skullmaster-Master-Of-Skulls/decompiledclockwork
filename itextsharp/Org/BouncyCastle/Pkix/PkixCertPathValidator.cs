...
namespace Org.BouncyCastle.Pkix
{
	...
	{
		// Token: 0x06000EE0 RID: 3808 RVA: 0x000566F0 File Offset: 0x000556F0
		public virtual PkixCertPathValidatorResult Validate(PkixCertPath certPath, PkixParameters paramsPkix)
		{
			...
			int i = 0;
			IList[] array = new ArrayList[count + 1];
			...
			PkixPolicyNode pkixPolicyNode = new PkixPolicyNode(new ArrayList(), 0, set, null, new HashSet(), Rfc3280CertPathUtilities.ANY_POLICY, false);
			...
			try
			{
				algorithmIdentifier = PkixCertPathValidatorUtilities.GetAlgorithmIdentifier(asymmetricKeyParameter);
			}
			...
			DerObjectIdentifier objectID = algorithmIdentifier.ObjectID;
			...
			for (i = certificates.Count - 1; i >= 0; i--)
			{
				...
				if (num2 != count)
				{
					...
					algorithmIdentifier = PkixCertPathValidatorUtilities.GetAlgorithmIdentifier(asymmetricKeyParameter);
					DerObjectIdentifier objectID2 = algorithmIdentifier.ObjectID;
					Asn1Encodable parameters2 = algorithmIdentifier.Parameters;
				}
			}
