...
namespace System.Web.Configuration
{
	...
	internal sealed class ProcessHostMapPath : IConfigMapPath, IConfigMapPath2
	{
		...
		void IConfigMapPath.ResolveSiteArgument(string siteArgument, out string siteName, out string siteID)
		{
			...
			{
				uint num = UnsafeIISMethods.MgdResolveSiteName(siteArgument);
				if (num != 0U)
				{
		...
		private VirtualPath GetAppPathForPathWorker(string siteID, VirtualPath path)
		{
			...
			try
			{
				text = ((UnsafeIISMethods.MgdGetAppPathForPath(siteId, path.VirtualPathString, out zero, out num) == 0 && num > 0) ? StringUtil.StringFromWCharPtr(zero, num) : null);
			}
			finally
			{
				if (zero != IntPtr.Zero)
		...
		private string MapPathCaching(string siteID, VirtualPath path)
		{
			...
			MapPathCacheInfo mapPathCacheInfo = (MapPathCacheInfo)HttpRuntime.CacheInternal.Get(key);
			if (mapPathCacheInfo == null)
			{
				lock (this._mapPathCacheLock)
				{
					mapPathCacheInfo = (MapPathCacheInfo)HttpRuntime.CacheInternal.Get(key);
					if (mapPathCacheInfo == null)
					{
						mapPathCacheInfo = new MapPathCacheInfo();
						HttpRuntime.CacheInternal.UtcInsert(key, mapPathCacheInfo, null, DateTime.UtcNow.AddMinutes(10.0), Cache.NoSlidingExpiration);
					}
				}
			}
					...
					if (!mapPathCacheInfo.Evaluated)
					{
						try
						{
							...
							if (uint.TryParse(siteID, out siteId))
							{
								...
								text = ProcessHostConfigUtils.MapPathActual(siteNameFromId, path);
							}
