...
namespace System.Web.Configuration
{
	...
	internal static class ProcessHostConfigUtils
	{
		...
		internal static string MapPathActual(string siteName, VirtualPath path)
		{
			...
			try
			{
				int num = UnsafeIISMethods.MgdMapPathDirect(siteName, path.VirtualPathString, out zero, out length);
				if (num < 0)
				{
		...
		internal static string GetSiteNameFromId(uint siteId)
		{
			...
			try
			{
				result = ((UnsafeIISMethods.MgdGetSiteNameFromId(siteId, out zero, out length) == 0 && zero != IntPtr.Zero) ? StringUtil.StringFromWCharPtr(zero, length) : string.Empty);
			}
			finally
			{
				if (zero != IntPtr.Zero)
