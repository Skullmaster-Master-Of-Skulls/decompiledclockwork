...
namespace System.Web
{
	...
	public class HttpApplication : IHttpAsyncHandler, IHttpHandler, IComponent, IDisposable
	{
		...
		private HttpModuleCollection GetModuleCollection(IntPtr appContext)
		{
			...
			{
				int num4 = 0;
				int num5 = UnsafeIISMethods.MgdGetModuleCollection(appContext, out zero, out num4);
				if (num5 < 0)
				{
		...
		internal class PipelineStepManager : HttpApplication.StepManager
		{
			...
			internal override void BuildSteps(WaitCallback stepCallback)
			{
				...
				HttpApplication.IExecutionStep step3 = new HttpApplication.CallFilterExecutionStep(application);
				application.AddEventMapping("AspNetFilterModule", RequestNotification.UpdateRequestCache, false, step3);
				application.AddEventMapping("AspNetFilterModule", RequestNotification.LogRequest, false, step3);
				this._resumeStepsWaitCallback = stepCallback;
			}
				...
				try
				{
					...
					try
					{
						...
						for (;;)
						{
							...
							{
								status = (flag5 ? RequestNotificationStatus.FinishRequest : RequestNotificationStatus.Continue);
					...
					if (flag2)
					{
						this._application.AsyncResult.Complete(flag, null, null, status);
					}
				}
				finally
				{
