	...
	public class SqlMembershipProvider : MembershipProvider
	{
		...
		public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
		{
			...
			if (!string.IsNullOrEmpty(passwordAnswer))
			{
				...
				objValue = base.EncodePassword(passwordAnswer.ToLower(CultureInfo.InvariantCulture), (int)this._PasswordFormat, text);
			}
			else
			{
		...
		public override bool ChangePasswordQuestionAndAnswer(string username, string password, string newPasswordQuestion, string newPasswordAnswer)
		{
			...
			{
				objValue = base.EncodePassword(newPasswordAnswer.ToLower(CultureInfo.InvariantCulture), passwordFormat, salt);
			}
			else
			{
		...
		public override string GetPassword(string username, string passwordAnswer)
		{
			...
			{
				return base.UnEncodePassword(passwordFromDB, passwordFormat);
			}
		...
		public override string ResetPassword(string username, string passwordAnswer)
		{
			...
			else
			{
				...
				if (!string.IsNullOrEmpty(passwordAnswer))
				{
					objValue = base.EncodePassword(passwordAnswer.ToLower(CultureInfo.InvariantCulture), num2, text2);
				}
				else
				{
					objValue = passwordAnswer;
				...
				if (!validatePasswordEventArgs.Cancel)
				{
					...
					try
					{
						...
						try
						{
							...
							sqlCommand.Parameters.Add(this.CreateInputParam("@ApplicationName", SqlDbType.NVarChar, this.ApplicationName));
							sqlCommand.Parameters.Add(this.CreateInputParam("@UserName", SqlDbType.NVarChar, username));
							sqlCommand.Parameters.Add(this.CreateInputParam("@NewPassword", SqlDbType.NVarChar, base.EncodePassword(text3, num2, text2)));
		...
		private bool CheckPassword(string username, string password, bool updateLastLoginActivityDate, bool failIfNotApproved, out string salt, out int passwordFormat)
		{
			...
			string value = base.EncodePassword(password, passwordFormat, salt);
		...
		private string GetEncodedPasswordAnswer(string username, string passwordAnswer)
		{
			...
			if (num == 0)
			{
				return base.EncodePassword(passwordAnswer.ToLower(CultureInfo.InvariantCulture), passwordFormat, salt);
			}
			throw new ProviderException(this.GetExceptionText(num));
		}
