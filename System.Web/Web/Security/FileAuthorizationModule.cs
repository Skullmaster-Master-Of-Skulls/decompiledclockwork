...
namespace System.Web.Security
{
	...
	public sealed class FileAuthorizationModule : IHttpModule
	{
		...
		internal static FileSecurityDescriptorWrapper GetFileSecurityDescriptorWrapper(string fileName, out bool freeDescriptor)
		{
			...
			FileSecurityDescriptorWrapper fileSecurityDescriptorWrapper = HttpRuntime.CacheInternal.Get(key) as FileSecurityDescriptorWrapper;
			if (fileSecurityDescriptorWrapper == null)
			{
				...
				if (fileSecurityDescriptorWrapper.IsSecurityDescriptorValid())
				{
					try
					{
						...
						HttpRuntime.CacheInternal.UtcInsert(key, fileSecurityDescriptorWrapper, dependencies, Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration, CacheItemPriority.Normal, new CacheItemRemovedCallback(fileSecurityDescriptorWrapper.OnCacheItemRemoved));
					}
					catch
					{
						freeDescriptor = true;
		...
		internal static bool RequestRequiresAuthorization(HttpContext context)
		{
			...
			object obj = HttpRuntime.CacheInternal.Get(key);
