		...
		private void BuildBuildMethodInternal(ControlBuilder builder, Type ctrlType, bool fInTemplate, bool topLevelControlInTemplate, PropertyEntry pse, CodeStatementCollection statements, bool fStandardControl, bool fControlFieldDeclared, string deviceFilter, bool fControlSkin)
		{
				...
				foreach (object obj5 in builder.SubBuilders)
				{
					if (obj5 is ControlBuilder && !(obj5 is CodeBlockBuilder) && !(obj5 is ContentBuilderInternal))
					{
						...
						if (partialCachingAttribute == null)
						{
							...
							CodeTypeReference type2 = new CodeTypeReference(controlBuilder.ControlType, CodeTypeReferenceOptions.GlobalReference);
					...
					else if (obj5 is string && !builder.HasAspCode && (!fControlSkin || !builder.AllowWhitespaceLiterals()))
					{
						...
						if (!this.UseResourceLiteralString(text3))
						{
						...
						else
						{
							...
							int num4;
							bool flag2;
							this._stringResourceBuilder.AddString(text3, out num3, out num4, out flag2);
			...
			if (builder.ComplexPropertyEntries.Count > 0)
			{
				...
				foreach (object obj6 in builder.ComplexPropertyEntries)
				{
					...
					{
						string text4 = "__ctrl" + num5++.ToString(CultureInfo.InvariantCulture);
						CodeTypeReference type3 = new CodeTypeReference(complexPropertyEntry.Builder.ControlType, CodeTypeReferenceOptions.GlobalReference);
						codeStatementCollection9.Add(new CodeVariableDeclarationStatement(type3, text4));
						CodeVariableReferenceExpression codeVariableReferenceExpression2 = new CodeVariableReferenceExpression(text4);
		...
		protected CodeMemberMethod BuildPropertyBindingMethod(ControlBuilder builder, bool fControlSkin)
		{
			if (builder is DataBoundLiteralControlBuilder)
			{
				...
				foreach (object obj in builder.SubBuilders)
				{
					...
					if (obj != null && num % 2 != 0)
					{
						...
						{
							CodeExpression value = CodeDomUtility.GenerateConvertToString(new CodeSnippetExpression(codeBlockBuilder.Content.Trim()));
				...
				{
					CodeStatement value3 = (CodeStatement)obj3;
			...
			foreach (object obj4 in builder.BoundPropertyEntries)
			{
				...
				if ((!boundPropertyEntry.TwoWayBound || (!flag2 && !boundPropertyEntry.ReadOnlyProperty)) && boundPropertyEntry.IsDataBindingEntry)
				{
					...
					if (boundPropertyEntry.TwoWayBound)
					{
						DataBindingExpressionBuilder.BuildEvalExpression(boundPropertyEntry.FieldName, boundPropertyEntry.FormatString, boundPropertyEntry.Name, boundPropertyEntry.Type, builder, codeStatementCollection3, statements, linePragma2, ref flag4);
					}
					else
					{
						DataBindingExpressionBuilder.BuildExpressionStatic(boundPropertyEntry, builder, null, codeStatementCollection3, statements, linePragma2, ref flag4);
					}
				}
			}
			if (codeStatementCollection3 != null)
			{
				...
				{
					CodeStatement value4 = (CodeStatement)obj5;
			...
			if (codeStatementCollection4 != null)
			{
				...
				{
					CodeStatement value5 = (CodeStatement)obj6;
					codeMemberMethod2.Statements.Add(value5);
				}
			}
		...
		protected virtual void BuildSourceDataTreeFromBuilder(ControlBuilder builder, bool fInTemplate, bool topLevelControlInTemplate, PropertyEntry pse)
		{
			...
			foreach (object obj2 in builder.TemplatePropertyEntries)
			{
				...
				if (templatePropertyEntry.PropertyInfo != null)
				{
					...
					{
						fInTemplate2 = (templateInstanceAttribute.Instances == TemplateInstance.Multiple);
					}
				}
