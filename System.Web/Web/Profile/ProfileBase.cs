	...
	public class ProfileBase : SettingsBase
	{
		...
		internal static Hashtable GetPropertiesForCompilation()
		{
			...
			lock (ProfileBase.s_InitializeLock)
			{
				...
				ProfileBase.AddProfilePropertySettingsForCompilation(profile.PropertySettings, ht, null);
								...
								foreach (PropertyInfo propertyInfo2 in properties2)
								{
									if (nameValueCollection[propertyInfo2.Name] == null)
									{
										...
										foreach (Attribute attribute in customAttributes)
										{
											...
											{
												flag3 = ((SettingsAllowAnonymousAttribute)attribute).Allow;
												if (!flag && flag3)
												{
													throw new ConfigurationErrorsException(SR.GetString("Annoymous_id_module_not_enabled", new object[]
													{
														propertyInfo2.Name
													}), profile.ElementInformation.Properties["inherits"].Source, profile.ElementInformation.Properties["inherits"].LineNumber);
												}
											}
										...
										settingsAttributeDictionary.Add("AllowAnonymous", flag3);
