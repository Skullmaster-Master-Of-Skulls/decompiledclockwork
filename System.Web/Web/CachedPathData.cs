	...
	internal class CachedPathData
	{
		...
		private static CachedPathData GetConfigPathData(string configPath)
		{
			...
			try
			{
				...
				{
					cachedPathData2 = new CachedPathData(configPath, virtualPath, text, flag);
					try
					{
					}
					finally
					{
						cachedPathData = (CachedPathData)cacheInternal.UtcAdd(key, cachedPathData2, cacheDependency, Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration, priority, CachedPathData.s_callback);
						if (cachedPathData == null)
						{
							flag4 = true;
						}
				...
				lock (cachedPathData2)
				{
					try
					{
						cachedPathData2.Init(parentData);
						flag5 = true;
					}
					finally
					{
						cachedPathData2._flags[1] = true;
						Monitor.PulseAll(cachedPathData2);
						if (cachedPathData2._flags[64])
						{
							cachedPathData2.Close();
						}
					}
				}
			}
			finally
			{
				if (flag4)
				{
					...
					if (!flag5 || (cachedPathData2.ConfigRecord != null && cachedPathData2.ConfigRecord.HasInitErrors))
					{
						...
						using (cacheDependency)
						{
							cacheInternal.UtcInsert(key, cachedPathData2, cacheDependency, DateTime.UtcNow.AddSeconds(5.0), Cache.NoSlidingExpiration, CacheItemPriority.Normal, CachedPathData.s_callback);
						}
					}
				}
			}
