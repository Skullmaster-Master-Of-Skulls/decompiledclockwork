...
namespace System.Web
{
	...
	public sealed class HttpRuntime
	{
		...
		private static void StaticInit()
		{
			...
			if (!flag)
			{
				string libFilename = runtimeDirectory + Path.DirectorySeparatorChar + "webengine.dll";
		...
		private void HostingInit(HostingEnvironmentFlags hostingFlags)
		{
			using (new ApplicationImpersonationContext())
			{
				try
				{
					...
					CompilationSection upCodegenDirectory;
					HostingEnvironmentSection hostingEnvironmentSection;
					Exception ex;
					this.GetInitConfigSections(out cacheSection, out trustSection, out securityPolicySection, out upCodegenDirectory, out hostingEnvironmentSection, out ex);
					HttpRuntime.CacheInternal.ReadCacheInternalConfig(cacheSection);
					this.SetUpCodegenDirectory(upCodegenDirectory);
					try
					{
		...
		private void SetUpCodegenDirectory(CompilationSection compilationSection)
		{
			...
			string path = AppManagerAppDomainFactory.ConstructSimpleAppName(HttpRuntime.AppDomainAppVirtualPath);
			string text = null;
			string text2 = null;
		...
		private void ReleaseResourcesAndUnloadAppDomain(object state)
		{
			...
			{
				HttpRuntime.AddAppDomainTraceMessage("Unload Exception: " + arg);
				throw;
			}
		}
		...
		private void CoalesceNotifications()
		{
			...
			try
			{
				while (DateTime.UtcNow < t && !(DateTime.UtcNow > this.LastShutdownAttemptTime.AddSeconds((double)num)))
				{
					Thread.Sleep(250);
				}
			}
		...
		public static Cache Cache
		{
			get
			{
				...
				Cache cachePublic = HttpRuntime._theRuntime._cachePublic;
				if (cachePublic == null)
				{
					CacheInternal cacheInternal = HttpRuntime.CacheInternal;
					CacheSection cache = RuntimeConfig.GetAppConfig().Cache;
					cacheInternal.ReadCacheInternalConfig(cache);
					HttpRuntime._theRuntime._cachePublic = cacheInternal.CachePublic;
					cachePublic = HttpRuntime._theRuntime._cachePublic;
				}
				return cachePublic;
			}
		}
