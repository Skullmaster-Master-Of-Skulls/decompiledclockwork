...
namespace System.Web.SessionState
{
	...
	internal class StateApplication : IHttpHandler
	{
		...
		internal void DoGet(HttpContext context)
		{
			...
			CacheEntry cacheEntry = (CacheEntry)HttpRuntime.CacheInternal.Get(key, CacheGetOptions.ReturnCacheEntry);
			if (cacheEntry == null)
			{
		...
		private unsafe IntPtr FinishPut(HttpContext context)
		{
			...
			string key = this.CreateKey(request);
			CacheEntry cacheEntry = (CacheEntry)cacheInternal.Get(key, CacheGetOptions.ReturnCacheEntry);
			if (cacheEntry != null)
			{
				if ((1 & num3) == 1)
				{
			...
			CachedContent value = new CachedContent(array, intPtr, false, DateTime.MinValue, lockCookie, num3);
			cacheInternal.UtcInsert(key, value, null, Cache.NoAbsoluteExpiration, timeSpan, CacheItemPriority.NotRemovable, this._removedHandler);
			if (cacheEntry == null)
			{
		...
		internal void DoDelete(HttpContext context)
		{
			...
			CacheInternal cacheInternal = HttpRuntime.CacheInternal;
			CachedContent cachedContent = (CachedContent)cacheInternal.Get(key);
