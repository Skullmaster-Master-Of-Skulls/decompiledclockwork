...
namespace System.Web.SessionState
{
	...
	internal sealed class OutOfProcSessionStateStore : SessionStateStoreProviderBase
	{
		...
		internal IPartitionInfo CreatePartitionInfo(string stateConnectionString)
		{
			...
			return new OutOfProcSessionStateStore.StateServerPartitionInfo(new ResourcePool(new TimeSpan(0, 0, 5), int.MaxValue), text, port);
		}
		...
		private void MakeRequest(UnsafeNativeMethods.StateProtocolVerb verb, string id, UnsafeNativeMethods.StateProtocolExclusive exclusiveAccess, int extraFlags, int timeout, int lockCookie, byte[] buf, int cb, int networkTimeout, out UnsafeNativeMethods.SessionNDMakeRequestResults results)
		{
			...
			try
			{
				...
				num = UnsafeNativeMethods.SessionNDMakeRequest(socket, this._partitionInfo.Server, this._partitionInfo.Port, networkTimeout, verb, uri, exclusiveAccess, extraFlags, timeout, lockCookie, buf, cb, flag, out results);
				if (outOfProcConnection != null)
				{
			...
			try
			{
				...
				else
				{
					...
					if (contentLength > 0)
					{
						...
						result = SessionStateUtility.Deserialize(context, unmanagedMemoryStream);
		...
		public override void SetAndReleaseItemExclusive(HttpContext context, string id, SessionStateStoreData item, object lockId, bool newItem)
		{
			...
			try
			{
				SessionStateUtility.SerializeStoreData(item, 0, out buf, out cb);
			}
			catch
			{
				if (!newItem)
		...
		public override void CreateUninitializedItem(HttpContext context, string id, int timeout)
		{
			byte[] buf;
			int cb;
			SessionStateUtility.SerializeStoreData(this.CreateNewStoreData(context, timeout), 0, out buf, out cb);
			UnsafeNativeMethods.SessionNDMakeRequestResults sessionNDMakeRequestResults;
