	...
	internal class SqlSessionStateStore : SessionStateStoreProviderBase
	{
				...
				else
				{
					...
					else
					{
						...
						try
						{
							...
							sessionStateStoreData = SessionStateUtility.Deserialize(context, memoryStream);
		...
		public override void ReleaseItemExclusive(HttpContext context, string id, object lockId)
		{
			...
			try
			{
				...
				{
					SqlSessionStateStore.ThrowSqlConnectionException(sqlStateConnection.Connection, e);
				}
			}
			finally
			{
		...
		public override void SetAndReleaseItemExclusive(HttpContext context, string id, SessionStateStoreData item, object lockId, bool newItem)
		{
			...
			try
			{
				...
				try
				{
					SessionStateUtility.SerializeStoreData(item, 7000, out value, out num);
				}
				catch
				{
					if (!newItem)
				...
				{
					this.HandleInsertException(sqlStateConnection.Connection, e, newItem, id);
				}
			}
			finally
			{
		...
		public override void CreateUninitializedItem(HttpContext context, string id, int timeout)
		{
			...
			try
			{
				...
				byte[] value;
				int size;
				SessionStateUtility.SerializeStoreData(this.CreateNewStoreData(context, timeout), 7000, out value, out size);
				sqlStateConnection = this.GetConnection(id, ref usePooling);
				try
				{
				...
				{
					this.HandleInsertException(sqlStateConnection.Connection, e, true, id);
				}
			}
			finally
			{
		...
		internal class SqlPartitionInfo : PartitionInfo
		{
			...
			private void GetServerSupportOptions(SqlConnection sqlConnection)
			{
				...
				if (flag)
				{
					...
					{
						SqlSessionStateStore.ThrowSqlConnectionException(sqlConnection, e2);
					}
					return;
				}
				if (SqlSessionStateStore.s_usePartition)
			...
			internal SqlStateConnection(SqlSessionStateStore.SqlPartitionInfo sqlPartitionInfo)
			{
				...
				catch (Exception ex)
				{
					...
					if (ex2 != null && (ex2.Number == 18456 || ex2.Number == 18452 || ex2.Number == 18450))
					{
						...
						HttpException ex3 = new HttpException(SR.GetString("Login_failed_sql_session_database", new object[]
						{
							text
						}), ex);
