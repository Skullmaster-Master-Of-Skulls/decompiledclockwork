...
namespace System.Web.SessionState
{
	...
	internal sealed class InProcSessionStateStore : SessionStateStoreProviderBase
	{
		...
		private SessionStateStoreData DoGet(HttpContext context, string id, bool exclusive, out bool locked, out TimeSpan lockAge, out object lockId, out SessionStateActions actionFlags)
		{
			...
			InProcSessionState inProcSessionState = (InProcSessionState)HttpRuntime.CacheInternal.Get(key);
		...
		public override void SetAndReleaseItemExclusive(HttpContext context, string id, SessionStateStoreData item, object lockId, bool newItem)
		{
			...
			if (!newItem)
			{
				InProcSessionState inProcSessionState = (InProcSessionState)cacheInternal.Get(key);
				int num = (int)lockId;
				if (inProcSessionState == null)
				{
					return;
			...
			if (flag)
			{
				...
				{
					cacheInternal.UtcInsert(key, inProcSessionState2, null, Cache.NoAbsoluteExpiration, new TimeSpan(0, inProcSessionState2._timeout, 0), CacheItemPriority.NotRemovable, this._callback);
		...
		public override void CreateUninitializedItem(HttpContext context, string id, int timeout)
		{
			string key = this.CreateSessionStateCacheKey(id);
			SessionIDManager.CheckIdLength(id, true);
			InProcSessionState value = new InProcSessionState(null, null, timeout, false, DateTime.MinValue, InProcSessionStateStore.NewLockCookie, 1);
			try
			{
			}
			finally
			{
				if (HttpRuntime.CacheInternal.UtcAdd(key, value, null, Cache.NoAbsoluteExpiration, new TimeSpan(0, timeout, 0), CacheItemPriority.NotRemovable, this._callback) == null)
				{
					PerfCounters.IncrementCounter(AppPerfCounter.SESSIONS_TOTAL);
					PerfCounters.IncrementCounter(AppPerfCounter.SESSIONS_ACTIVE);
				}
