	...
	public sealed class HttpContext : IServiceProvider
	{
		...
		internal void SetPrincipalNoDemand(IPrincipal principal, bool needToSetNativePrincipal)
		{
			...
			if (needToSetNativePrincipal && this._isIntegratedPipeline && this._notificationContext.CurrentNotification == RequestNotification.AuthenticateRequest)
			{
				...
				try
				{
					...
					if (principal != null)
					{
						GCHandle value = GCHandle.Alloc(principal);
						try
						{
							pManagedPrincipal = GCHandle.ToIntPtr(value);
							iis7WorkerRequest.SetPrincipal(principal, pManagedPrincipal);
							goto IL_7B;
						}
						catch
						{
				...
				finally
				{
					...
					{
						GCHandle gchandle = GCHandle.FromIntPtr(this._pManagedPrincipal);
						if (gchandle.IsAllocated)
						{
							gchandle.Free();
						}
					}
					this._pManagedPrincipal = pManagedPrincipal;
				}
			}
		}
		...
		internal void EnsureTimeout()
		{
			if (!this._timeoutSet)
			{
				...
				int seconds = (int)httpRuntime.ExecutionTimeout.TotalSeconds;
				this._timeout = new TimeSpan(0, 0, seconds);
				this._timeoutSet = true;
			}
		}
			...
			get
			{
				...
				if (this.IsInCancellablePeriod && this.Timeout.TotalMilliseconds >= 0.0)
				{
					try
					{
						if (!CompilationUtil.IsDebuggingEnabled(this) && !Debugger.IsAttached)
						{
							result = (long)(this._timeoutStartTime.Add(this.Timeout) - DateTime.UtcNow).TotalMilliseconds;
						}
					}
					catch
					{
