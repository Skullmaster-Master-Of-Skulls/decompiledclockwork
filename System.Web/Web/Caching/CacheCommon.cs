...
namespace System.Web.Caching
{
	...
	internal class CacheCommon
	{
		...
		{
			int val = CacheMemoryPrivateBytesPressure.PollInterval / 1000;
			int num = Math.Max(Math.Min(val, 60), 5);
			bool flag = this._cacheMemoryStats.TotalMemoryPressure.IsAboveHighPressure();
			long totalMemoryChange = 0L;
			bool flag2;
			UnsafeNativeMethods.SetGCLastCalledTime(out flag2, flag ? num : 5);
			if (flag2)
			{
				...
				totalMemoryChange = Math.Max(privateBytes - privateBytes2, totalMemory - totalMemory2);
			}
			if (!flag2 || flag || this._cacheMemoryStats.IsGcCollectIneffective(totalMemoryChange))
			{
				this._timerSuspendTime = DateTime.UtcNow.AddSeconds((double)num);
			}
		}
