	...
	internal sealed class CacheMemoryPrivateBytesPressure : CacheMemoryPressure
	{
			...
			get
			{
				if (CacheMemoryPrivateBytesPressure.s_autoPrivateBytesLimit == -1L)
				{
					...
					if (totalPhysical != 0L)
					{
						...
						long val2 = HostingEnvironment.IsHosted ? (totalPhysical * 3L / 5L) : totalPhysical;
						CacheMemoryPrivateBytesPressure.s_autoPrivateBytesLimit = Math.Min(val2, val);
					}
					else
					{
		...
		internal static long GetPrivateBytes(bool nocache)
		{
			...
			else
			{
				uint num3 = 0U;
				uint num4;
				num = UnsafeNativeMethods.GetProcessMemoryInformation(CacheMemoryPrivateBytesPressure.s_pid, out num3, out num4, nocache);
				result = (long)((long)((ulong)num3) << 20);
			}
			if (num == 0)
			{
				CacheMemoryPrivateBytesPressure.s_lastReadPrivateBytes = result;
