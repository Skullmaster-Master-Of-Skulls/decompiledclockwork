	...
	internal class VBCodeGenerator : CodeCompiler
	{
		...
		protected override string CmdArgsFromParameters(CompilerParameters options)
		{
			foreach (string value in options.ReferencedAssemblies)
			{
				...
				{
					throw new ArgumentException(SR.GetString("NullOrEmpty_Value_in_Property", new object[]
					{
			...
			if (!string.IsNullOrWhiteSpace(text))
			{
				...
				stringBuilder.Append("/R:\"").Append(text3).Append("\" ");
			}
			foreach (string text4 in options.ReferencedAssemblies)
			{
		...
		protected override CompilerResults FromFileBatch(CompilerParameters options, string[] fileNames)
		{
			...
			{
				compilerResults.TempFiles.AddExtension(fileExtension, true);
			}
			else
			{
			...
			base.Compile(options, RedistVersionInfo.GetCompilerPath(this.provOptions, this.CompilerName), this.CompilerName, text4, ref file, ref num, trueArgs);
			compilerResults.NativeCompilerReturnValue = num;
			if (num != 0 || options.WarningLevel > 0)
			{
				byte[] bytes = VBCodeGenerator.ReadAllBytes(file, FileShare.ReadWrite);
