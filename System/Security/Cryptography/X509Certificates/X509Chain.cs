	...
	public class X509Chain
	{
		...
		private static class CompatSwitches
		{
			...
			private static int ReadInt32CompatSwitch(string switchName, int defaultValue)
			{
				...
				int? num = X509Chain.CompatSwitches.ReadInt32CompatSwitchFromRegistry(Registry.CurrentUser, switchName);
				if (num != null)
				{
					return num.GetValueOrDefault();
				}
				int? num2 = X509Chain.CompatSwitches.ReadInt32CompatSwitchFromRegistry(Registry.LocalMachine, switchName);
				if (num2 == null)
				{
					return defaultValue;
				}
