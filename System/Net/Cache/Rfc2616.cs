...
namespace System.Net.Cache
{
	...
	{
		// Token: 0x06002B38 RID: 11064 RVA: 0x000B8E8D File Offset: 0x000B7E8D
		private Rfc2616()
		{
		}

		// Token: 0x06002B39 RID: 11065 RVA: 0x000B8E98 File Offset: 0x000B7E98
		...
		// Token: 0x06002B3A RID: 11066 RVA: 0x000B911C File Offset: 0x000B811C
		public static CacheFreshnessStatus OnValidateFreshness(HttpRequestCacheValidator ctx)
		{
			CacheFreshnessStatus result = Rfc2616.Common.ComputeFreshness(ctx);
		...
		// Token: 0x06002B3B RID: 11067 RVA: 0x000B91D8 File Offset: 0x000B81D8
		public static CacheValidationStatus OnValidateCache(HttpRequestCacheValidator ctx)
		{
			...
			else
			{
				...
				if (cacheValidationStatus == CacheValidationStatus.ReturnCachedResponse)
				{
					if (Logging.On)
					{
						Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_valid_as_fresh_or_because_policy", new object[]
						{
							ctx.Policy.ToString()
						}));
					}
		...
		// Token: 0x06002B3C RID: 11068 RVA: 0x000B92B0 File Offset: 0x000B82B0
		public static CacheValidationStatus OnValidateResponse(HttpRequestCacheValidator ctx)
		{
			if (ctx.ResponseCount > 1)
			{
				if (Logging.On)
				{
					Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_accept_based_on_retry_count", new object[]
					{
						ctx.ResponseCount
					}));
				}
			...
			if (ctx.RequestRangeCache && httpWebResponse.StatusCode == HttpStatusCode.RequestedRangeNotSatisfiable)
			{
				if (Logging.On)
				{
					Logging.PrintError(Logging.RequestCache, SR.GetString("net_log_cache_server_didnt_satisfy_range", new object[]
					{
		...
		// Token: 0x06002B3D RID: 11069 RVA: 0x000B95EC File Offset: 0x000B85EC
		public static CacheValidationStatus OnUpdateCache(HttpRequestCacheValidator ctx)
		{
			...
			if (Logging.On)
			{
				Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_resp_status", new object[]
				{
		...
		{
			// Token: 0x06002B3E RID: 11070 RVA: 0x000B98CC File Offset: 0x000B88CC
			public static CacheValidationStatus OnValidateRequest(HttpRequestCacheValidator ctx)
			{
			...
			// Token: 0x06002B3F RID: 11071 RVA: 0x000B99D8 File Offset: 0x000B89D8
			public static CacheFreshnessStatus ComputeFreshness(HttpRequestCacheValidator ctx)
			{
				...
				if (ctx.CacheAge != TimeSpan.MinValue)
				{
					if (Logging.On)
					{
						Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_age2", new object[]
						{
							((int)ctx.CacheAge.TotalSeconds).ToString(NumberFormatInfo.InvariantInfo)
						}));
					}
				...
				if (ctx.CacheAge != TimeSpan.MinValue)
				{
					if (!ctx.CacheEntry.IsPrivateEntry && ctx.CacheCacheControl.SMaxAge != -1)
					{
						...
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_max_age_cache_s_max_age", new object[]
							{
								((int)ctx.CacheMaxAge.TotalSeconds).ToString(NumberFormatInfo.InvariantInfo)
							}));
						}
					...
					else if (ctx.CacheCacheControl.MaxAge != -1)
					{
						...
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_max_age_cache_max_age", new object[]
							{
								((int)ctx.CacheMaxAge.TotalSeconds).ToString(NumberFormatInfo.InvariantInfo)
							}));
						}
				...
				else
				{
					...
					else
					{
						...
						if (Logging.On)
						{
							...
							{
								((int)ctx.UnspecifiedMaxAge.TotalSeconds).ToString(NumberFormatInfo.InvariantInfo)
							}));
						}
			...
			// Token: 0x06002B40 RID: 11072 RVA: 0x000BA05C File Offset: 0x000B905C
			internal static Rfc2616.TriState OnUpdateCache(HttpRequestCacheValidator ctx, HttpWebResponse resp)
			{
				...
				else
				{
					if (ctx.ResponseCacheControl.SMaxAge != -1 || ctx.ResponseCacheControl.MaxAge != -1)
					{
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_resp_allows_caching", new object[]
							{
								ctx.ResponseCacheControl.ToString()
							}));
						}
						return Rfc2616.TriState.Valid;
					}
					...
					if (resp.StatusCode == HttpStatusCode.NotModified || resp.StatusCode == HttpStatusCode.OK || resp.StatusCode == HttpStatusCode.NonAuthoritativeInformation || resp.StatusCode == HttpStatusCode.PartialContent || resp.StatusCode == HttpStatusCode.MultipleChoices || resp.StatusCode == HttpStatusCode.MovedPermanently || resp.StatusCode == HttpStatusCode.Gone)
					{
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_valid_based_on_status_code", new object[]
							{
								(int)resp.StatusCode
							}));
						}
					...
					{
						Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_resp_no_cache_control", new object[]
						{
							(int)resp.StatusCode
						}));
					}
			...
			// Token: 0x06002B41 RID: 11073 RVA: 0x000BA45C File Offset: 0x000B945C
			public static bool ValidateCacheByClientPolicy(HttpRequestCacheValidator ctx)
			{
				if (ctx.Policy.Level == HttpRequestCacheLevel.Default)
				{
					...
					if (ctx.Policy.MinFresh > TimeSpan.Zero)
					{
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_policy_min_fresh", new object[]
							{
								((int)ctx.Policy.MinFresh.TotalSeconds).ToString(NumberFormatInfo.InvariantInfo)
							}));
						}
					...
					if (ctx.Policy.MaxAge != TimeSpan.MaxValue)
					{
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_policy_max_age", new object[]
							{
								((int)ctx.Policy.MaxAge.TotalSeconds).ToString(NumberFormatInfo.InvariantInfo)
							}));
						}
					...
					if (ctx.Policy.MaxStale > TimeSpan.Zero)
					{
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_policy_max_stale", new object[]
							{
								((int)ctx.Policy.MaxStale.TotalSeconds).ToString(NumberFormatInfo.InvariantInfo)
							}));
						}
			...
			// Token: 0x06002B42 RID: 11074 RVA: 0x000BA77C File Offset: 0x000B977C
			...
			// Token: 0x06002B43 RID: 11075 RVA: 0x000BA8E8 File Offset: 0x000B98E8
			...
			// Token: 0x06002B44 RID: 11076 RVA: 0x000BAEAC File Offset: 0x000B9EAC
			...
			// Token: 0x06002B45 RID: 11077 RVA: 0x000BAF9C File Offset: 0x000B9F9C
			internal static Rfc2616.TriState ValidateCacheByVaryHeader(HttpRequestCacheValidator ctx)
			{
				...
				for (int i = 0; i < arrayList.Count; i++)
				{
					...
					if (arrayList2.Count != arrayList3.Count)
					{
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_vary_header_mismatched_count", new object[]
							{
								(string)arrayList[i]
							}));
						}
			...
			// Token: 0x06002B46 RID: 11078 RVA: 0x000BB1D8 File Offset: 0x000BA1D8
			public static CacheValidationStatus TryConditionalRequest(HttpRequestCacheValidator ctx)
			{
				...
				if (Rfc2616.Common.TryConditionalRangeRequest(ctx))
				{
					...
					if (Logging.On)
					{
						Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_range", new object[]
						{
							ctx.Request.Headers["Range"]
						}));
					}
			...
			// Token: 0x06002B47 RID: 11079 RVA: 0x000BB274 File Offset: 0x000BA274
			public static CacheValidationStatus TryResponseFromCache(HttpRequestCacheValidator ctx)
			{
				...
				if (triState == Rfc2616.TriState.Invalid)
				{
					...
					if (!Rfc2616.Common.GetBytesRange(text, ref num, ref num2, ref num3, true))
					{
						if (Logging.On)
						{
							Logging.PrintError(Logging.RequestCache, SR.GetString("net_log_cache_range_invalid_format", new object[]
							{
								text
							}));
						}
					...
					if (num >= ctx.CacheEntry.StreamSize || num2 > ctx.CacheEntry.StreamSize || (num2 == -1L && ctx.CacheEntityLength == -1L) || (num2 == -1L && ctx.CacheEntityLength > ctx.CacheEntry.StreamSize) || (num == -1L && (num2 == -1L || ctx.CacheEntityLength == -1L || ctx.CacheEntityLength - num2 >= ctx.CacheEntry.StreamSize)))
					{
						if (Logging.On)
						{
							Logging.PrintWarning(Logging.RequestCache, SR.GetString("net_log_cache_range_not_in_cache", new object[]
							{
								text
							}));
						}
					...
					if (Logging.On)
					{
						Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_range_in_cache", new object[]
						{
							ctx.CacheHeaders["Content-Range"]
						}));
					}
					return CacheValidationStatus.ReturnCachedResponse;
				}
				else
				{
					if (ctx.Policy.Level == HttpRequestCacheLevel.CacheOnly && (ctx.Uri.Scheme == Uri.UriSchemeHttp || ctx.Uri.Scheme == Uri.UriSchemeHttps))
					{
						...
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_partial_resp", new object[]
							{
								ctx.CacheHeaders["Content-Range"]
							}));
						}
					...
					if (ctx.CacheEntry.StreamSize >= 2147483647L)
					{
						if (Logging.On)
						{
							Logging.PrintWarning(Logging.RequestCache, SR.GetString("net_log_cache_entry_size_too_big", new object[]
							{
								ctx.CacheEntry.StreamSize
							}));
						}
			...
			// Token: 0x06002B48 RID: 11080 RVA: 0x000BB538 File Offset: 0x000BA538
			private static Rfc2616.TriState CheckForRangeRequest(HttpRequestCacheValidator ctx, out string ranges)
			{
				...
				if (text != null)
				{
					...
					if (Logging.On)
					{
						Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_range_request_range", new object[]
						{
							ctx.Request.Headers["Range"]
						}));
					}
			...
			// Token: 0x06002B49 RID: 11081 RVA: 0x000BB65C File Offset: 0x000BA65C
			public static CacheValidationStatus ConstructConditionalRequest(HttpRequestCacheValidator ctx)
			{
				...
				if (text != null)
				{
					...
					if (Logging.On)
					{
						Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_condition_if_none_match", new object[]
						{
				...
				if (ctx.CacheEntry.LastModifiedUtc != DateTime.MinValue)
				{
					...
					if (Logging.On)
					{
						Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_condition_if_modified_since", new object[]
						{
							ctx.Request.Headers["If-Modified-Since"]
						}));
					}
				}
			...
			// Token: 0x06002B4A RID: 11082 RVA: 0x000BB7C0 File Offset: 0x000BA7C0
			private static bool TryConditionalRangeRequest(HttpRequestCacheValidator ctx)
			{
				...
				if (text != null)
				{
					...
					if (Logging.On)
					{
						Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_condition_if_range", new object[]
						{
							ctx.Request.Headers["If-Range"]
						}));
					}
					return true;
				}
			...
			// Token: 0x06002B4B RID: 11083 RVA: 0x000BB980 File Offset: 0x000BA980
			public static void Construct206PartialContent(HttpRequestCacheValidator ctx, int rangeStart)
			{
				...
				string value = string.Concat(new object[]
				{
					...
					rangeStart,
					'-',
					(long)rangeStart + ctx.CacheStreamLength - 1L,
					'/',
			...
			// Token: 0x06002B4C RID: 11084 RVA: 0x000BBA6C File Offset: 0x000BAA6C
			...
			// Token: 0x06002B4D RID: 11085 RVA: 0x000BBB08 File Offset: 0x000BAB08
			...
			// Token: 0x06002B4E RID: 11086 RVA: 0x000BBB94 File Offset: 0x000BAB94
			public static void ReplaceOrUpdateCacheHeaders(HttpRequestCacheValidator ctx, HttpWebResponse resp)
			{
				...
				if (values != null)
				{
					...
					if (arrayList.Count != 0 && ((string)arrayList[0])[0] != '*')
					{
						if (Logging.On)
						{
							Logging.PrintInfo(Logging.RequestCache, SR.GetString("net_log_cache_saving_request_headers", new object[]
							{
								resp.Headers["Vary"]
							}));
						}
			...
			// Token: 0x06002B4F RID: 11087 RVA: 0x000BBDA8 File Offset: 0x000BADA8
			...
			// Token: 0x06002B50 RID: 11088 RVA: 0x000BBDF0 File Offset: 0x000BADF0
			...
			// Token: 0x06002B51 RID: 11089 RVA: 0x000BBE38 File Offset: 0x000BAE38
