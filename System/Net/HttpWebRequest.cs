...
namespace System.Net
{
	...
	public class HttpWebRequest : WebRequest, ISerializable
	{
			...
			if (this.DoSubmitRequestProcessing(ref ex) == HttpProcessingResult.Continue)
			{
				...
				if (this._ReadAResult == null)
				{
					lock (this)
					{
						if (this._ReadAResult == null)
						{
							this._ReadAResult = new LazyAsyncResult(null, null, null, result);
						}
					}
				}
				try
		...
		internal void SerializeHeaders()
		{
			...
			string text = this._HttpRequestHeaders.ToString();
			...
			WebHeaderCollection.HeaderEncoding.GetBytes(text, 0, text.Length, this.WriteBuffer, num);
		}
		...
		private bool CheckResubmit(ref Exception e)
		{
			...
			try
			{
				if (!(flag = this.CheckResubmitForAuth()))
				{
					...
					{
						NetRes.GetWebStatusCodeString(this.ResponseStatusCode, this._HttpResponse.StatusDescription)
					}), null, WebExceptionStatus.ProtocolError, this._HttpResponse);
					return false;
				}
				goto IL_4CE;
			}
			...
			if (this.AllowAutoRedirect && (this.ResponseStatusCode == HttpStatusCode.MultipleChoices || this.ResponseStatusCode == HttpStatusCode.MovedPermanently || this.ResponseStatusCode == HttpStatusCode.Found || this.ResponseStatusCode == HttpStatusCode.SeeOther || this.ResponseStatusCode == HttpStatusCode.TemporaryRedirect))
			{
				...
				{
					e = new WebException(SR.GetString("net_resubmitprotofailed"), innerException2, WebExceptionStatus.ProtocolError, this._HttpResponse);
					return false;
				}
				...
				{
					e = new SecurityException(SR.GetString("net_redirect_perm"), new WebException(SR.GetString("net_resubmitcanceled"), innerException3, WebExceptionStatus.ProtocolError, this._HttpResponse));
					return false;
				}
				this._Uri = uri;
				bool flag2 = false;
