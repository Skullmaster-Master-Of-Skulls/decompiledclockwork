...
namespace Microsoft.CSharp
{
	...
	internal class CSharpCodeGenerator : ICodeCompiler, ICodeGenerator
	{
		...
		private string CmdArgsFromParameters(CompilerParameters options)
		{
			...
			foreach (string value in options.ReferencedAssemblies)
			{
				stringBuilder.Append("/R:");
				stringBuilder.Append("\"");
			...
			foreach (string value2 in options.EmbeddedResources)
			{
				stringBuilder.Append("/res:\"");
				stringBuilder.Append(value2);
				stringBuilder.Append("\" ");
			}
			foreach (string value3 in options.LinkedResources)
			{
		...
		private string GetBaseTypeOutput(CodeTypeReference typeRef)
		{
			...
			if ((key = text2) != null)
			{
				if (<PrivateImplementationDetails>{EC170762-C949-477C-A874-3D92AB9E80C6}.$$method0x6001706-1 == null)
				{
					<PrivateImplementationDetails>{EC170762-C949-477C-A874-3D92AB9E80C6}.$$method0x6001706-1 = new Dictionary<string, int>(16)
					{
						{
							"system.int16",
							0
				...
				int num;
				if (<PrivateImplementationDetails>{EC170762-C949-477C-A874-3D92AB9E80C6}.$$method0x6001706-1.TryGetValue(key, out num))
				{
					switch (num)
					{
					case 0:
		...
		private CompilerResults FromFileBatch(CompilerParameters options, string[] fileNames)
		{
			...
			{
				compilerResults.TempFiles.AddExtension(fileExtension2, true);
			}
			else
			{
				compilerResults.TempFiles.AddExtension(fileExtension2);
			}
			string text = this.CmdArgsFromParameters(options) + " " + CSharpCodeGenerator.JoinStringArray(fileNames, " ");
			...
			this.Compile(options, RedistVersionInfo.GetCompilerPath(this.provOptions, this.CompilerName), this.CompilerName, text, ref path, ref num, trueArgs);
