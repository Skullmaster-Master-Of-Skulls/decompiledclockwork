...
namespace System
{
	...
	public class Uri : ISerializable
	{
		...
		private static void GetConfig(ref UriIdnScope idnScope, ref bool iriParsing)
		{
			...
			Uri.ParseConfigFile(file, out idnScopeFromConfig, out iriParsingFromConfig);
								...
								while (xmlReader.NodeType != XmlNodeType.EndElement || !Uri.ConfigStringEqual(xmlReader.Name, "uri"))
								{
									if (xmlReader.NodeType == XmlNodeType.Element)
									{
										if (Uri.ConfigStringEqual(xmlReader.Name, "idn"))
										{
											...
											if (attribute != null)
											{
												if (Uri.ConfigStringEqual(attribute, "None"))
												{
													idnStateConfig = Uri.IdnScopeFromConfig.None;
												}
												else if (Uri.ConfigStringEqual(attribute, "AllExceptIntranet"))
												{
													idnStateConfig = Uri.IdnScopeFromConfig.AllExceptIntranet;
												}
												else if (Uri.ConfigStringEqual(attribute, "All"))
												{
										...
										else if (Uri.ConfigStringEqual(xmlReader.Name, "iriParsing"))
										{
											...
											if (attribute != null)
											{
												if (Uri.ConfigStringEqual(attribute, "false"))
												{
													iriParsingConfig = Uri.IriParsingFromConfig.False;
												}
												else if (Uri.ConfigStringEqual(attribute, "true"))
												{
		...
		public string DnsSafeHost
		{
			get
			{
				...
				else if (this.HostType == Uri.Flags.BasicHostType && this.InFact(Uri.Flags.HostNotCanonical | Uri.Flags.E_HostNotCanonical))
				{
					...
					int length = 0;
					Uri.UnescapeString(text, 0, text.Length, array, ref length, char.MaxValue, char.MaxValue, char.MaxValue, Uri.UnescapeMode.CopyOnly, this.m_Syntax, false, false);
		...
		public unsafe static UriHostNameType CheckHostName(string name)
		{
			...
			{
				UriHostNameType result;
				if (name[0] == '[' && name[name.Length - 1] == ']' && IPv6AddressHelper.IsValid(name2, 1, ref num) && num == name.Length)
				{
					result = UriHostNameType.IPv6;
				}
				else
				{
					num = name.Length;
					if (IPv4AddressHelper.IsValid(name2, 0, ref num, false, false) && num == name.Length)
					{
						result = UriHostNameType.IPv4;
					}
					else
					{
						num = name.Length;
						bool flag = false;
						if (DomainNameHelper.IsValid(name2, 0, ref num, ref flag, false) && num == name.Length)
						{
							result = UriHostNameType.Dns;
						}
						else
						{
							string text = null;
							...
							{
								if (!IPv6AddressHelper.IsValid(name3, 1, ref num) || num != name.Length)
								{
									string text2 = null;
									return UriHostNameType.Unknown;
								}
		...
		internal static string InternalEscapeString(string rawString)
		{
			...
			int length = 0;
			char[] array = Uri.EscapeString(rawString, 0, rawString.Length, null, ref length, true, '?', '#', '%');
			if (array == null)
			{
				return rawString;
			}
		...
		private unsafe static Uri.ParsingError ParseScheme(string uriString, ref Uri.Flags flags, ref UriParser syntax)
		{
			...
			fixed (char* uriString2 = uriString)
			{
				...
				ushort num = Uri.ParseSchemeCheckImplicitFile(uriString2, (ushort)length, ref parsingError, ref flags, ref syntax);
			...
			fixed (char* ptr = (this.m_iriParsing && (this.m_Flags & Uri.Flags.HasUnicode) != Uri.Flags.Zero && (this.m_Flags & Uri.Flags.HostUnicodeNormalized) == Uri.Flags.Zero) ? this.m_originalUnicodeString : this.m_String)
			{
				...
				else
				{
					...
					else
					{
						...
						result = Uri.ParsingError.BadAuthorityTerminator;
					}
				}
				return result;
			}
		...
		private unsafe void CreateHostString()
		{
			...
			if (text.Length != 0)
			{
				if (this.HostType == Uri.Flags.BasicHostType)
				{
					...
					fixed (char* str = text)
					{
						check = this.CheckCanonical(str, ref num, (ushort)text.Length, char.MaxValue);
					}
					...
					if ((check & (Uri.Check.EscapedCanonical | Uri.Check.BackslashInPath)) != Uri.Check.EscapedCanonical)
					{
						...
						{
							int length = 0;
							char[] array = Uri.EscapeString(text, 0, text.Length, null, ref length, true, '?', '#', this.IsImplicitFile ? char.MaxValue : '%');
							if (array != null)
							{
								text = new string(array, 0, length);
							}
		...
		private unsafe void GetHostViaCustomSyntax()
		{
			...
			if (this.m_Info.Host == null)
			{
				...
				{
					string text2 = null;
					if (this.CheckAuthorityHelper(pString, 0, (ushort)text.Length, ref parsingError, ref flags, this.m_Syntax, ref text2) != (ushort)text.Length)
					{
		...
		private unsafe ushort CheckAuthorityHelper(char* pString, ushort idx, ushort length, ref Uri.ParsingError err, ref Uri.Flags flags, UriParser syntax, ref string newHost)
		{
			...
			if ((flags & Uri.Flags.HostTypeMask) == Uri.Flags.Zero)
			{
				...
				else if (syntax.InFact(UriSyntaxFlags.V1_UnknownUri))
				{
					...
					{
						string text2 = new string(pString, num7, num7 - num);
						try
						{
		...
		private unsafe void FindEndOfComponent(string input, ref ushort idx, ushort end, char delim)
		{
			...
			{
				this.FindEndOfComponent(str, ref idx, end, delim);
			}
		}
		...
		private unsafe char[] GetCanonicalPath(char[] dest, ref int pos, UriFormat formatAs)
		{
			...
			if (this.InFact(Uri.Flags.ShouldBeCompressed))
			{
				...
				{
					string input = new string(dest, pos, num - pos);
					dest = Uri.EscapeString(input, 0, num - pos, dest, ref pos, true, '?', '#', this.IsImplicitFile ? char.MaxValue : '%');
					num = pos;
				}
			}
		...
		protected virtual string Unescape(string path)
		{
			...
			int length = 0;
			array = Uri.UnescapeString(path, 0, path.Length, array, ref length, char.MaxValue, char.MaxValue, char.MaxValue, Uri.UnescapeMode.Unescape | Uri.UnescapeMode.UnescapeAll, null, false, true);
			return new string(array, 0, length);
		}
		...
		protected static string EscapeString(string str)
		{
			...
			int length = 0;
			char[] array = Uri.EscapeString(str, 0, str.Length, null, ref length, true, '?', '#', '%');
			if (array == null)
			{
				return str;
			}
		...
		private void CreateThis(string uri, bool dontEscape, UriKind uriKind)
		{
			...
			Uri.ParsingError err = Uri.ParseScheme(this.m_String, ref this.m_Flags, ref this.m_Syntax);
			UriFormatException ex;
		...
		private unsafe bool CheckForUnicode(string data)
		{
			...
			int length = 0;
			array = Uri.UnescapeString(data, 0, data.Length, array, ref length, char.MaxValue, char.MaxValue, char.MaxValue, Uri.UnescapeMode.Unescape | Uri.UnescapeMode.UnescapeAll, null, false, false);
		...
		public static string EscapeUriString(string stringToEscape)
		{
			...
			int length = 0;
			char[] array = Uri.EscapeString(stringToEscape, 0, stringToEscape.Length, null, ref length, true, char.MaxValue, char.MaxValue, char.MaxValue);
			if (array == null)
			{
		...
		public static string EscapeDataString(string stringToEscape)
		{
			...
			int length = 0;
			char[] array = Uri.EscapeString(stringToEscape, 0, stringToEscape.Length, null, ref length, false, char.MaxValue, char.MaxValue, char.MaxValue);
			if (array == null)
			{
				return stringToEscape;
			}
		...
		private unsafe static char[] UnescapeString(string input, int start, int end, char[] dest, ref int destPosition, char rsvd1, char rsvd2, char rsvd3, Uri.UnescapeMode unescapeMode, UriParser syntax, bool isQuery, bool readOnlyConfig)
		{
			...
			char* pStr = intPtr2;
			return Uri.UnescapeString(pStr, start, end, dest, ref destPosition, rsvd1, rsvd2, rsvd3, unescapeMode, syntax, isQuery, readOnlyConfig);
		}
			...
			while (i < end)
			{
				...
				else
				{
					...
					if (char.IsHighSurrogate(c) && i + 1 < end)
					{
						...
						flag2 = !Uri.CheckIriUnicodeRange(c, lowSurr, ref flag, component == UriComponents.Query);
						if (!flag2)
						{
							ptr[(IntPtr)(num2++) * 2] = pInput[i++];
							ptr[(IntPtr)(num2++) * 2] = pInput[i];
		...
		internal unsafe bool InternalIsWellFormedOriginalString()
		{
			...
			fixed (char* @string = this.m_String)
			{
				...
				if (!this.IsAbsoluteUri)
				{
					result = ((this.CheckCanonical(@string, ref num, (ushort)this.m_String.Length, '￾') & (Uri.Check.EscapedCanonical | Uri.Check.BackslashInPath)) == Uri.Check.EscapedCanonical);
				}
		...
		private string GetRelativeSerializationString(UriFormat format)
		{
			if (format == UriFormat.UriEscaped)
			{
				...
				int length = 0;
				char[] array = Uri.EscapeString(this.m_String, 0, this.m_String.Length, null, ref length, true, char.MaxValue, char.MaxValue, '%');
				if (array == null)
				{
					return this.m_String;
				}
			...
			else
			{
				...
				int length2 = 0;
				array2 = Uri.UnescapeString(this.m_String, 0, this.m_String.Length, array2, ref length2, char.MaxValue, char.MaxValue, char.MaxValue, Uri.UnescapeMode.EscapeUnescape, null, false, true);
		...
		internal unsafe bool IsBaseOfHelper(Uri uriLink)
		{
			...
			fixed (char* pMe = parts)
			{
				...
				{
					return Uri.TestForSubPath(pMe, (ushort)parts.Length, pShe, (ushort)parts2.Length, this.IsUncOrDosPath || uriLink.IsUncOrDosPath);
				}
			}
		}
