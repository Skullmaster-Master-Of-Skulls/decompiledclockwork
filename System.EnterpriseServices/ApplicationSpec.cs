...
namespace System.EnterpriseServices
{
	...
	internal class ApplicationSpec
	{
		...
		private string FormatApplicationName(Assembly asm)
		{
			...
			{
				result = ((ApplicationNameAttribute)customAttributes[0]).Value;
			}
			else
			{
				result = asm.GetName().Name;
			}
			return result;
		}
		...
		public bool Matches(ICatalogObject obj)
		{
			if (this.ID != null)
			{
				Guid a = new Guid(this.ID);
				Guid b = new Guid((string)obj.GetValue("ID"));
				if (a == b)
				{
					return true;
				}
			...
			{
				string b2 = ((string)obj.GetValue("Name")).ToLower(CultureInfo.InvariantCulture);
				if (this.Name.ToLower(CultureInfo.InvariantCulture) == b2)
				{
					return true;
				}
			}
