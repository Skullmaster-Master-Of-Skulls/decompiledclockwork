	...
	internal class RegistrationDriver
	{
			...
			try
			{
				...
				if (num > 0)
				{
					...
					for (int i = 0; i < num; i++)
					{
						...
						int errorCode = (int)catalogObject.GetValue("ErrorCode");
						...
						array[i] = new RegistrationErrorInfo(majorRef, minorRef, name, errorCode);
					}
				}
				result = array;
			}
		...
		private RegistrationException WrapCOMException(ICatalogCollection coll, COMException e, string msg)
		{
			...
			if (e.ErrorCode == Util.COMADMIN_E_OBJECTERRORS)
			{
				...
				{
					errorInfo = RegistrationDriver.BuildErrorInfoChain(catalogCollection);
				}
			}
		...
		private void InstallTypeLibrary_W2K(ApplicationSpec spec)
		{
			...
			{
				throw this.WrapCOMException(null, e, Resource.FormatString("Reg_TypeLibInstallErr", spec.TypeLib, spec.Name));
			}
		}
		...
		private void InstallTypeLibrary_MTS(ApplicationSpec spec)
		{
			...
			try
			{
				...
				foreach (Type type in spec.NormalTypes)
				{
					...
					for (int j = 0; j < catalogCollection.Count(); j++)
					{
						...
						Guid a = new Guid((string)catalogObject2.Key());
						if (a == guid)
						{
							flag = true;
							break;
			...
			{
				throw this.WrapCOMException(catalogCollection, e, Resource.FormatString("Reg_TypeLibInstallErr", spec.TypeLib, spec.Name));
			}
			catch (Exception inner)
			{
		...
		internal void ConfigureCollection(ICatalogCollection coll, IConfigCallback cb)
		{
			...
			foreach (object key in cb)
			{
				object a = cb.FindObject(coll, key);
				cb.ConfigureDefaults(a, key);
			}
			RegistrationDriver.SaveChanges(coll);
			flag = false;
			foreach (object key2 in cb)
			{
				object a2 = cb.FindObject(coll, key2);
				if (cb.Configure(a2, key2))
				{
					flag = true;
				}
			}
			RegistrationDriver.SaveChanges(coll);
			flag = false;
			foreach (object key3 in cb)
			{
				object a3 = cb.FindObject(coll, key3);
				if (cb.AfterSaveChanges(a3, key3))
				{
					flag = true;
				}
			}
		...
		private void ConfigureComponents(ApplicationSpec spec)
		{
			...
			try
			{
				...
				coll = (ICatalogCollection)this._appColl.GetCollection(CollectionName.Components, catalogObject.Key());
			...
			{
				throw this.WrapCOMException(coll, e, Resource.FormatString("Reg_ConfigErr"));
			}
			catch (Exception inner)
			{
				throw new RegistrationException(Resource.FormatString("Reg_ConfigUnkErr"), inner);
			...
			if (!Platform.IsLessThan(Platform.Whistler) && this._cat != null)
			{
				foreach (Type type in spec.ConfigurableTypes)
				{
					...
					try
					{
						...
						{
							Type type3 = type2;
							string name = "GetComponentVersions";
							BindingFlags invokeAttr = BindingFlags.InvokeMethod;
							Binder binder = null;
							object cat = this._cat;
							object[] array = new object[5];
							array[0] = text;
							num = (int)this.InvokeMemberHelper(type3, name, invokeAttr, binder, cat, array);
						}
						catch (COMException ex)
						{
							...
							num = (int)this.InvokeMemberHelper(type2, "GetComponentVersionCount", BindingFlags.InvokeMethod, null, this._cat, new object[]
							{
								text
							});
						}
		...
		private object InvokeMemberHelper(Type type, string name, BindingFlags invokeAttr, Binder binder, object target, object[] args)
		{
			...
			try
			{
				result = type.InvokeMember(name, invokeAttr, binder, target, args, CultureInfo.InvariantCulture);
			}
		...
		private void PrepDriver(ref ApplicationSpec spec)
		{
			...
			if ((spec.Partition == null || spec.Partition.Length == 0) && spec.ID != null)
			{
				try
				{
					...
					try
					{
						spec.Partition = (string)this.InvokeMemberHelper(type, "GetAppPartitionId", BindingFlags.InvokeMethod, null, this._cat, new object[]
						{
							spec.ID
						});
					}
					catch (COMException ex)
					{
						if (Util.DISP_E_UNKNOWNNAME == ex.ErrorCode)
						{
							spec.Partition = (string)this.InvokeMemberHelper(type, "GetPartitionID", BindingFlags.InvokeMethod, null, this._cat, new object[]
							{
								spec.ID
							});
						}
			...
			if (spec.Partition != null && spec.Partition.Length != 0)
			{
				try
				{
					...
					try
					{
						this.InvokeMemberHelper(type2, "SetApplicationPartition", BindingFlags.InvokeMethod, null, this._cat, new object[]
						{
							spec.Partition
						});
					}
					catch (COMException ex2)
					{
						...
						this.InvokeMemberHelper(type2, "CurrentPartition", BindingFlags.SetProperty, null, this._cat, new object[]
						{
							spec.Partition
						});
					}
		...
		private bool ValidateBitness(ApplicationSpec spec, out string message)
		{
			...
			bool flag = Wow64Helper.IsWow64Process();
			ICatalogObject catalogObject = this.FindApplication(this._appColl, spec);
			if (catalogObject == null)
			{
				return result;
			...
			for (int j = 0; j < num; j++)
			{
				...
				Guid key = new Guid(g);
				if (this.FindIndexOf(array, key) != -1)
				{
		...
		public void UninstallAssembly(RegistrationConfig regConfig, object obSync)
		{
			...
			ApplicationSpec applicationSpec = new ApplicationSpec(asm, regConfig);
			if (applicationSpec.ConfigurableTypes == null)
			{
				return;
			}
			...
			this.UnregisterAssembly(asm, applicationSpec);
			this.CleanupDriver();
		}
