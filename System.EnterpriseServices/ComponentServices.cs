...
namespace System.EnterpriseServices
{
	...
	internal sealed class ComponentServices
	{
		...
		private static void ReconnectForPooling(ServicedComponentProxy scp)
		{
			...
			servicedComponentProxy.ConnectForPooling(scp, server, proxyTearoff, false);
		...
		internal static string ConvertToString(IMessage reqMsg)
		{
			...
			long num;
			byte[] bytes = componentSerializer.MarshalToBuffer(reqMsg, out num);
			string @string = ComponentServices.GetString(bytes, (int)num);
