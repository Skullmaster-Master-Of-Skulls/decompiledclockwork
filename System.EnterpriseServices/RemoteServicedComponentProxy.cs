	...
	internal class RemoteServicedComponentProxy : RealProxy
	{
		...
		public override IMessage Invoke(IMessage reqMsg)
		{
			...
			else
			{
				...
				{
					int num;
					if (this._fUseIntfDispatch || ((num = ServicedComponentInfo.MICachedLookup(m)) & 4) != 0 || (num & 8) != 0)
					{
						...
						result = RemotingServices.ExecuteMessage((MarshalByRefObject)this._server, reqMsg2);
					}
					else
					{
						bool flag = (num & 2) != 0;
						...
						if (flag)
						{
							s2 = remoteDispatch.RemoteDispatchAutoDone(s);
						}
						else
						{
							s2 = remoteDispatch.RemoteDispatchNotAutoDone(s);
						}
						result = ComponentServices.ConvertToReturnMessage(s2, reqMsg);
					}
				}
				catch (COMException ex)
				{
					...
					result = new ReturnMessage(null, null, 0, methodCallMessage3.LogicalCallContext, methodCallMessage3);
				}
				if (this.IsDisposeRequest(reqMsg))
				{
					this.Dispose(true);
