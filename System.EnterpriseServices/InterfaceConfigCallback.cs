...
namespace System.EnterpriseServices
{
	...
	internal class InterfaceConfigCallback : IConfigCallback
	{
		...
		{
			Guid b = new Guid((string)ifcObj.GetValue("IID"));
			foreach (Type type in interfaces)
			{
				Guid a = Marshal.GenerateGuidForType(type);
				if (a == b)
				{
					return type;
				}
		...
		{
			string b = (string)ifcObj.GetValue("Name");
			foreach (Type type in interfaces)
			{
				if (type.IsInterface)
				{
		...
		public InterfaceConfigCallback(ICatalogCollection coll, Type t, Hashtable cache, RegistrationDriver driver)
		{
			...
			foreach (Type type in this._ifcs)
			{
				if (Marshal.GenerateGuidForType(type) == InterfaceConfigCallback.IID_IProcessInitializer)
				{
					...
					{
						throw new RegistrationException(Resource.FormatString("Reg_FailPIT", this._type), inner);
					}
					catch
					{
		...
		public object FindObject(ICatalogCollection coll, object key)
		{
			...
			Type type = this.FindInterfaceByID(ifcObj, this._type, this._ifcs);
			if (type == null)
			{
		...
		public IEnumerator GetEnumerator()
		{
			...
			this._coll.GetEnumerator(out result);
			return result;
		}
		...
		public void ConfigureSubCollections(ICatalogCollection coll)
		{
			foreach (object obj in this)
			{
				...
				if (type != null)
				{
					...
					this._driver.ConfigureCollection(coll2, new MethodConfigCallback(coll2, type, this._type, this._cache, this._driver));
				}
			}
		}
