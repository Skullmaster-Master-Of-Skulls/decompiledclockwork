...
namespace System.EnterpriseServices
{
	...
	internal class ServicedComponentProxy : RealProxy, IProxyInvoke, IManagedPoolAction
	{
		...
		internal ServicedComponentProxy(Type serverType, bool fIsJitActivated, bool fIsPooled, bool fAreMethodsSecure, bool fCreateRealServer) : base(serverType, ServicedComponentProxy._stub, -1)
		{
			this._gitCookie = 0;
			...
			if (bGit)
			{
				if (ServicedComponentProxy._gitQueue.Count > 0)
				{
					...
					lock (ServicedComponentProxy._gitQueue)
					{
						if (ServicedComponentProxy._gitQueue.Count > 0)
						{
							cookie = (int)ServicedComponentProxy._gitQueue.Dequeue();
							flag3 = true;
			...
			if (this._fDeliverADC)
			{
				...
				if (this.IsObjectPooled)
				{
					...
					try
					{
						...
						this._proxyTearoff.SetCanBePooled(canBePooled);
					}
					catch
					{
		...
		internal void ActivateObject()
		{
			...
			if (this.IsObjectPooled && this.IsJitActivated && this.HomeToken != currentContextToken)
			{
				...
				if (obj != null)
				{
					...
					servicedComponentProxy.ConnectForPooling(this, server, proxyTearoff, true);
		...
		public override IntPtr GetCOMIUnknown(bool fIsBeingMarshalled)
		{
			if (this._token == IntPtr.Zero || this._token == ServicedComponentProxy.NegativeOne || this._token == Proxy.GetCurrentContextToken())
			{
				if (fIsBeingMarshalled)
				{
					...
					{
						intPtr = base.GetCOMIUnknown(false);
						result = Proxy.GetStandardMarshal(intPtr);
					}
					finally
					{
						if (intPtr != IntPtr.Zero)
			...
			else
			{
				if (Util.ExtendedLifetime)
				{
					...
					try
					{
						byte[] b = this._callback.SwitchMarshal(this._context, comiunknown);
						result2 = Proxy.UnmarshalObject(b);
					}
					finally
					{
		...
		public override IMessage Invoke(IMessage request)
		{
			...
			{
				result = this.LocalInvoke(request);
			}
			else
			{
				result = this.CrossCtxInvoke(request);
			}
			return result;
		}
		...
		public IMessage LocalInvoke(IMessage reqMsg)
		{
			...
			if (reqMsg is IConstructionCallMessage)
			{
				...
				MarshalByRefObject retObj = (MarshalByRefObject)this.GetTransparentProxy();
				message = EnterpriseServicesHelper.CreateConstructionReturnMessage((IConstructionCallMessage)reqMsg, retObj);
			}
			else if (reqMsg is IMethodCallMessage)
			{
				...
				catch (Exception except)
				{
					if (flag)
					{
						this.SendMethodReturn(reqMsg, except);
					}
					throw;
				}
				catch
		...
		private IMessage CrossCtxInvoke(IMessage reqMsg)
		{
			...
			MemberInfo m = ReflectionCache.ConvertToClassMI(base.GetProxiedType(), memberInfo);
			...
			else if ((memberInfo3 = this.AliasCall(methodBase as MethodInfo)) != null)
			{
			...
			return this._callback.DoCallback(transparentProxy, reqMsg, this._context, fIsAutoDone, memberInfo2, this._gitCookie != 0);
		}
		...
		private static MethodInfo _getComIUnknownMethod = typeof(MarshalByRefObject).GetMethod("GetComIUnknown", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null, new Type[]
		{
			typeof(bool)
		}, null);
