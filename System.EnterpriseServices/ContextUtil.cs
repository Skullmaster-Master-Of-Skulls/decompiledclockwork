	...
	public sealed class ContextUtil
	{
		...
		internal static object ObjectContext
		{
			get
			{
				...
				IObjectContext result = null;
				int objectContext = Util.GetObjectContext(out result);
				if (objectContext == 0)
				{
					return result;
				}
		...
		public static bool IsSecurityEnabled
		{
			get
			{
				...
				try
				{
					result = ((IObjectContext)ContextUtil.ObjectContext).IsSecurityEnabled();
				}
				catch
				{
					result = false;
		...
		public static Transaction SystemTransaction
		{
			get
			{
				...
				if (obj == null)
				{
					...
					Guid ownerGuid = ContextThunk.RegisterTransactionProxy(transactionProxy);
				...
				IDtcTransaction transactionNative = ContextThunk.GetTransaction() as IDtcTransaction;
