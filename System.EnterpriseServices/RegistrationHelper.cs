		...
		private bool TryTransactedInstall(RegistrationConfig regConfig)
		{
			...
			catch (Exception ex)
			{
				try
				{
					...
					string message = string.Format(CultureInfo.CurrentCulture, Resource.FormatString("Reg_ErrTxInst"), new object[]
					{
						ex
					});
			...
			catch
			{
				try
				{
					...
					{
						Resource.FormatString("Err_NonClsException", "RegistrationHelper.TryTransactedInstall")
					});
		...
		private bool TryTransactedUninstall(RegistrationConfig regConfig)
		{
			...
			catch (Exception ex)
			{
				try
				{
					EventLog eventLog = new EventLog();
					eventLog.Source = "System.EnterpriseServices";
					string message = string.Format(CultureInfo.CurrentCulture, Resource.FormatString("Reg_ErrTxUninst"), new object[]
					{
						ex
					});
					eventLog.WriteEntry(message, EventLogEntryType.Error);
			...
			catch
			{
				try
				{
					...
					string message2 = string.Format(CultureInfo.CurrentCulture, Resource.FormatString("Reg_ErrTxInst"), new object[]
					{
						Resource.FormatString("Err_NonClsException", "RegistrationHelper.TryTransactedUninstall")
					});
					eventLog2.WriteEntry(message2, EventLogEntryType.Error);
				}
				catch
