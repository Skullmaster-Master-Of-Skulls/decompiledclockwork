	...
	public class GenerateMetadata : IComSoapMetadata
	{
		...
		public string GenerateSigned(string strSrcTypeLib, string outPath, bool InstallGac, out string Error)
		{
			...
			try
			{
				...
				uint num = 0U;
				GenerateMetadata.StrongNameKeyGen(strSrcTypeLib, dwFlags, out zero, out num);
				...
				result = this.GenerateMetaData(strSrcTypeLib, outPath, null, keyPair);
			}
			catch (Exception ex)
			{
				Error = ex.ToString();
		...
		public string GenerateMetaData(string strSrcTypeLib, string outPath, byte[] PublicKey, StrongNameKeyPair KeyPair)
		{
			...
			string str;
			text = CacheInfo.GetMetadataName(strSrcTypeLib, typeLib, out str);
			if (text.Length == 0)
			{
				return text;
			}
			...
			string text2 = outPath + str;
			if (this._signed)
			{
				try
				{
					AssemblyManager assemblyManager = new AssemblyManager();
					if (assemblyManager.CompareToCache(text2, strSrcTypeLib))
					{
						Publish publish = new Publish();
						publish.GacInstall(text2);
						return text;
					}
					if (assemblyManager.GetFromCache(text2, strSrcTypeLib))
					{
						Publish publish2 = new Publish();
						publish2.GacInstall(text2);
						return text;
					}
					goto IL_13B;
				}
			...
			try
			{
				...
				AssemblyBuilder assemblyBuilder = typeLibConverter.ConvertTypeLibToAssembly(typeLib, text2, TypeLibImporterFlags.UnsafeInterfaces, new GenerateMetadata.ImporterCallback
				{
					OutputDir = outPath
				...
				if (this._signed)
				{
					...
					assemblyManager2.CopyToCache(text2, strSrcTypeLib);
					Publish publish3 = new Publish();
					publish3.GacInstall(text2);
				}
			}
		...
		internal class ImporterCallback : ITypeLibImporterNotifySink
		{
			...
			public Assembly ResolveRef(object TypeLib)
			{
				...
				if (assembly == null)
				{
					...
					string str = Resource.FormatString("Soap_ResolutionForTypeLibFailed");
					ComSoapPublishError.Report(str + " " + typeLibName);
				}
				return assembly;
			}
