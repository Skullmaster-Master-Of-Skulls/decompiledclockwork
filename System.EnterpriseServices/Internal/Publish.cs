...
namespace System.EnterpriseServices.Internal
{
	...
	public class Publish : IComSoapPublisher
	{
					...
					if (assemblyName.Name == "System.EnterpriseServices" && version < assemblyName.Version)
					{
						...
						if (uri.IsFile && uri.LocalPath != "")
						{
							...
							foreach (string text in files)
							{
								...
								ITypeLib typeLib;
								Marshal.ThrowExceptionForHR(Publish.LoadTypeLib(text, out typeLib));
								object obj;
		...
		public void GacInstall(string AssemblyPath)
		{
			...
			IntPtr value = Publish.LoadLibrary(text);
		...
		private void PrivateGacInstall(string AssemblyPath)
		{
			try
			{
				...
				if (num != 0)
				{
					string str = Resource.FormatString("Soap_GacInstallFailed");
					ComSoapPublishError.Report(str + " " + AssemblyPath);
				}
			}
			catch (Exception ex)
			{
		...
		public void GacRemove(string AssemblyPath)
		{
			try
			{
				...
				if (num != 0)
				{
					...
					ComSoapPublishError.Report(string.Concat(new string[]
					{
						text,
						" ",
						AssemblyPath,
		...
		private bool GetVRootPhysicalPath(string VirtualRoot, out string PhysicalPath, out string BinDirectory, bool CreateDir)
		{
			...
			result = Directory.Exists(BinDirectory);
			return result;
		}
		...
		public void CreateVirtualRoot(string Operation, string FullUrl, out string BaseUrl, out string VirtualRoot, out string PhysicalPath, out string Error)
		{
			...
			try
			{
				...
				if (VirtualRoot.Length > 0)
				{
					...
					string text = "";
					this.GetVRootPhysicalPath(VirtualRoot, out PhysicalPath, out text, flag);
					...
					else if (flag)
					{
						...
						string text2 = "";
						serverWebConfig.Create(PhysicalPath, "Web", out text2);
						...
						try
						{
							...
							iisvirtualRoot.Create(rootWeb, PhysicalPath, VirtualRoot, out text3);
						}
						catch (Exception ex)
						{
							if (text3.Length <= 0)
							{
								...
								text3 = string.Format(CultureInfo.CurrentCulture, string.Concat(new string[]
								{
									text4,
									" ",
									VirtualRoot,
						...
						catch
						{
							if (text3.Length <= 0)
							{
								text3 = Resource.FormatString("Soap_VRootCreationFailed") + VirtualRoot + " " + Resource.FormatString("Err_NonClsException", "Publish.CreateVirtualRoot");
							}
						}
						if (text3.Length > 0)
						{
