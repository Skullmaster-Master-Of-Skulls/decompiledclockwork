	...
	public sealed class ResourcePool : IObjPool
	{
		...
		public object GetResource()
		{
			...
			try
			{
				...
				if (transactionResourcePool != null)
				{
					int resource = transactionResourcePool.GetResource(intPtr, out result);
					if (resource >= 0)
					{
						Marshal.Release(intPtr);
					}
		...
		{
			ITransactionResourcePool result = null;
			object obj = null;
			int num = 0;
			((IContext)ContextUtil.ObjectContext).GetProperty(ResourcePool.GUID_TransactionProperty, out num, out obj);
			int transactionResourcePool = ((ITransactionProperty)obj).GetTransactionResourcePool(out result);
			if (transactionResourcePool >= 0)
			{
				return result;
			}
			return null;
