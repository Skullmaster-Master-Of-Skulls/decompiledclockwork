	...
	public sealed class RegistrationHelperTx : ServicedComponent
	{
		...
		private static ICatalogObject FindApplication(ICatalogCollection coll, Guid appid, ref int idx)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				Guid a = new Guid((string)catalogObject.GetValue("ID"));
				if (a == appid)
				{
					idx = i;
		...
		private static ICatalogObject FindComponent(ICatalogCollection coll, Guid clsid, ref int idx)
		{
			...
			{
				ICatalogObject catalogObject = (ICatalogObject)coll.Item(i);
				Guid a = new Guid((string)catalogObject.GetValue("CLSID"));
				if (a == clsid)
				{
					idx = i;
					return catalogObject;
			...
			if (!Platform.Supports(PlatformFeature.SWC))
			{
				try
				{
					if (!Platform.IsLessThan(Platform.W2K))
					{
						...
						ICatalogObject catalogObject2 = RegistrationHelperTx.FindComponent(coll, guid, ref num);
						if (catalogObject2 == null)
						{
							...
							catalogObject2 = RegistrationHelperTx.FindComponent(coll, guid, ref num);
						}
						RegistrationHelperTx.ConfigureComponent(coll, catalogObject2);
				...
				catch (Exception ex)
				{
					try
					{
						...
						string message = string.Format(CultureInfo.CurrentCulture, Resource.FormatString("Reg_ErrInstSysEnt"), new object[]
						{
							ex
						});
				...
				catch
				{
					try
					{
						...
						string message2 = string.Format(CultureInfo.CurrentCulture, Resource.FormatString("Reg_ErrTxInst"), new object[]
						{
							Resource.FormatString("Err_NonClsException", "RegistrationHelperTx.InstallUtilityApplication")
						});
			...
			if (!Platform.Supports(PlatformFeature.SWC))
			{
				try
				{
					if (!Platform.IsLessThan(Platform.W2K))
					{
						...
						RegistrationDriver.Populate(catalogCollection);
						ICatalogObject catalogObject = RegistrationHelperTx.FindApplication(catalogCollection, RegistrationHelperTx._appid, ref lIndex);
						if (catalogObject != null)
						{
							catalogObject.SetValue("Changeable", true);
							catalogObject.SetValue("Deleteable", true);
							...
							ICatalogObject catalogObject2 = RegistrationHelperTx.FindComponent(catalogCollection2, clsid, ref lIndex2);
							int num = catalogCollection2.Count();
							if (catalogObject2 != null)
							{
								catalogCollection2.Remove(lIndex2);
								catalogCollection2.SaveChanges();
							}
							if (catalogObject2 != null && num == 1)
							{
								catalogCollection.Remove(lIndex);
								catalogCollection.SaveChanges();
							}
							else
							{
				...
				catch (Exception ex)
				{
					try
					{
						EventLog eventLog = new EventLog();
						eventLog.Source = "System.EnterpriseServices";
						string message = string.Format(CultureInfo.CurrentCulture, Resource.FormatString("Reg_ErrUninstSysEnt"), new object[]
						{
							ex
						});
						eventLog.WriteEntry(message, EventLogEntryType.Error);
				...
				catch
				{
					try
					{
						...
						string message2 = string.Format(CultureInfo.CurrentCulture, Resource.FormatString("Reg_ErrTxInst"), new object[]
						{
							Resource.FormatString("Err_NonClsException", "RegistrationHelperTx.UninstallUtilityApplication")
						});
						eventLog2.WriteEntry(message2, EventLogEntryType.Error);
					}
					catch
