...
namespace System.Web
{
	...
	{
		// Token: 0x060009C2 RID: 2498 RVA: 0x000030B5 File Offset: 0x000012B5
		internal HttpDebugHandler()
		{
		}

		// Token: 0x060009C3 RID: 2499 RVA: 0x000166D8 File Offset: 0x000148D8
		public void ProcessRequest(HttpContext context)
		{
			try
			{
				...
				{
					context.Response.Write(SR.GetString("Debugging_forbidden", new object[]
					{
				...
				else
				{
					...
					else
					{
						...
						{
							context.Response.Write(SR.GetString("Debug_Access_Denied", new object[]
							{
								context.Request.Path
							}));
						...
						else
						{
							...
							else
							{
								...
								HttpValueCollection httpValueCollection = new HttpValueCollection(str, true, true, Encoding.UTF8);
								string text3 = httpValueCollection["autoattachclsid"];
								bool flag = false;
								if (text3 != null)
								{
									for (int i = 0; i < HttpDebugHandler.validClsIds.Length; i++)
									{
										if (StringUtil.EqualsIgnoreCase(text3, HttpDebugHandler.validClsIds[i]))
										{
											flag = true;
											break;
										}
								...
								{
									int num = UnsafeNativeMethods.AttachDebugger(text3, text2, context.WorkerRequest.GetUserToken());
									if (num != 0)
									{
										context.Response.Write(SR.GetString("Error_Attaching_with_MDM", new object[]
										{
											"0x" + num.ToString("X8", CultureInfo.InvariantCulture)
										}));
										context.Response.StatusCode = 500;
									}
									else
		...
		// (get) Token: 0x060009C4 RID: 2500 RVA: 0x000097B7 File Offset: 0x000079B7
		...
		private static string[] validClsIds = new string[]
		{
			"{70f65411-fe8c-4248-bcff-701c8b2f4529}",
