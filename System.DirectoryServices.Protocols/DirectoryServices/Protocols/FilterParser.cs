	...
	internal class FilterParser
	{
		...
		public static ADFilter ParseFilterString(string filter)
		{
			...
			if (match.Groups["item"].ToString().Length == 0)
			{
				...
				string a;
				if ((a = match.Groups["filtercomp"].ToString()) != null)
				{
					if (!(a == "|"))
					{
						if (!(a == "&"))
						...
						else
						{
							...
							foreach (object obj in arrayList)
							{
								...
								ADFilter adfilter2 = FilterParser.ParseFilterString(filter2);
								if (adfilter2 == null)
								{
									return null;
								}
					...
					else
					{
						...
						{
							string filter3 = (string)obj2;
							ADFilter adfilter2 = FilterParser.ParseFilterString(filter3);
							if (adfilter2 == null)
							{
								return null;
							}
			...
			else
			{
				if (match.Groups["simple"].ToString().Length != 0)
				{
					...
					{
						ADValue value = FilterParser.StringFilterValueToADValue(match.Groups["simplevalue"].ToString());
						adattribute.Values.Add(value);
					}
					adattribute.Name = match.Groups["simpleattr"].ToString();
					string a2;
					if ((a2 = match.Groups["filtertype"].ToString()) != null)
					{
						if (a2 == "=")
						{
		...
		protected static ADValue StringFilterValueToADValue(string strVal)
		{
			...
			else
			{
				...
				for (int i = 1; i < array.Length; i++)
				{
					...
					{
						byte.Parse(s, NumberStyles.AllowHexSpecifier, CultureInfo.InvariantCulture)
					});
					if (array[i].Length > 2)
					{
