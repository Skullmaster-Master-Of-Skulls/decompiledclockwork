...
namespace System.DirectoryServices.Protocols
{
	...
	public class LdapConnection : DirectoryConnection, IDisposable
	{
		...
		static LdapConnection()
		{
			...
			LdapConnection.asyncResultTable = Hashtable.Synchronized(table);
		...
		internal void Init()
		{
			...
			{
				this.ldapHandle = Wldap32.cldap_open(hostName, ((LdapDirectoryIdentifier)this.directoryIdentifier).PortNumber);
			}
			else
			{
				this.ldapHandle = Wldap32.ldap_init(hostName, ((LdapDirectoryIdentifier)this.directoryIdentifier).PortNumber);
			}
		...
		public DirectoryResponse SendRequest(DirectoryRequest request, TimeSpan requestTimeout)
		{
			...
			if (num2 == 0 && num != -1)
			{
				return this.ConstructResponse(num, operation, ResultAll.LDAP_MSG_ALL, requestTimeout, true);
			}
			if (num2 == 0)
			{
				num2 = Wldap32.LdapGetLastError();
			}
			throw this.ConstructException(num2, operation);
		}
		...
		public IAsyncResult BeginSendRequest(DirectoryRequest request, TimeSpan requestTimeout, PartialResultProcessing partialMode, AsyncCallback callback, object state)
		{
			...
			if (partialMode == PartialResultProcessing.NoPartialResultSupport)
			{
				...
				this.fd.BeginInvoke(num, operation, ResultAll.LDAP_MSG_ALL, requestTimeout, true, new AsyncCallback(this.ResponseCallback), ldapRequestState);
			...
			LdapPartialAsyncResult ldapPartialAsyncResult = new LdapPartialAsyncResult(num, callback, state, true, this, partialCallback, requestTimeout);
		...
		private void ResponseCallback(IAsyncResult asyncResult)
		{
			...
			try
			{
				...
				ldapRequestState.response = response;
			}
			catch (Exception exception)
			{
				ldapRequestState.exception = exception;
		...
		public void Abort(IAsyncResult asyncResult)
		{
			...
			{
				throw new ArgumentException(Res.GetString("NotReturnedAsyncResult", new object[]
				{
					"asyncResult"
				}));
			}
			...
			if (!ldapAsyncResult.partialResults)
			{
				...
				messagId = (int)LdapConnection.asyncResultTable[asyncResult];
		...
		private int SendRequestHelper(DirectoryRequest request, ref int messageID)
		{
			...
			try
			{
				...
				if (array != null)
				{
					...
					for (int i = 0; i < array.Length; i++)
					{
						...
						ptr = (IntPtr)((long)intPtr + (long)(Marshal.SizeOf(typeof(IntPtr)) * i));
						Marshal.WriteIntPtr(ptr, intPtr5);
					}
					ptr = (IntPtr)((long)intPtr + (long)(Marshal.SizeOf(typeof(IntPtr)) * array.Length));
				...
				if (array2 != null)
				{
					...
					for (int j = 0; j < array2.Length; j++)
					{
						...
						ptr = (IntPtr)((long)intPtr2 + (long)(Marshal.SizeOf(typeof(IntPtr)) * j));
						Marshal.WriteIntPtr(ptr, intPtr5);
					}
					ptr = (IntPtr)((long)intPtr2 + (long)(Marshal.SizeOf(typeof(IntPtr)) * array2.Length));
					Marshal.WriteIntPtr(ptr, (IntPtr)0);
				}
				if (request is DeleteRequest)
				{
				...
				else if (request is CompareRequest)
				{
					...
					num3 = Wldap32.ldap_compare(this.ldapHandle, ((CompareRequest)request).DistinguishedName, assertion.Name, strValue, berval, intPtr, intPtr2, ref messageID);
				}
				else if (request is AddRequest || request is ModifyRequest)
				{
					...
					for (k = 0; k < num - 1; k++)
					{
						...
						ptr = (IntPtr)((long)intPtr3 + (long)(Marshal.SizeOf(typeof(IntPtr)) * k));
						Marshal.WriteIntPtr(ptr, intPtr5);
					}
					ptr = (IntPtr)((long)intPtr3 + (long)(Marshal.SizeOf(typeof(IntPtr)) * k));
					Marshal.WriteIntPtr(ptr, (IntPtr)0);
						...
						if (request is SearchRequest)
						{
							...
							if (num2 != 0)
							{
								...
								for (l = 0; l < num2; l++)
								{
									...
									ptr = (IntPtr)((long)intPtr4 + (long)(Marshal.SizeOf(typeof(IntPtr)) * l));
									Marshal.WriteIntPtr(ptr, intPtr5);
								}
								ptr = (IntPtr)((long)intPtr4 + (long)(Marshal.SizeOf(typeof(IntPtr)) * l));
								Marshal.WriteIntPtr(ptr, (IntPtr)0);
							...
							int timelimit = (int)(searchRequest.TimeLimit.Ticks / 10000000L);
							DereferenceAlias derefAlias = this.options.DerefAlias;
							this.options.DerefAlias = searchRequest.Aliases;
							try
							{
								num3 = Wldap32.ldap_search(this.ldapHandle, searchRequest.DistinguishedName, scope, filter2, intPtr4, searchRequest.TypesOnly, intPtr, intPtr2, timelimit, searchRequest.SizeLimit, ref messageID);
								goto IL_666;
							}
							finally
							{
			...
			finally
			{
				...
				if (intPtr != (IntPtr)0)
				{
					...
					{
						IntPtr intPtr6 = Marshal.ReadIntPtr(intPtr, Marshal.SizeOf(typeof(IntPtr)) * m);
						if (intPtr6 != (IntPtr)0)
						{
				...
				if (intPtr2 != (IntPtr)0)
				{
					...
					{
						IntPtr intPtr7 = Marshal.ReadIntPtr(intPtr2, Marshal.SizeOf(typeof(IntPtr)) * num4);
						if (intPtr7 != (IntPtr)0)
						{
				...
				if (intPtr3 != (IntPtr)0)
				{
					...
					{
						IntPtr intPtr8 = Marshal.ReadIntPtr(intPtr3, Marshal.SizeOf(typeof(IntPtr)) * num6);
						if (intPtr8 != (IntPtr)0)
						{
				...
				if (intPtr4 != (IntPtr)0)
				{
					...
					{
						IntPtr intPtr9 = Marshal.ReadIntPtr(intPtr4, Marshal.SizeOf(typeof(IntPtr)) * num8);
						if (intPtr9 != (IntPtr)0)
						{
		...
		private bool ProcessClientCertificate(IntPtr ldapHandle, IntPtr CAs, ref IntPtr certificate)
		{
			...
			if (CAs != (IntPtr)0)
			{
				...
				{
					ptr = (IntPtr)((long)secPkgContext_IssuerListInfoEx.aIssuers + (long)(Marshal.SizeOf(typeof(CRYPTOAPI_BLOB)) * i));
					CRYPTOAPI_BLOB cryptoapi_BLOB = (CRYPTOAPI_BLOB)Marshal.PtrToStructure(ptr, typeof(CRYPTOAPI_BLOB));
		...
		private void Connect()
		{
			...
			{
				string message2 = LdapErrorMappings.MapResultCode(num2);
				throw new LdapException(num2, message2);
			}
		...
		private void BindHelper(NetworkCredential newCredential, bool needSetCredential)
		{
			...
			else
			{
				...
				{
					num = Wldap32.ldap_bind_s(this.ldapHandle, null, null, method);
				}
				else
				{
					num = Wldap32.ldap_bind_s(this.ldapHandle, null, sec_WINNT_AUTH_IDENTITY_EX, method);
				}
			}
			if (num == 0)
			{
			...
			{
				message = OperationErrorMappings.MapResultCode(num);
			...
			message = LdapErrorMappings.MapResultCode(num);
			...
			if (directoryAttributes != null && directoryAttributes.Count != 0)
			{
				...
				for (int i = 0; i < directoryAttributes.Count; i++)
				{
					...
					for (k = 0; k < num; k++)
					{
						...
						ptr = (IntPtr)((long)array[i].values + (long)(Marshal.SizeOf(typeof(IntPtr)) * k));
		...
		internal DirectoryResponse ConstructResponse(int messageId, LdapOperation operation, ResultAll resultType, TimeSpan requestTimeOut, bool exceptionOnTimeOut)
		{
			...
			if (num != -1 && num != 0)
			{
				...
				try
				{
					...
					if (num != 100 && num != 115)
					{
						num3 = this.ConstructParsedResult(intPtr, ref num2, ref dn, ref message, ref referral, ref controls);
					}
					if (num3 != 0)
					{
						num = num3;
					...
					if (num == 105)
					{
						directoryResponse = new AddResponse(dn, controls, (ResultCode)num3, message, referral);
					}
					else if (num == 103)
					{
						directoryResponse = new ModifyResponse(dn, controls, (ResultCode)num3, message, referral);
					}
					else if (num == 107)
					{
						directoryResponse = new DeleteResponse(dn, controls, (ResultCode)num3, message, referral);
					}
					else if (num == 109)
					{
						directoryResponse = new ModifyDNResponse(dn, controls, (ResultCode)num3, message, referral);
					}
					else if (num == 111)
					{
						directoryResponse = new CompareResponse(dn, controls, (ResultCode)num3, message, referral);
					}
					else if (num == 120)
					{
						directoryResponse = new ExtendedResponse(dn, controls, (ResultCode)num3, message, referral);
						if (num3 == 0)
						{
							...
							if (num3 == 0)
							{
								...
								if (intPtr2 != (IntPtr)0)
								{
									name = Marshal.PtrToStringUni(intPtr2);
								}
								byte[] array = null;
								if (intPtr3 != (IntPtr)0)
								{
					...
					{
						directoryResponse = new SearchResponse(dn, controls, (ResultCode)num3, message, referral);
						if (num == 101)
						{
			...
			try
			{
				...
				if (num == 0)
				{
					...
					if (intPtr3 != (IntPtr)0)
					{
						...
						while (ptr2 != null)
						{
							string value = Marshal.PtrToStringUni((IntPtr)((void*)ptr2));
							arrayList.Add(value);
							num2++;
					...
					if (intPtr4 != (IntPtr)0)
					{
						int num3 = 0;
						IntPtr ptr3 = intPtr4;
						IntPtr intPtr5 = Marshal.ReadIntPtr(ptr3, 0);
						ArrayList arrayList2 = new ArrayList();
						while (intPtr5 != (IntPtr)0)
						{
							DirectoryControl value2 = this.ConstructControl(intPtr5);
							arrayList2.Add(value2);
							num3++;
							intPtr5 = Marshal.ReadIntPtr(ptr3, num3 * Marshal.SizeOf(typeof(IntPtr)));
						}
		...
		internal DirectoryAttribute ConstructAttribute(IntPtr entryMessage, IntPtr attributeName)
		{
			...
			directoryAttribute.Name = name;
			IntPtr intPtr = Wldap32.ldap_get_values_len(this.ldapHandle, entryMessage, name);
			try
			{
				IntPtr intPtr2 = (IntPtr)0;
				int num = 0;
			...
			try
			{
				if (num == 0)
				{
					...
					if (intPtr != (IntPtr)0)
					{
						...
						while (intPtr2 != (IntPtr)0)
						{
							string value = Marshal.PtrToStringUni(intPtr2);
							arrayList.Add(value);
							num2++;
							intPtr2 = Marshal.ReadIntPtr(intPtr, Marshal.SizeOf(typeof(IntPtr)) * num2);
						}
					...
					if (arrayList.Count > 0)
					{
						...
						result = new SearchResultReference(array);
					}
				}
			}
			finally
		...
		private DirectoryControl ConstructControl(IntPtr controlPtr)
		{
			...
			return new DirectoryControl(type, array, ldctl_iscritical, true);
		}
