...
namespace System.DirectoryServices.Protocols
{
	...
	public sealed class BerConverter
	{
		...
		public static byte[] Encode(string format, params object[] value)
		{
			...
			foreach (char c in format)
			{
				...
				else if (c == 's')
				{
					...
					if (value[num] != null)
					{
						tempValue = utf8Encoding.GetBytes((string)value[num]);
					}
					num2 = BerConverter.EncodingByteArrayHelper(berElement, tempValue, 'o');
					num++;
				}
				else if (c == 'o' || c == 'X')
				{
					...
					byte[] tempValue2 = (byte[])value[num];
					num2 = BerConverter.EncodingByteArrayHelper(berElement, tempValue2, c);
					num++;
				}
				else if (c == 'v')
				{
					...
					num2 = BerConverter.EncodingMultiByteArrayHelper(berElement, array3, 'V');
					num++;
				}
				else
				{
					...
					num2 = BerConverter.EncodingMultiByteArrayHelper(berElement, tempValue3, c);
					num++;
				}
				if (num2 == -1)
				{
			...
			try
			{
				int num2 = Wldap32.ber_flatten(berElement, ref intPtr);
				if (num2 == -1)
				{
					throw new BerConversionException();
				}
		...
		{
			bool flag;
			object[] result = BerConverter.TryDecode(format, value, out flag);
			if (flag)
			{
				return result;
			}
		...
		internal static object[] TryDecode(string format, byte[] value, out bool decodeSucceeded)
		{
			...
			foreach (char c in format)
			{
				if (c == '{' || c == '}' || c == '[' || c == ']' || c == 'n' || c == 'x')
				{
					num = Wldap32.ber_scanf(berElement, new string(c, 1));
					if (num != 0)
					{
					}
				}
				...
				{
					int num2 = 0;
					num = Wldap32.ber_scanf_int(berElement, new string(c, 1), ref num2);
					if (num == 0)
					{
						if (c == 'b')
						{
				...
				{
					byte[] array2 = BerConverter.DecodingByteArrayHelper(berElement, 'O', ref num);
					if (num == 0)
					{
						string value2 = null;
						if (array2 != null)
						{
							value2 = utf8Encoding.GetString(array2);
						}
				...
				{
					byte[] value3 = BerConverter.DecodingByteArrayHelper(berElement, c, ref num);
					if (num == 0)
					{
				...
				else if (c == 'B')
				{
					IntPtr intPtr = (IntPtr)0;
					int num3 = 0;
					num = Wldap32.ber_scanf_bitstring(berElement, "B", ref intPtr, ref num3);
					if (num == 0)
					{
						byte[] array3 = null;
						if (intPtr != (IntPtr)0)
						{
							...
							Marshal.Copy(intPtr, array3, 0, num3);
						}
				...
				{
					string[] array4 = null;
					byte[][] array5 = BerConverter.DecodingMultiByteArrayHelper(berElement, 'V', ref num);
					if (num == 0)
					{
						if (array5 != null)
						{
							array4 = new string[array5.Length];
							for (int j = 0; j < array5.Length; j++)
							{
								...
								{
									array4[j] = utf8Encoding.GetString(array5[j]);
								}
							}
						}
				...
				else
				{
					...
					byte[][] value4 = BerConverter.DecodingMultiByteArrayHelper(berElement, c, ref num);
					if (num == 0)
					{
		...
		private static int EncodingByteArrayHelper(BerSafeHandle berElement, byte[] tempValue, char fmt)
		{
			...
			if (tempValue != null)
			{
				...
				result = Wldap32.ber_printf_bytearray(berElement, new string(fmt, 1), value, tempValue.Length);
			}
			else
			{
				result = Wldap32.ber_printf_bytearray(berElement, new string(fmt, 1), new HGlobalMemHandle((IntPtr)0), 0);
			}
			return result;
		}
			...
			try
			{
				if (tempValue != null)
				{
					...
					for (i = 0; i < tempValue.Length; i++)
					{
						...
						Marshal.StructureToPtr(array[i], intPtr2, false);
						ptr = (IntPtr)((long)intPtr + (long)(Marshal.SizeOf(typeof(IntPtr)) * i));
						Marshal.WriteIntPtr(ptr, intPtr2);
					}
					ptr = (IntPtr)((long)intPtr + (long)(Marshal.SizeOf(typeof(IntPtr)) * i));
					Marshal.WriteIntPtr(ptr, (IntPtr)0);
				}
				result = Wldap32.ber_printf_berarray(berElement, new string(fmt, 1), intPtr);
				GC.KeepAlive(array);
			}
			finally
			{
				if (intPtr != (IntPtr)0)
				{
					...
					{
						IntPtr intPtr3 = Marshal.ReadIntPtr(intPtr, Marshal.SizeOf(typeof(IntPtr)) * j);
						if (intPtr3 != (IntPtr)0)
						{
