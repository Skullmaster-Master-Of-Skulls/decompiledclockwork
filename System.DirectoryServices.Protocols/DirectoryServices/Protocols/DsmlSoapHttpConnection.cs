	...
	public class DsmlSoapHttpConnection : DsmlSoapConnection
	{
		...
		public AuthType AuthType
		{
			...
			set
			{
				...
				{
					throw new ArgumentException(Res.GetString("WrongAuthType", new object[]
					{
						value
					}), "value");
				}
		...
		public override DirectoryResponse SendRequest(DirectoryRequest request)
		{
			...
			DsmlResponseDocument dsmlResponseDocument = this.SendRequestHelper(new DsmlRequestDocument
			{
				request
			}.ToXml().InnerXml);
		...
		private void PrepareHttpWebRequest(HttpWebRequest dsmlConnection)
		{
			...
			else
			{
				...
				dsmlConnection.Credentials = new CredentialCache
				{
					{
						dsmlConnection.RequestUri,
						authType,
						this.directoryCredential
					}
				};
			}
			foreach (X509Certificate value in base.ClientCertificates)
			{
				dsmlConnection.ClientCertificates.Add(value);
			}
		...
		private static void ReadCallback(IAsyncResult asyncResult)
		{
			...
			try
			{
				...
				if (num > 0)
				{
					...
					int count = Math.Min(@string.Length, num);
					requestState.responseString.Append(@string, 0, count);
					requestState.responseStream.BeginRead(requestState.bufferRead, 0, 1024, new AsyncCallback(DsmlSoapHttpConnection.ReadCallback), requestState);
				}
				else
				{
