	...
	public class LdapSessionOptions
	{
		...
		public bool SecureSocketLayer
		{
			...
			set
			{
				...
				this.SetIntValueHelper(LdapOption.LDAP_OPT_SSL, value2);
			}
		}
		...
		public TimeSpan PingKeepAliveTimeout
		{
			...
			set
			{
				...
				int value2 = (int)(value.Ticks / 10000000L);
				this.SetIntValueHelper(LdapOption.LDAP_OPT_PING_KEEP_ALIVE, value2);
			}
		}
		...
		public TimeSpan PingWaitTimeout
		{
			...
			set
			{
				...
				int value2 = (int)(value.Ticks / 10000L);
				this.SetIntValueHelper(LdapOption.LDAP_OPT_PING_WAIT_TIME, value2);
			}
		}
		...
		public bool AutoReconnect
		{
			...
			set
			{
				...
				this.SetIntValueHelper(LdapOption.LDAP_OPT_AUTO_RECONNECT, value2);
			}
		}
		...
		public SecurityPackageContextConnectionInformation SslInformation
		{
			get
			{
				...
				int error = Wldap32.ldap_get_option_secInfo(this.connection.ldapHandle, LdapOption.LDAP_OPT_SSL_INFO, securityPackageContextConnectionInformation);
		...
		public object SecurityContext
		{
			get
			{
				...
				int error = Wldap32.ldap_get_option_sechandle(this.connection.ldapHandle, LdapOption.LDAP_OPT_SECURITY_CONTEXT, ref securityHandle);
				ErrorChecking.CheckAndSetLdapError(error);
		...
		public bool Signing
		{
			...
			set
			{
				...
				this.SetIntValueHelper(LdapOption.LDAP_OPT_SIGN, value2);
			}
		}
		...
		public bool Sealing
		{
			...
			set
			{
				...
				this.SetIntValueHelper(LdapOption.LDAP_OPT_ENCRYPT, value2);
			}
		}
		...
		public bool RootDseCache
		{
			...
			set
			{
				...
				this.SetIntValueHelper(LdapOption.LDAP_OPT_ROOTDSE_CACHE, value2);
			}
		}
		...
		public bool TcpKeepAlive
		{
			...
			set
			{
				...
				this.SetIntValueHelper(LdapOption.LDAP_OPT_TCP_KEEPALIVE, value2);
			}
		}
		...
		public TimeSpan SendTimeout
		{
			...
			set
			{
				...
				int value2 = (int)(value.Ticks / 10000000L);
				this.SetIntValueHelper(LdapOption.LDAP_OPT_SEND_TIMEOUT, value2);
			}
		}
		...
		public VerifyServerCertificateCallback VerifyServerCertificate
		{
			...
			set
			{
				...
				if (value != null)
				{
					int error = Wldap32.ldap_set_option_servercert(this.connection.ldapHandle, LdapOption.LDAP_OPT_SERVER_CERTIFICATE, this.serverCertificateRoutine);
					ErrorChecking.CheckAndSetLdapError(error);
				}
		...
		public unsafe void StartTransportLayerSecurity(DirectoryControlCollection controls)
		{
			...
			try
			{
				...
				int num2 = Wldap32.ldap_start_tls(this.connection.ldapHandle, ref num, ref intPtr3, intPtr, intPtr2);
				...
				{
					char** ptr2 = (char**)((void*)intPtr4);
					char* ptr3 = *(IntPtr*)(ptr2 + 0 / sizeof(char*));
					int num3 = 0;
					ArrayList arrayList = new ArrayList();
					while (ptr3 != null)
					{
						string value = Marshal.PtrToStringUni((IntPtr)((void*)ptr3));
						arrayList.Add(value);
						num3++;
						ptr3 = *(IntPtr*)(ptr2 + (IntPtr)num3 * (IntPtr)sizeof(char*) / (IntPtr)sizeof(char*));
					}
					if (intPtr4 != (IntPtr)0)
					{
				...
				if (num2 != 0)
				{
					string message = Res.GetString("DefaultLdapError");
					if (Utility.IsResultCode((ResultCode)num2))
					{
						...
						message = OperationErrorMappings.MapResultCode(num2);
						throw new TlsOperationException(new ExtendedResponse(null, null, (ResultCode)num2, message, array3)
						{
					...
					{
						message = LdapErrorMappings.MapResultCode(num2);
			...
			finally
			{
				if (intPtr != (IntPtr)0)
				{
					...
					{
						IntPtr intPtr6 = Marshal.ReadIntPtr(intPtr, Marshal.SizeOf(typeof(IntPtr)) * l);
						if (intPtr6 != (IntPtr)0)
						{
				...
				if (intPtr2 != (IntPtr)0)
				{
					...
					{
						IntPtr intPtr7 = Marshal.ReadIntPtr(intPtr2, Marshal.SizeOf(typeof(IntPtr)) * n);
						if (intPtr7 != (IntPtr)0)
						{
				...
				if (array2 != null)
				{
					...
					{
						if (array2[num4].ldctl_oid != (IntPtr)0)
						{
							Marshal.FreeHGlobal(array2[num4].ldctl_oid);
						}
						if (array2[num4].ldctl_value != null && array2[num4].ldctl_value.bv_val != (IntPtr)0)
						{
							Marshal.FreeHGlobal(array2[num4].ldctl_value.bv_val);
						}
					}
				}
				if (intPtr4 != (IntPtr)0)
		...
		private int GetIntValueHelper(LdapOption option)
		{
			...
			int result = 0;
			int error = Wldap32.ldap_get_option_int(this.connection.ldapHandle, option, ref result);
			ErrorChecking.CheckAndSetLdapError(error);
			return result;
		}
		...
		private void SetIntValueHelper(LdapOption option, int value)
		{
			...
			int num = value;
			int error = Wldap32.ldap_set_option_int(this.connection.ldapHandle, option, ref num);
			ErrorChecking.CheckAndSetLdapError(error);
		}
		...
		private string GetStringValueHelper(LdapOption option, bool releasePtr)
		{
			...
			int error = Wldap32.ldap_get_option_ptr(this.connection.ldapHandle, option, ref intPtr);
			ErrorChecking.CheckAndSetLdapError(error);
			string result = null;
			if (intPtr != (IntPtr)0)
			{
		...
		private void SetStringValueHelper(LdapOption option, string value)
		{
			...
			try
			{
				int error = Wldap32.ldap_set_option_ptr(this.connection.ldapHandle, option, ref intPtr);
				ErrorChecking.CheckAndSetLdapError(error);
			}
			finally
			{
		...
		private void ProcessCallBackRoutine(ReferralCallback tempCallback)
		{
			...
			int error = Wldap32.ldap_set_option_referral(this.connection.ldapHandle, LdapOption.LDAP_OPT_REFERRAL_CALLBACK, ref ldapReferralCallback);
			ErrorChecking.CheckAndSetLdapError(error);
		}
		...
		private int ProcessQueryConnection(IntPtr PrimaryConnection, IntPtr ReferralFromConnection, IntPtr NewDNPtr, string HostName, int PortNumber, SEC_WINNT_AUTH_IDENTITY_EX SecAuthIdentity, Luid CurrentUserToken, ref IntPtr ConnectionToUse)
		{
			...
			if (this.callbackRoutine.QueryForConnection != null)
			{
				...
				long currentUserToken = (long)((ulong)CurrentUserToken.LowPart + (ulong)((ulong)((long)CurrentUserToken.HighPart) << 32));
				LdapConnection ldapConnection2 = this.callbackRoutine.QueryForConnection(this.connection, ldapConnection, newDistinguishedName, identifier, credential, currentUserToken);
				...
				lock (LdapConnection.objectLock)
				{
					...
					if (NewConnection != (IntPtr)0)
					{
						...
						else
						{
							...
							ldapConnection = new LdapConnection(identifier, credential, this.connection.AuthType, NewConnection);
							LdapConnection.handleTable.Add(NewConnection, new WeakReference(ldapConnection));
						}
					}
				}
				long currentUserToken = (long)((ulong)CurrentUser.LowPart + (ulong)((ulong)((long)CurrentUser.HighPart) << 32));
				bool flag = this.callbackRoutine.NotifyNewConnection(this.connection, ldapConnection2, newDistinguishedName, identifier, ldapConnection, credential, currentUserToken, ErrorCodeFromBind);
				if (flag)
				{
		...
		private int ProcessDereferenceConnection(IntPtr PrimaryConnection, IntPtr ConnectionToDereference)
		{
			if (ConnectionToDereference != (IntPtr)0 && this.callbackRoutine.DereferenceConnection != null)
			{
				...
				{
					connectionToDereference = (LdapConnection)weakReference.Target;
				}
		...
		private bool ProcessServerCertificate(IntPtr Connection, IntPtr pServerCert)
		{
			...
			if (this.serverCertificateDelegate != null)
			{
				...
				result = this.serverCertificateDelegate(this.connection, certificate);
			}
			return result;
		}
