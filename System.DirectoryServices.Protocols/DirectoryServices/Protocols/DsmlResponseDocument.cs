		...
		internal DsmlResponseDocument(HttpWebResponse resp, string xpathToResponse) : this()
		{
			...
			try
			{
				...
				foreach (object obj in childNodes)
				{
					...
					{
						DirectoryResponse value = this.ConstructElement((XmlElement)xmlNode);
						this.dsmlResponse.Add(value);
					}
				}
			}
			finally
		...
		private DirectoryResponse ConstructElement(XmlElement node)
		{
			...
			if ((localName = node.LocalName) != null)
			{
				...
				if (<PrivateImplementationDetails>{0F2CC45B-8908-4A79-99BF-EFE68DF0A5C8}.$$method0x60001f1-1.TryGetValue(localName, out num))
				{
					...
					{
					case 0:
						result = new DsmlErrorResponse(node);
						break;
					case 1:
						result = new SearchResponse(node);
						break;
					case 2:
						result = new ModifyResponse(node);
						break;
					case 3:
						result = new AddResponse(node);
						break;
					case 4:
						result = new DeleteResponse(node);
						break;
					case 5:
						result = new ModifyDNResponse(node);
						break;
					case 6:
						result = new CompareResponse(node);
						break;
					case 7:
						result = new ExtendedResponse(node);
						break;
					case 8:
						result = new DsmlAuthResponse(node);
						break;
					default:
						goto IL_120;
					}
