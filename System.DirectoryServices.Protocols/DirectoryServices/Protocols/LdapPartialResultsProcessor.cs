...
namespace System.DirectoryServices.Protocols
{
	...
	internal class LdapPartialResultsProcessor
	{
		...
		private void GetResultsHelper(LdapPartialAsyncResult asyncResult)
		{
			...
			try
			{
				SearchResponse searchResponse = (SearchResponse)con.ConstructResponse(asyncResult.messageID, LdapOperation.LdapSearch, resultType, asyncResult.requestTimeout, false);
		...
		{
			PartialResultsCollection result;
			lock (this)
			{
				if (!this.resultList.Contains(asyncResult))
				{
				...
				result = partialResultsCollection;
			}
			return result;
		}
